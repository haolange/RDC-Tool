...
from rdx.runtime_catalog import describe_operation, validate_operation_arguments
import sys
...
def run_batch(source: str, execute: Callable[[str, dict[str, Any]], dict[str, Any]]) -> int:
    ...
    try:
        with Path(source).open("rb") as stream:
            for index, line in enumerate(stream):
                ...
                try:
                    ...
                    payload = canonical_error(result_kind=operation or "rdx.batch", code="batch_cancelled" if isinstance(exc, InterruptedError) else "batch_failed",
        ...
        payload = canonical_error(result_kind="rdx.batch", code="batch_input_failed", category="validation", message=str(exc), transport="cli")
