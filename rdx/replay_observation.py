...
from rdx import server_runtime as runtime
...
async def handle(action: str, args: dict[str, Any]) -> str:
    ...
    if args.get("session_id") and str(args["session_id"]) != session_id:
    ...
    _, flat, by_event = await runtime._load_action_index(session_id)
    events = sorted({int(a.eventId): {"event_id": int(a.eventId), "name": runtime._action_name(a)}
    ...
    result: dict[str, Any] = {
        ...
        "remote_display": {"status": "unsupported", "reason": "Remote replay client exposes no device presentation acknowledgement"}
            if backend == "remote" else {"status": "not_applicable"},
    }
    try:
