...
- **Python 包管理**：使用 `setuptools` + `wheel`（`pyproject.toml` 中声明 `build-backend = "setuptools.build_meta"`），通过 `[project.scripts]` 暴露 `rdx = rdx.cli:main` 作为唯一公共命令。
...
- **版本来源**：单一来源 `rdx/__init__.py` 中的 `__version__ = "1.0.0"`，被 `package_release.py` 直接 import 用于制品命名与元数据。

## 2. 关键文件与职责

| 文件 | 职责 |
|---|---|
| `pyproject.toml` | 定义包名 `rdx-tools`、依赖、可选 dev 依赖、pytest 配置、entrypoint |
| `rdx/__init__.py` | 单一版本源 `__version__` |
| `scripts/_shared.py` | 共享路径解析、子进程封装、JSON 读写、`tools_root()` 定位仓库根（支持 `RDX_TOOLS_ROOT` 覆盖） |
| `scripts/package_runtime.py` | 将 RenderDoc 原生二进制与可选 CPython home 拷贝到 `binaries/windows/x64/`，生成 `manifest.runtime.json` |
| `scripts/package_release.py` | 从仓库树复制受控目录到 staging，生成 `RELEASE_MANIFEST.json`、`LICENSE_INVENTORY.json`、`SBOM.json`，压缩为 `dist/rdx-tools-{version}-windows-x64.zip` 并写 `SHA256SUMS` |
...
| `scripts/smoke_cli.sh` | Bash 冒烟脚本，按步骤执行 `rdx doctor/tools list/capture open/vfs` 等，输出 `intermediate/logs/smoke_cli.log` 与 findings |
...
| `spec/build_catalog.py` | 从 `rdx.runtime_catalog.catalog_payload()` 生成 `spec/tool_catalog.json`，支持 `--check` 模式 |
...
| `rdx.bat` / `bin/rdx` / `cli/run_cli.py` | 三个受保护的 public entrypoints，被 manifest 与 verify 严格约束 |
...
2. **Release 打包**：`python scripts/package_release.py [--out-dir dist] [--version X.Y.Z]` 仅复制白名单目录（`bin`, `binaries`, `cli`, `docs`, `policy`, `rdx`, `scripts`, `spec`）及根级文件（`.gitattributes`, `.gitignore`, `AGENTS.md`, `CHANGELOG.md`, `LICENSE`, `THIRD_PARTY_NOTICES.md`, `README.md`, `pyproject.toml`, `rdx.bat`），跳过 `tests/`、`intermediate/`、`dist/`、`__pycache__`、`.mypy_cache`、`.ruff_cache`、`.venv` 以及 `.pyc/.pyo/.pdb/.ilk/.exp/.lib/.rdc` 等后缀。最终产出 `dist/rdx-tools-{version}-windows-x64.zip`、同目录 `SHA256SUMS` 与 `release_report.md`。
...
4. **门禁**：`python scripts/release_gate.py [--require-release-package] [--require-smoke-reports] [--report intermediate/logs/release_gate_report.md]` 依次执行：
   ...
   - 用户文档不得出现 `uv.lock`、`uv sync`、`python -m venv`、`pip install`、`rdx.bat <command>` 示例
...
- **Release manifest**：`rdx-tools/RELEASE_MANIFEST.json`，记录 name/version/platform/public_commands/entrypoints/file_count/files。
- **License Inventory**：`rdx-tools/LICENSE_INVENTORY.json`，遍历 `binaries/windows/x64/python/Lib/site-packages/*.dist-info/METADATA` 提取组件名/版本/许可证。
- **SBOM**：`rdx-tools/SBOM.json`，schema `rdx-tools.sbom.v1`，components 来自 license inventory。
- **Checksums**：`dist/SHA256SUMS` 列出 zip 文件名与 sha256。

### 3.3 入口点约束
- 公开命令固定为 `rdx`，对应 entrypoints 列表 `rdx.bat`、`bin/rdx`、`cli/run_cli.py` 三者必须存在于包内且 manifest 中声明一致。
- 用户文档中禁止出现 `rdx.bat <command>` 形式的命令示例，只能将其作为 launcher-file 引用。

## 4. 约定与约束

- **版本单一来源**：所有制品版本号来自 `rdx/__init__.py.__version__`；`package_release.py` 在启动时比对 `--version` 参数与导入值，不一致即返回 2。
...
- **文档即契约**：`docs/tool-reference.md` 必须与 `generate_tool_reference()` 输出完全一致；用户文档不得包含 `uv.lock`、`uv sync`、`python -m venv`、`pip install`、`rdx.bat <cmd>` 等文本。
...
- **工作区隔离**：所有中间产物（staging、artifacts、logs、pytest 缓存、context snapshot、worker-state）统一放在 `intermediate/`，构建脚本通过 `rdx.runtime_paths.intermediate_root()` 访问，避免污染源码树。
- **路径安全**：`scripts/_shared.py` 提供 `ensure_within_root()` 防止 `--source`/`--python-home` 等参数逃逸仓库根；`tools_root()` 支持通过 `RDX_TOOLS_ROOT` 环境变量覆盖仓库根定位。