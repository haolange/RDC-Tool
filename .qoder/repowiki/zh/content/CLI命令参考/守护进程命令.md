# 守护进程命令

<cite>
**本文引用的文件**
- [rdx/cli.py](file://rdx/cli.py)
- [rdx/daemon/client.py](file://rdx/daemon/client.py)
- [rdx/daemon/server.py](file://rdx/daemon/server.py)
- [rdx/daemon/worker.py](file://rdx/daemon/worker.py)
- [tests/test_cli_daemon_status.py](file://tests/test_cli_daemon_status.py)
- [tests/test_daemon_client.py](file://tests/test_daemon_client.py)
- [cli/run_cli.py](file://cli/run_cli.py)
</cite>

## 更新摘要
**所做更改**
- 新增基于所有者PID的守护进程所有权管理系统
- 增强守护进程生命周期管理，支持自动清理当父进程意外退出
- 添加 `--owner-pid` CLI参数用于指定宿主进程PID
- 改进进程所有权跟踪和资源清理机制
- 更新多上下文守护进程管理以支持所有者PID功能

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件系统性阐述守护进程命令，覆盖以下方面：
- 命令与用法：daemon start、daemon stop、daemon status 及其参数（如 --daemon-context、--owner-pid）
- 启动、停止与状态查询机制
- 上下文管理与多上下文守护进程
- 生命周期管理与健康检查（租约、空闲超时、所有者进程监控）
- 故障排除与重启策略
- 多上下文守护进程的管理方法

**更新** 本次更新增强了守护进程的生命周期管理，引入了基于所有者PID的所有权系统，确保当父进程意外退出时能够正确清理守护进程资源。

## 项目结构
守护进程相关能力由 CLI 子命令、客户端与服务端实现协同完成，并通过"上下文"隔离不同守护进程实例。

```mermaid
graph TB
subgraph "CLI 层"
CLI["rdx/cli.py<br/>定义子命令与参数解析"]
RUNCLI["cli/run_cli.py<br/>帮助与示例输出"]
end
subgraph "守护进程客户端"
DCLIENT["rdx/daemon/client.py<br/>状态加载/保存/请求封装"]
end
subgraph "守护进程服务端"
DSERV["rdx/daemon/server.py<br/>运行时/状态持久化/健康检查"]
DWORKER["rdx/daemon/worker.py<br/>工作进程管理"]
end
subgraph "测试"
TSTATUS["tests/test_cli_daemon_status.py"]
TCLIENT["tests/test_daemon_client.py"]
end
CLI --> DCLIENT
DCLIENT --> DSERV
DSERV --> DWORKER
RUNCLI -. 帮助/示例 .-> CLI
TSTATUS -. 验证行为 .-> CLI
TCLIENT -. 验证行为 .-> DCLIENT
```

**图示来源**
- [rdx/cli.py:1381-1436](file://rdx/cli.py#L1381-L1436)
- [rdx/daemon/client.py:199-604](file://rdx/daemon/client.py#L199-L604)
- [rdx/daemon/server.py:101-545](file://rdx/daemon/server.py#L101-L545)
- [rdx/daemon/worker.py](file://rdx/daemon/worker.py)
- [cli/run_cli.py:88-150](file://cli/run_cli.py#L88-L150)
- [tests/test_cli_daemon_status.py](file://tests/test_cli_daemon_status.py)
- [tests/test_daemon_client.py](file://tests/test_daemon_client.py)

章节来源
- [rdx/cli.py:1381-1436](file://rdx/cli.py#L1381-L1436)
- [cli/run_cli.py:88-150](file://cli/run_cli.py#L88-L150)

## 核心组件
- CLI 子命令与参数
  - daemon start：支持 --pipe-name、--owner-pid、--daemon-context 等
  - daemon stop：停止指定上下文的守护进程
  - daemon status：查询守护进程状态
  - --daemon-context：指定守护进程上下文标识，默认为 "default"
  - **新增** --owner-pid：指定宿主进程PID，用于自动停止检测
- 守护进程客户端
  - 负责状态载入/保存、规范化状态字段、向服务端发起请求
  - 提供 ensure_daemon：确保单上下文唯一运行、清理陈旧状态
  - **新增** 支持所有者PID传递和所有权声明
- 守护进程服务端
  - 维护运行时状态、令牌鉴权、活动时间戳、客户端租约与空闲超时
  - **增强** 所有者PID监控，当所有者进程丢失且租约过期时自动停止
  - 自检循环：在无附加客户端且空闲超时或租约过期时自动停止
- 测试
  - 验证 CLI 守护进程状态命令行为
  - 验证客户端状态处理与清理逻辑
  - **新增** 验证所有者PID功能和自动清理机制

章节来源
- [rdx/cli.py:1381-1436](file://rdx/cli.py#L1381-L1436)
- [rdx/daemon/client.py:199-604](file://rdx/daemon/client.py#L199-L604)
- [rdx/daemon/server.py:101-545](file://rdx/daemon/server.py#L101-L545)
- [tests/test_cli_daemon_status.py](file://tests/test_cli_daemon_status.py)
- [tests/test_daemon_client.py](file://tests/test_daemon_client.py)

## 架构总览
守护进程命令的调用链路如下：

```mermaid
sequenceDiagram
participant U as "用户"
participant C as "CLI(rdx/cli.py)"
participant CC as "客户端(rdx/daemon/client.py)"
participant S as "服务端(rdx/daemon/server.py)"
U->>C : "rdx daemon start/stop/status --daemon-context <ctx> --owner-pid <pid>"
alt start
C->>CC : "ensure_daemon(..., owner_pid=...)"
CC->>S : "status(若已有实例)"
S-->>CC : "返回状态"
CC-->>C : "已运行/启动新实例"
else stop
C->>CC : "daemon_request('stop', ...)"
CC->>S : "转发停止请求"
S-->>CC : "确认"
CC-->>C : "完成"
else status
C->>CC : "load_daemon_state(context)"
CC->>S : "status(若可连通)"
S-->>CC : "状态"
CC-->>C : "结果"
end
```

**图示来源**
- [rdx/cli.py:1634-1654](file://rdx/cli.py#L1634-L1654)
- [rdx/daemon/client.py:623-733](file://rdx/daemon/client.py#L623-L733)
- [rdx/daemon/server.py:572-643](file://rdx/daemon/server.py#L572-L643)

## 详细组件分析

### CLI 子命令与参数
- 子命令定义
  - daemon start：添加管道名、所有者进程PID等参数
  - daemon stop：停止当前上下文守护进程
  - daemon status：查询状态
  - daemon attach/detach/heartbeat/cleanup：内部维护接口
- 参数
  - --daemon-context：上下文标识符，默认"default"
  - **新增** --owner-pid：宿主进程PID，用于守护进程自动停止检测

**更新** 新增了 `--owner-pid` 参数，允许指定宿主进程的PID。当宿主进程意外退出时，守护进程会在租约过期后自动停止，确保资源正确清理。

章节来源
- [rdx/cli.py:1381-1436](file://rdx/cli.py#L1381-L1436)
- [cli/run_cli.py:88-150](file://cli/run_cli.py#L88-L150)

### 守护进程客户端
- 状态规范化
  - 规范化上下文、时间戳、超时、附加客户端列表等字段
  - **新增** 规范化所有者PID字段
- 状态持久化
  - 将运行时状态写入磁盘，便于后续查询与恢复
- 请求封装
  - 通过 daemon_request 发起 RPC 请求，携带令牌与上下文
- ensure_daemon
  - 清理陈旧状态后判断是否已有运行中的守护进程
  - **新增** 支持所有权声明：如果守护进程已运行但所有者PID为空，会尝试声明所有权
  - 若存在且可通信则返回"已运行"，否则尝试启动新实例

```mermaid
flowchart TD
Start(["进入 ensure_daemon"]) --> Clean["清理陈旧状态"]
Clean --> Load["加载上下文状态"]
Load --> Exists{"是否存在有效PID?"}
Exists --> |是| CheckOwner{"所有者PID是否为空?"}
CheckOwner --> |是| Claim["尝试声明所有权"]
Claim --> Ping["向服务端 status 请求"]
CheckOwner --> |否| Ping
Ping --> Ok{"通信成功?"}
Ok --> |是| Update["从响应更新本地状态"] --> Return1["返回 已运行"]
Ok --> |否| Clear["清理状态并清除会话状态"] --> New["启动新实例"]
Exists --> |否| New
New --> Return2["返回 新实例已启动"]
```

**图示来源**
- [rdx/daemon/client.py:623-733](file://rdx/daemon/client.py#L623-L733)
- [rdx/daemon/client.py:202-226](file://rdx/daemon/client.py#L202-L226)

章节来源
- [rdx/daemon/client.py:199-604](file://rdx/daemon/client.py#L199-L604)

### 守护进程服务端
- 运行时模型
  - 维护管道名、令牌、上下文、所有者进程PID、租约与空闲超时
  - **增强** 所有者PID监控：定期检查所有者进程是否仍在运行
  - 持久化状态到磁盘，记录最后活动时间、附加客户端、活跃请求数等
- 鉴权与请求处理
  - 以令牌校验请求合法性
  - **新增** claim_owner 方法：允许新的所有者声明对守护进程的所有权
  - 分发具体方法（status/stop/attach/detach等）
- 健康检查与自停
  - 无附加客户端且空闲超时或租约过期时触发停止
  - **增强** 所有者进程丢失检测：当所有者进程丢失且无附加客户端且无活跃请求时触发停止

```mermaid
classDiagram
class DaemonRuntime {
+string pipe_name
+string token
+string daemon_context
+int owner_pid
+int lease_timeout_seconds
+int idle_timeout_seconds
+bool running
+Dict state
+_auth(request) tuple
+_persist_state() void
+_touch_activity() void
+_prune_clients_locked(now_ms) void
_handle_claim_owner(params) Dict
handle_request(request) Dict
_watch_lifecycle() void
}
```

**图示来源**
- [rdx/daemon/server.py:101-163](file://rdx/daemon/server.py#L101-L163)
- [rdx/daemon/server.py:164-168](file://rdx/daemon/server.py#L164-L168)
- [rdx/daemon/server.py:169-178](file://rdx/daemon/server.py#L169-L178)
- [rdx/daemon/server.py:179-192](file://rdx/daemon/server.py#L179-L192)
- [rdx/daemon/server.py:401-413](file://rdx/daemon/server.py#L401-L413)
- [rdx/daemon/server.py:532-570](file://rdx/daemon/server.py#L532-L570)
- [rdx/daemon/server.py:572-643](file://rdx/daemon/server.py#L572-L643)

章节来源
- [rdx/daemon/server.py:101-545](file://rdx/daemon/server.py#L101-L545)

### CLI 状态查询流程
- 若无上下文状态或无法连接服务端，则清理陈旧状态并返回未运行
- 若可连接，返回服务端最新状态
- 异常场景会尝试清理并重试

```mermaid
sequenceDiagram
participant CLI as "CLI"
participant CC as "客户端"
participant S as "服务端"
CLI->>CC : "_daemon_status_payload(context)"
CC->>CC : "load_daemon_state(context)"
alt 无状态
CC->>CC : "cleanup_stale_daemon_states(context)"
CC-->>CLI : "{running : false}"
else 有状态
CC->>S : "status"
alt 成功
S-->>CC : "状态"
CC-->>CLI : "状态"
else 失败
CC->>CC : "cleanup_stale_daemon_states(context)"
CC->>CC : "load_daemon_state(context)"
alt 仍无状态
CC-->>CLI : "{running : false, cleaned : true}"
else 有状态
CC-->>CLI : "状态"
end
end
end
```

**图示来源**
- [rdx/cli.py:267-308](file://rdx/cli.py#L267-L308)
- [rdx/daemon/client.py:568-573](file://rdx/daemon/client.py#L568-L573)

章节来源
- [rdx/cli.py:267-308](file://rdx/cli.py#L267-L308)

### 多上下文守护进程管理
- 上下文隔离
  - 每个 --daemon-context 对应独立的状态存储与运行实例
- 并行管理
  - 不同上下文可同时运行各自守护进程
- 切换与清理
  - 使用不同上下文ID即可切换目标守护进程
  - 陈旧状态会在启动或状态查询时被清理
- **新增** 所有者PID隔离
  - 每个上下文可以有不同的所有者PID设置
  - 支持在不同所有者进程间迁移守护进程所有权

章节来源
- [rdx/daemon/client.py:568-573](file://rdx/daemon/client.py#L568-L573)
- [rdx/daemon/server.py:101-124](file://rdx/daemon/server.py#L101-L124)

## 依赖关系分析
- CLI 依赖客户端进行状态与请求处理
- 客户端依赖服务端通过命名管道通信
- 服务端依赖工作进程执行任务（见 worker.py）
- **新增** 所有组件都支持所有者PID参数传递

```mermaid
graph LR
CLI["rdx/cli.py"] --> DCLIENT["rdx/daemon/client.py"]
DCLIENT --> DSERV["rdx/daemon/server.py"]
DSERV --> DWORKER["rdx/daemon/worker.py"]
```

**图示来源**
- [rdx/cli.py:1634-1654](file://rdx/cli.py#L1634-L1654)
- [rdx/daemon/client.py:199-604](file://rdx/daemon/client.py#L199-L604)
- [rdx/daemon/server.py:101-545](file://rdx/daemon/server.py#L101-L545)
- [rdx/daemon/worker.py](file://rdx/daemon/worker.py)

章节来源
- [rdx/cli.py:1634-1654](file://rdx/cli.py#L1634-L1654)
- [rdx/daemon/client.py:199-604](file://rdx/daemon/client.py#L199-L604)
- [rdx/daemon/server.py:101-545](file://rdx/daemon/server.py#L101-L545)
- [rdx/daemon/worker.py](file://rdx/daemon/worker.py)

## 性能考量
- 状态持久化频率
  - 仅在状态变更时写入，避免频繁IO
- 健康检查轮询
  - 在自停逻辑中进行周期性检查，避免长阻塞
- 客户端租约与空闲超时
  - 通过合理设置租约与空闲超时平衡资源占用与可用性
- **新增** 所有者进程监控
  - 轻量级进程存活检查，不影响守护进程性能
  - 仅在必要时检查所有者进程状态

## 故障排除指南
- 症状：状态查询显示未运行
  - 可能原因：无上下文状态或服务端不可达
  - 处理：执行状态查询会自动清理陈旧状态；若仍失败，尝试重新启动守护进程
- 症状：守护进程异常退出
  - 可能原因：租约过期、空闲超时、所有者进程丢失
  - 处理：检查 --owner-pid 是否正确传入；调整租约与空闲超时；确认客户端是否正常续租
- 症状：多上下文冲突
  - 可能原因：上下文ID重复或状态残留
  - 处理：使用不同 --daemon-context；必要时手动清理对应上下文状态文件
- **新增** 症状：守护进程无法被新进程接管
  - 可能原因：现有守护进程已有所有者PID
  - 处理：先停止现有守护进程，或使用相同所有者PID重新启动

章节来源
- [rdx/cli.py:267-308](file://rdx/cli.py#L267-L308)
- [rdx/daemon/server.py:532-570](file://rdx/daemon/server.py#L532-L570)
- [tests/test_cli_daemon_status.py](file://tests/test_cli_daemon_status.py)
- [tests/test_daemon_client.py](file://tests/test_daemon_client.py)

## 结论
守护进程命令通过 CLI、客户端与服务端的协作，提供了可靠的启动、停止与状态查询能力。**更新后的版本**增强了生命周期管理，引入了基于所有者PID的所有权系统，确保当父进程意外退出时能够正确清理守护进程资源。借助上下文隔离与健康检查机制，可在多场景下稳定运行。建议在生产环境中合理配置租约与空闲超时，并使用 --daemon-context 实现多实例隔离与管理，同时利用 --owner-pid 参数确保进程间的正确关联。

## 附录
- 常用命令示例
  - 启动：rdx daemon start --daemon-context <ctx> --owner-pid <pid>
  - 停止：rdx daemon stop --daemon-context <ctx>
  - 查询：rdx daemon status --daemon-context <ctx>
- 关键参数
  - --daemon-context：守护进程上下文标识符，默认"default"
  - --pipe-name：命名管道名称（用于服务端监听）
  - **新增** --owner-pid：启动进程PID，用于自动停止检测
- **新增** 所有权管理
  - 守护进程可以声明新的所有者，当原所有者进程退出时自动清理
  - 支持在多进程环境中安全地转移守护进程控制权

章节来源
- [cli/run_cli.py:88-150](file://cli/run_cli.py#L88-L150)
- [rdx/cli.py:1381-1436](file://rdx/cli.py#L1381-L1436)
- [tests/test_daemon_client.py:390-394](file://tests/test_daemon_client.py#L390-L394)