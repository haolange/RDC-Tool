...
- rdc-tool daemon start：启动后台服务进程，支持 --daemon-context 指定上下文。
- rdc-tool daemon stop：停止守护进程，包含优雅关闭与资源清理。
- rdc-tool daemon status：检查守护进程运行状态与健康状况。
- 自动化脚本中的生命周期管理、监控、异常处理。
- 守护进程的通信协议、错误恢复机制与性能监控要点。

## 项目结构
守护进程相关代码主要分布在三个模块：
- CLI入口与命令路由：rdc_tool/cli.py
...
```mermaid
graph TB
A["CLI 入口<br/>rdc_tool/cli.py"] --> B["守护进程客户端<br/>rdc_tool/daemon/client.py"]
B --> C["守护进程服务端<br/>rdc_tool/daemon/server.py"]
...
图表来源
- [rdc_tool/cli.py:1634-1653](file://rdc_tool/cli.py#L1634-L1653)
...
- [rdc_tool/daemon/server.py:693-723](file://rdc_tool/daemon/server.py#L693-L723)
...
U->>CLI : rdc-tool daemon start --daemon-context <ctx>
CLI->>CL : ensure_daemon(context, owner_pid)
CL->>S : 启动子进程(命名管道+token)
CL->>S : ping(健康检查)
S-->>CL : ok + state
CL-->>CLI : 返回状态
U->>CLI : rdc-tool daemon status
CLI->>CL : load_daemon_state()
CL->>S : status
S-->>CL : running + state
CL-->>CLI : JSON 输出
U->>CLI : rdc-tool daemon stop
CLI->>CL : stop_daemon()
CL->>S : shutdown
S->>W : 停止工作进程
S-->>CL : stopping + state
...
### 守护进程启动流程（rdc-tool daemon start）
...
图表来源
- [rdc_tool/daemon/client.py:623-733](file://rdc_tool/daemon/client.py#L623-L733)
- [rdc_tool/cli.py:1634-1645](file://rdc_tool/cli.py#L1634-L1645)

章节来源
- [rdc_tool/cli.py:1634-1645](file://rdc_tool/cli.py#L1634-L1645)
- [rdc_tool/daemon/client.py:623-733](file://rdc_tool/daemon/client.py#L623-L733)

### 守护进程停止流程（rdc-tool daemon stop）
...
图表来源
- [rdc_tool/daemon/client.py:874-901](file://rdc_tool/daemon/client.py#L874-L901)
- [rdc_tool/daemon/server.py:227-245](file://rdc_tool/daemon/server.py#L227-L245)
- [rdc_tool/cli.py:1646-1650](file://rdc_tool/cli.py#L1646-L1650)
...
### 守护进程状态查询（rdc-tool daemon status）
...
- [rdc_tool/daemon/server.py:589-591](file://rdc_tool/daemon/server.py#L589-L591)
...
- [rdc_tool/daemon/client.py:402-464](file://rdc_tool/daemon/client.py#L402-L464)
...
章节来源
- [rdc_tool/daemon/server.py:259-285](file://rdc_tool/daemon/server.py#L259-L285)
- [rdc_tool/daemon/server.py:452-492](file://rdc_tool/daemon/server.py#L452-L492)
- [rdc_tool/daemon/client.py:49-65](file://rdc_tool/daemon/client.py#L49-L65)
- [rdc_tool/daemon/client.py:49-65](file://rdc_tool/daemon/client.py#L49-L65)
...
- [rdc_tool/daemon/client.py:467-515](file://rdc_tool/daemon/client.py#L467-L515)
- [rdc_tool/daemon/server.py:118-125](file://rdc_tool/daemon/server.py#L118-L125)
...
- 守护进程无响应：
  ...
  - 使用 rdc-tool daemon status 获取最新状态。
- 停止失败：
  - 优雅关闭失败时会强制终止进程并清理状态。
  - 若仍有残留进程，可使用系统工具终止。
- 上下文污染：
  - 使用 --daemon-context 隔离上下文。
  - 使用 rdc-tool context clear 清理上下文快照与状态。
...
- rdc-tool daemon status
  - --daemon-context <id>：指定要查询的上下文。
  - 行为：读取本地状态，远程查询守护进程状态，输出 JSON。

### 使用示例
- 启动特定上下文：
  - rdc-tool --daemon-context local daemon start
- 查询状态：
  - rdc-tool --daemon-context local daemon status
- 停止守护进程：
  - rdc-tool --daemon-context local daemon stop
