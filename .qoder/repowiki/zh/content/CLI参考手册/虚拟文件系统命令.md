...
## 简介
本章节面向使用 rdx vfs 子命令的开发者与测试工程师，系统性说明以下能力：
- rdx vfs ls：列出虚拟文件系统节点内容，支持表格投影输出。
- rdx vfs cat：读取只读 VFS 节点的 JSON 表示，用于路径式浏览与导航。
- rdx vfs tree：以树形结构展示 VFS 层级，支持深度限制与节点预算控制。
- rdx vfs resolve：解析 VFS 路径到对应节点元数据，判断是否存在、是否需要 session 以及可用的只读视图。
...
## 项目结构
VFS 相关代码主要分布在以下位置：
- CLI 入口与子命令定义：rdx/cli.py
- VFS 处理器转发：rdx/handlers/vfs.py
- VFS 实现与调度：rdx/server_runtime.py
- 操作契约与参数描述：rdx/operation_definitions.py
- 行为验证测试：tests/test_cli_vfs.py、tests/test_vfs.py

```mermaid
graph TB
A["CLI 解析<br/>rdx/cli.py"] --> B["VFS 处理器<br/>rdx/handlers/vfs.py"]
B --> C["VFS 调度器<br/>rdx/server_runtime.py"]
...
**图表来源**
- [rdx/cli.py:1490-1496](file://rdx/cli.py#L1490-L1496)
- [rdx/handlers/vfs.py:8-9](file://rdx/handlers/vfs.py#L8-L9)
- [rdx/server_runtime.py:12085-12120](file://rdx/server_runtime.py#L12085-L12120)
- [rdx/server_runtime.py:11703-11716](file://rdx/server_runtime.py#L11703-L11716)
...
**章节来源**
- [rdx/handlers/vfs.py:8-9](file://rdx/handlers/vfs.py#L8-L9)
- [rdx/server_runtime.py:11600-11623](file://rdx/server_runtime.py#L11600-L11623)
- [rdx/server_runtime.py:11703-11716](file://rdx/server_runtime.py#L11703-L11716)
- [rdx/server_runtime.py:11668-11693](file://rdx/server_runtime.py#L11668-L11693)
...
participant H as "VFS 处理器<br/>rdx/handlers/vfs.py"
participant S as "VFS 调度器<br/>rdx/server_runtime.py"
participant R as "运行时工具<br/>rd.event.* / rd.pipeline.* / rd.resource.*"
U->>CLI : "rdx vfs <action> --path ... [--format json|tsv]"
CLI->>H : "action, args"
...
**图表来源**
- [rdx/server_runtime.py:12085-12108](file://rdx/server_runtime.py#L12085-L12108)
...
**图表来源**
- [rdx/server_runtime.py:12109-12111](file://rdx/server_runtime.py#L12109-L12111)
...
**图表来源**
- [rdx/server_runtime.py:12115-12120](file://rdx/server_runtime.py#L12115-L12120)
...
**图表来源**
- [rdx/server_runtime.py:12112-12114](file://rdx/server_runtime.py#L12112-L12114)
...
```mermaid
graph LR
CLI["CLI 参数解析<br/>rdx/cli.py"] --> Handler["VFS 处理器<br/>rdx/handlers/vfs.py"]
Handler --> Dispatch["VFS 调度器<br/>rdx/server_runtime.py"]
Dispatch --> Tools["运行时工具<br/>rd.event.* / rd.pipeline.* / rd.resource.*"]
Dispatch --> Contract["操作契约<br/>rdx/operation_definitions.py"]
```
...
- [rdx/handlers/vfs.py:8-9](file://rdx/handlers/vfs.py#L8-L9)
- [rdx/server_runtime.py:12085-12120](file://rdx/server_runtime.py#L12085-L12120)
- [rdx/operation_definitions.py:4063-4174](file://rdx/operation_definitions.py#L4063-L4174)
...
**章节来源**
- [rdx/server_runtime.py:11668-11693](file://rdx/server_runtime.py#L11668-L11693)
- [rdx/server_runtime.py:12085-12093](file://rdx/server_runtime.py#L12085-L12093)
- [tests/test_vfs.py:91-131](file://tests/test_vfs.py#L91-L131)
- [tests/test_cli_vfs.py:358-386](file://tests/test_cli_vfs.py#L358-L386)

## 结论
rdx vfs 命令为运行时资源提供了“路径式”的只读导航能力，覆盖上下文、绘制事件、渲染通道、资源清单、管线与着色器等关键领域。通过 ls、cat、tree、resolve 的组合，用户可以高效探索运行时状态、定位问题、编写自动化脚本。对于大数据量场景，应充分利用 depth、max_nodes、tabular 投影与延迟详情机制，确保性能与可维护性。
...
- rdx vfs ls
  ...
  - 示例：
    - 列出根目录并以表格形式输出：rdx vfs ls --path / --format tsv
    - 列出 draws 目录：rdx vfs ls --path /draws --session-id sess_demo
- rdx vfs cat
  - 参数：--path、--session-id
  - 示例：
    - 读取上下文快照：rdx vfs cat --path /context --format json
    - 读取某事件的管线摘要：rdx vfs cat --path /draws/42/pipeline/summary --session-id sess_demo
- rdx vfs tree
  - 参数：--path、--session-id、--depth、--max-nodes、--format
  - 示例：
    - 以深度 2 浏览 draws：rdx vfs tree --path /draws --depth 2 --max-nodes 2000 --format json
- rdx vfs resolve
  - 参数：--path、--session-id
  - 示例：
    - 解析路径元数据：rdx vfs resolve --path /pipeline --session-id sess_demo
