...
- rdx tools list：列出所有可用工具，支持按命名空间过滤和限制返回数量。
- rdx tools search：基于关键词对工具进行模糊搜索（名称、命名空间、分组、描述）。
- rdx tools describe：获取指定工具的完整契约信息（参数、返回值、前置条件、作用域与影响等）。
...
- 操作定义源：rdx/operation_definitions.py
- 运行时路由与前置条件校验：rdx/tool_router.py

```mermaid
graph TB
A["CLI: rdx/cli.py"] --> B["工具目录加载<br/>load_tool_catalog()"]
B --> C["目录数据: spec/tool_catalog.json"]
A --> D["工具参考生成<br/>scripts/generate_tool_reference.py"]
D --> E["文档: docs/tool-reference.md"]
C --> F["运行时路由<br/>rdx/tool_router.py"]
...
章节来源
- [rdx/cli.py:694-750](file://rdx/cli.py#L694-L750)
- [rdx/cli.py:1408-1417](file://rdx/cli.py#L1408-L1417)
...
participant RT as "运行时路由<br/>rdx/tool_router.py"
U->>CLI : rdx tools list/search/describe
...
图表来源
- [rdx/cli.py:694-713](file://rdx/cli.py#L694-L713)

章节来源
- [rdx/cli.py:694-713](file://rdx/cli.py#L694-L713)
- [rdx/cli.py:1408-1410](file://rdx/cli.py#L1408-L1410)

### 工具搜索（rdx tools search）
- 行为
...
图表来源
- [rdx/cli.py:716-750](file://rdx/cli.py#L716-L750)

章节来源
- [rdx/cli.py:716-750](file://rdx/cli.py#L716-L750)
- [rdx/cli.py:1414-1417](file://rdx/cli.py#L1414-L1417)

### 工具描述（rdx tools describe）
- 行为
...
### 工具目录结构与组织
- 目录来源
  - 单一事实来源：rdx/operation_definitions.py 定义了所有 rd.* 操作及其约束、前置条件、作用域、影响等。
...
- [rdx/operation_definitions.py:490-504](file://rdx/operation_definitions.py#L490-L504)

### 如何添加自定义工具
- 步骤概览
  - 在 rdx/operation_definitions.py 中添加新的操作定义，包含 name、group、description、parameter_raw、returns_raw、input_schema、prerequisites、scope、effects、evidence_kind、path_inputs、namespace 等字段。
...
```mermaid
graph LR
CLI["CLI (rdx/cli.py)"] --> CAT["目录: spec/tool_catalog.json"]
CLI --> SUM["_tool_summary()"]
CLI --> OUT["统一载荷: canonical_*"]
CAT --> ROUTER["运行时路由: rdx/tool_router.py"]
ROUTER --> HND["领域处理器: handlers/*"]
```

图表来源
- [rdx/cli.py:397-407](file://rdx/cli.py#L397-L407)
- [rdx/tool_router.py:131-154](file://rdx/tool_router.py#L131-L154)
...
章节来源
- [rdx/cli.py:716-728](file://rdx/cli.py#L716-L728)
- [rdx/cli.py:694-713](file://rdx/cli.py#L694-L713)
...
- 示例
  - 列出所有工具摘要：rdx tools list --json
  - 仅列出 capture 命名空间的前 10 个工具：rdx tools list --namespace capture --limit 10 --json
  - 列出 pipeline 命名空间的完整条目：rdx tools list --namespace pipeline --full --json
...
- 参数
  ...
  - --limit：最大返回数量（默认 20）。
- 示例
  - 搜索包含 texture 的工具：rdx tools search texture --json
  - 搜索 capture 相关工具并限制 5 条：rdx tools search capture --limit 5 --json
...
- 示例
  - 获取纹理统计工具的详细描述：rdx tools describe rd.texture.compute_stats --json
  - 获取核心能力详情：rdx tools describe rd.core.get_capabilities --json
...
### 工具目录结构与添加自定义工具
- 目录结构
  - 单一事实来源：rdx/operation_definitions.py
