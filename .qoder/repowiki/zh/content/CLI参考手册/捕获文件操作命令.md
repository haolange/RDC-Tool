...
## 简介
本章节面向“捕获文件操作”相关命令，重点说明 rdx capture open 与 rdx capture status 的用法、参数、返回格式与错误处理。其中：
- rdx capture open：用于打开一个 .rdc 捕获文件，可选择指定帧索引并启用预览窗口。
- rdx capture status：用于检查当前上下文中的捕获会话状态（是否已有会话、上下文快照等）。

该文档基于仓库中 CLI 实现与测试用例进行整理，确保内容与实际代码一致。

## 项目结构
与捕获命令直接相关的代码主要位于 CLI 层与预览窗口模块：
- CLI 入口与子命令解析：rdx/cli.py
- 捕获打开流程与状态查询：rdx/cli.py 中的 _cmd_capture_open 与 _cmd_capture_status
- 预览窗口宿主：rdx/preview_window.py
- 帮助信息与示例：cli/run_cli.py

```mermaid
graph TB
A["命令行 rdx"] --> B["CLI 解析器<br/>_build_parser()"]
...
图表来源
- [rdx/cli.py:1381-1470](file://rdx/cli.py#L1381-L1470)
- [rdx/cli.py:1042-1194](file://rdx/cli.py#L1042-L1194)
...
章节来源
- [rdx/cli.py:1042-1194](file://rdx/cli.py#L1042-L1194)
- [rdx/cli.py:1174-1194](file://rdx/cli.py#L1174-L1194)
- [rdx/preview_window.py:192-238](file://rdx/preview_window.py#L192-L238)

## 架构总览
下图展示了 rdx capture open 的完整调用序列，包括可选的预览步骤与错误封装。
...
U->>CLI : rdx capture open --file ... [--frame-index] [--preview]
CLI->>DM : rd.core.init
DM-->>CLI : ok/data
...
- 返回值
  - 成功：JSON 对象，result_kind 为 "rdx.capture.open"，data 中包含 context_id、capture_file_id、capture_path、session_id、active_event_id、recovery_status、backend、remote_id、runtime、context 等字段
...
- 返回值
  - JSON 对象，result_kind 为 "rdx.capture.status"，data 中包含 context_id、has_session、state、context
...
章节来源
- [rdx/cli.py:1128-1140](file://rdx/cli.py#L1128-L1140)
- [rdx/preview_window.py:192-238](file://rdx/preview_window.py#L192-L238)
...
章节来源
- [rdx/cli.py:817-864](file://rdx/cli.py#L817-L864)
- [tests/test_cli_capture_open.py:160-367](file://tests/test_cli_capture_open.py#L160-L367)

## 结论
- rdx capture open 提供了完整的捕获文件打开、帧跳转与预览能力，并通过统一的错误封装与上下文快照辅助定位问题。
- rdx capture status 可用于快速检查当前上下文是否已有活跃会话以及上下文快照。
...
章节来源
- [rdx/cli.py:1458-1470](file://rdx/cli.py#L1458-L1470)
...
章节来源
- [rdx/cli.py:1154-1171](file://rdx/cli.py#L1154-L1171)
- [rdx/cli.py:817-864](file://rdx/cli.py#L817-L864)
...
### 使用示例
- 打开本地捕获文件并跳转到第 0 帧：
  - rdx capture open --file D:\path\capture.rdc --frame-index 0
- 打开捕获文件并启用预览窗口：
  - rdx capture open --file D:\path\capture.rdc --frame-index 0 --preview
- 通过远程句柄打开捕获文件：
  - rdx capture open --file D:\path\capture.rdc --remote-id remote_abc
- 检查当前捕获状态：
  - rdx capture status
