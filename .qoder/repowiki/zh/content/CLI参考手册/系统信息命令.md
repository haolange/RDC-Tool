# 系统信息命令

<cite>
**本文引用的文件**
- [rdx/cli.py](file://rdx/cli.py)
- [cli/run_cli.py](file://cli/run_cli.py)
- [rdx/__init__.py](file://rdx/__init__.py)
- [rdx/runtime_requirements.py](file://rdx/runtime_requirements.py)
- [rdx/python_runtime.py](file://rdx/python_runtime.py)
- [rdx/runtime_bootstrap.py](file://rdx/runtime_bootstrap.py)
- [docs/public-contract.md](file://docs/public-contract.md)
- [docs/tool-reference.md](file://docs/tool-reference.md)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与输出格式](#性能与输出格式)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录：命令速查与示例](#附录命令速查与示例)

## 简介
本章节面向 rdx-tools 的三类系统信息命令，提供完整、可操作的说明与最佳实践：
- rdx version：显示工具版本、平台信息与兼容性状态。
- rdx doctor：执行环境诊断，检查依赖项、Python 环境、RenderDoc 安装与布局、工具目录完整性等。
- rdx completion：生成 shell 补全脚本，支持 PowerShell、Bash、Zsh、Fish。

这些命令可用于获取系统信息、诊断环境问题、配置开发环境，并与其他工具（如 RenderDoc、SPIR-V 工具链）集成使用。

## 项目结构
与系统信息命令直接相关的代码集中在 CLI 入口与运行时辅助模块中：
- CLI 主逻辑与命令实现位于 rdx/cli.py。
- 独立启动器位于 cli/run_cli.py，用于早期版本/依赖缺失时的快速反馈。
- 版本常量定义在 rdx/__init__.py。
- 依赖检测在 rdx/runtime_requirements.py。
- 内置 Python 环境校验在 rdx/python_runtime.py。
- RenderDoc 运行时引导在 rdx/runtime_bootstrap.py。
- 公共契约与工具参考文档在 docs/public-contract.md 与 docs/tool-reference.md。

```mermaid
graph TB
A["用户命令行<br/>rdx"] --> B["启动器<br/>cli/run_cli.py"]
B --> C{"是否版本/帮助/缺依赖?"}
C -- 是 --> D["快速输出版本/最小诊断"]
C -- 否 --> E["加载主CLI<br/>rdx/cli.py"]
E --> F["version 命令<br/>_cmd_version/_version_payload"]
E --> G["doctor 命令<br/>_cmd_doctor"]
E --> H["completion 命令<br/>_cmd_completion/_completion_script"]
G --> I["依赖检查<br/>runtime_requirements.missing_dependencies"]
G --> J["Python 环境校验<br/>python_runtime.validate_bundled_python_layout"]
G --> K["RenderDoc 布局检查<br/>binaries_root/pymodules_dir"]
G --> L["工具目录/启动器存在性检查"]
```

图表来源
- [cli/run_cli.py:160-196](file://cli/run_cli.py#L160-L196)
- [rdx/cli.py:410-564](file://rdx/cli.py#L410-L564)
- [rdx/runtime_requirements.py:52-57](file://rdx/runtime_requirements.py#L52-L57)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)

章节来源
- [cli/run_cli.py:16-63](file://cli/run_cli.py#L16-L63)
- [rdx/cli.py:62-104](file://rdx/cli.py#L62-L104)

## 核心组件
- 版本命令（version）
  - 功能：返回工具版本、Schema 版本、平台、工具根路径、公开命令列表、入口点路径以及兼容性元数据。
  - 输出：默认打印“rdx <版本号>”；加 --json 时输出结构化 JSON。
  - 关键实现：_cmd_version、_version_payload。
- 诊断命令（doctor）
  - 功能：全面检查运行环境，包括：
    - 缺失依赖（Pydantic、NumPy、Pillow、Jinja2、aiofiles）。
    - 内置 Python 布局有效性（manifest、exe、DLL、stdlib、site-packages、pth）。
    - RenderDoc 布局（renderdoc.dll、renderdoc.json、renderdoc.pyd）。
    - SPIR-V 工具可用性（spirv-as、spirv-dis）。
    - 工具目录与启动器完整性（rdx.bat、bin/rdx、cli/run_cli.py）。
    - 工具目录与运行时目录信息。
    - Daemon 状态（若可用）。
  - 输出：成功时返回 ok=true 的诊断详情；失败时返回 ok=false 的错误码 setup_incomplete 与 details。
  - 关键实现：_cmd_doctor。
- 补全命令（completion）
  - 功能：为指定 shell 生成参数补全脚本，包含静态命令/子命令/参数与动态工具名。
  - 支持：PowerShell、Bash、Zsh、Fish。
  - 关键实现：_cmd_completion、_completion_script、_completion_words。

章节来源
- [rdx/cli.py:535-564](file://rdx/cli.py#L535-L564)
- [rdx/cli.py:410-532](file://rdx/cli.py#L410-L532)
- [rdx/cli.py:567-691](file://rdx/cli.py#L567-L691)
- [rdx/runtime_requirements.py:7-13](file://rdx/runtime_requirements.py#L7-L13)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)

## 架构总览
下图展示了从命令行到具体命令执行的调用链，以及各命令所调用的环境与依赖检查模块。

```mermaid
sequenceDiagram
participant U as "用户"
participant L as "启动器<br/>cli/run_cli.py"
participant C as "主CLI<br/>rdx/cli.py"
participant RQ as "依赖检查<br/>runtime_requirements"
participant PR as "Python环境校验<br/>python_runtime"
participant RB as "RenderDoc引导<br/>runtime_bootstrap"
U->>L : 执行 rdx version|doctor|completion
alt 版本或帮助或缺依赖
L-->>U : 快速输出版本/最小诊断
else 正常流程
L->>C : 进入主CLI
alt version
C->>C : _cmd_version/_version_payload
C-->>U : 输出版本JSON或文本
else doctor
C->>RQ : missing_dependencies()
C->>PR : validate_bundled_python_layout()
C->>RB : 确保运行时目录/引导(必要时)
C->>C : 检查RenderDoc布局/工具目录/启动器
C-->>U : 输出诊断结果(ok或setup_incomplete)
else completion
C->>C : _completion_words/_completion_script
C-->>U : 输出对应shell补全脚本
end
end
```

图表来源
- [cli/run_cli.py:160-196](file://cli/run_cli.py#L160-L196)
- [rdx/cli.py:410-691](file://rdx/cli.py#L410-L691)
- [rdx/runtime_requirements.py:52-57](file://rdx/runtime_requirements.py#L52-L57)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)

## 详细组件分析

### 版本命令（rdx version）
- 行为
  - 无参或 --version：输出“rdx <版本号>”。
  - --json：输出包含 tool_version、schema_version、platform、tools_root、public_commands、entrypoints、compatibility 的结构化 JSON。
- 关键点
  - 版本来自包内 __version__。
  - 平台字符串根据操作系统判定。
  - 兼容性与契约信息便于上层工具解析。
- 典型输出字段
  - result_kind: rdx.version
  - data.tool_version / schema_version / platform / tools_root / entrypoints / compatibility.stability / compatibility.json_envelope

```mermaid
flowchart TD
Start(["进入 version"]) --> CheckFlag{"是否 --json ?"}
CheckFlag -- 否 --> TextOut["输出 'rdx <版本>'"]
CheckFlag -- 是 --> BuildPayload["构建版本载荷<br/>tool_version/schema_version/platform/entrypoints/compatibility"]
BuildPayload --> JsonOut["输出 JSON"]
TextOut --> End(["结束"])
JsonOut --> End
```

图表来源
- [rdx/cli.py:535-564](file://rdx/cli.py#L535-L564)
- [rdx/__init__.py:1-4](file://rdx/__init__.py#L1-L4)

章节来源
- [rdx/cli.py:535-564](file://rdx/cli.py#L535-L564)
- [rdx/__init__.py:1-4](file://rdx/__init__.py#L1-L4)
- [docs/public-contract.md:1-11](file://docs/public-contract.md#L1-L11)

### 诊断命令（rdx doctor）
- 行为
  - 收集并验证：
    - 依赖项：pydantic、numpy、Pillow、jinja2、aiofiles。
    - 内置 Python 布局：manifest、exe、dll、stdlib、site-packages、pth。
    - RenderDoc 布局：renderdoc.dll、renderdoc.json、renderdoc.pyd。
    - SPIR-V 工具：spirv-as、spirv-dis。
    - 工具目录与启动器：rdx.bat、bin/rdx、cli/run_cli.py。
    - 运行时目录：runtime_root、cli_runtime_dir、artifacts_dir、logs_dir。
    - Daemon 状态（若可达）。
  - 成功：ok=true，data 包含上述详细信息。
  - 失败：ok=false，error.code=setup_incomplete，error.category=environment，details 包含问题清单。
- 关键实现
  - 依赖检查：missing_dependencies()。
  - Python 布局校验：validate_bundled_python_layout()。
  - RenderDoc 布局检查：基于 binaries_root() 与 pymodules_dir()。
  - 工具目录/启动器存在性检查。
  - 工具目录与运行时目录信息收集。
  - Daemon 状态查询（可能失败时降级为错误载荷）。

```mermaid
flowchart TD
S(["进入 doctor"]) --> D1["检查依赖<br/>missing_dependencies()"]
D1 --> P1["校验内置Python布局<br/>validate_bundled_python_layout()"]
P1 --> R1["检查RenderDoc布局<br/>renderdoc.dll/json/pyd"]
R1 --> T1["检查工具目录/启动器<br/>rdx.bat/bin/rdx/cli/run_cli.py"]
T1 --> SP1["探测SPIR-V工具<br/>spirv-as/spirv-dis"]
SP1 --> RT1["收集运行时目录<br/>runtime_root/cli_runtime_dir/artifacts/logs"]
RT1 --> DS1["尝试获取Daemon状态"]
DS1 --> OK{"全部通过?"}
OK -- 是 --> OutOK["输出 ok=true + details"]
OK -- 否 --> OutErr["输出 ok=false<br/>code=setup_incomplete<br/>category=environment<br/>details"]
```

图表来源
- [rdx/cli.py:410-532](file://rdx/cli.py#L410-L532)
- [rdx/runtime_requirements.py:52-57](file://rdx/runtime_requirements.py#L52-L57)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)

章节来源
- [rdx/cli.py:410-532](file://rdx/cli.py#L410-L532)
- [rdx/runtime_requirements.py:7-13](file://rdx/runtime_requirements.py#L7-L13)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)

### 补全命令（rdx completion）
- 行为
  - 根据传入的 shell 类型生成对应的补全脚本。
  - 补全词集合包含：
    - 静态命令/子命令/参数（如 version、doctor、tools、daemon、context、session、capture、call、vfs、event、pipeline、shader、export、pixel、resource、diff、assert、completion、powershell、bash、zsh、fish 及常用参数）。
    - 动态工具名：从工具目录加载后合并入补全词。
- 支持 Shell
  - PowerShell：注册原生参数补全。
  - Bash：定义 _rdx_complete 并使用 complete 绑定。
  - Zsh：使用 compdef 与 compadd。
  - Fish：生成 complete -c rdx -f -a ... 条目。
- 关键实现
  - _completion_words：生成补全词表。
  - _completion_script：按 shell 类型生成脚本。
  - _cmd_completion：输出脚本内容。

```mermaid
flowchart TD
A["输入: rdx completion <shell>"] --> B["_completion_words()<br/>静态+动态工具名"]
B --> C{"shell 类型?"}
C -- powershell --> D["生成 Register-ArgumentCompleter 脚本"]
C -- bash --> E["生成 _rdx_complete 与 complete 绑定"]
C -- zsh --> F["生成 #compdef 与 _rdx 函数"]
C -- fish --> G["生成 complete -c rdx 条目"]
D --> H["输出脚本"]
E --> H
F --> H
G --> H
```

图表来源
- [rdx/cli.py:567-691](file://rdx/cli.py#L567-L691)

章节来源
- [rdx/cli.py:567-691](file://rdx/cli.py#L567-L691)

## 依赖关系分析
- 版本命令
  - 依赖包版本常量与平台判断。
- 诊断命令
  - 依赖检查模块：runtime_requirements.missing_dependencies。
  - Python 环境校验：python_runtime.validate_bundled_python_layout。
  - 运行时路径：binaries_root、pymodules_dir、runtime_paths。
  - RenderDoc 引导：runtime_bootstrap（在需要时确保导入路径与 DLL 目录）。
- 补全命令
  - 依赖工具目录加载以动态补充补全词。

```mermaid
graph LR
V["version"] --> M["__version__"]
D["doctor"] --> RQ["runtime_requirements.missing_dependencies"]
D --> PR["python_runtime.validate_bundled_python_layout"]
D --> RP["runtime_paths (binaries_root/pymodules_dir)"]
D --> RB["runtime_bootstrap (可选)"]
C["completion"] --> TC["load_tool_catalog (动态补全词)"]
```

图表来源
- [rdx/cli.py:410-691](file://rdx/cli.py#L410-L691)
- [rdx/runtime_requirements.py:52-57](file://rdx/runtime_requirements.py#L52-L57)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)

章节来源
- [rdx/cli.py:410-691](file://rdx/cli.py#L410-L691)
- [rdx/runtime_requirements.py:7-13](file://rdx/runtime_requirements.py#L7-L13)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)

## 性能与输出格式
- 版本命令
  - 轻量级，几乎无 IO；--json 输出稳定结构，便于自动化消费。
- 诊断命令
  - 涉及文件系统检查与可能的 daemon 状态查询；整体开销较小。
  - 输出为统一 JSON 信封，便于解析与集成。
- 补全命令
  - 仅生成文本脚本；动态工具名加载可能受目录读取影响，但通常很快。
- 输出格式
  - 文本模式：人类可读。
  - JSON 模式：结构化，包含 result_kind、data、error、meta、projections 等字段，便于机器处理。

[本节为通用指导，不直接分析具体文件]

## 故障排查指南
- 常见错误与定位
  - 依赖缺失：doctor 返回 setup_incomplete，details.dependencies.missing 列出缺失包名。
  - Python 环境异常：details.python.bundled_python_failures 列出具体的缺失文件或目录。
  - RenderDoc 布局不完整：details.renderdoc.failures 列出缺失的 dll/json/pyd。
  - SPIR-V 工具不可用：details.shader_tools.spirv_as/spirv_dis.available=false，需安装对应工具。
  - 启动器缺失：details.launchers.*_exists=false，需确认 rdx.bat、bin/rdx、cli/run_cli.py 是否存在。
  - Daemon 状态异常：details.daemon 包含错误信息，可能是连接或状态读取失败。
- 建议步骤
  - 先运行 rdx doctor --json 获取完整诊断信息。
  - 根据 details 逐项修复依赖与环境。
  - 对 SPIR-V 相关能力，安装 spirv-as/spirv-dis 并确保 PATH 正确。
  - 若使用远程或预览功能，确认 RenderDoc 已正确安装并可被导入。

章节来源
- [rdx/cli.py:410-532](file://rdx/cli.py#L410-L532)
- [rdx/runtime_requirements.py:52-57](file://rdx/runtime_requirements.py#L52-L57)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)

## 结论
- rdx version 提供稳定的版本与兼容性信息，适合自动化识别与契约校验。
- rdx doctor 是环境健康检查的核心入口，能精准定位依赖、Python 布局、RenderDoc 与工具链问题。
- rdx completion 提升交互效率，支持主流 shell，便于开发与运维集成。
- 三者配合可实现“快速自检—修复—高效使用”的闭环。

[本节为总结，不直接分析具体文件]

## 附录：命令速查与示例
- 版本
  - 文本：rdx version
  - JSON：rdx version --json
  - 用途：查看工具版本、平台、入口点与兼容性。
- 诊断
  - 文本：rdx doctor
  - JSON：rdx doctor --json
  - 用途：检查依赖、Python 环境、RenderDoc 布局、SPIR-V 工具、工具目录与启动器、运行时目录、Daemon 状态。
- 补全
  - PowerShell：rdx completion powershell
  - Bash：rdx completion bash
  - Zsh：rdx completion zsh
  - Fish：rdx completion fish
  - 用途：生成 shell 补全脚本，提升命令行体验。

章节来源
- [rdx/cli.py:62-104](file://rdx/cli.py#L62-L104)
- [rdx/cli.py:535-691](file://rdx/cli.py#L535-L691)
- [cli/run_cli.py:86-118](file://cli/run_cli.py#L86-L118)