...
## 简介
本文件面向RDC-Tool的CLI工具“rdx”，提供从入门到快速参考的完整概览。内容涵盖：
- rdx主命令的基本用法、全局选项与参数
...
## 项目结构
rdx-tools采用“启动器 + 核心CLI + 守护进程”的分层设计：
...
- [rdx/cli.py:48-104](file://rdx/cli.py#L48-L104)
- [rdx/cli.py:234-264](file://rdx/cli.py#L234-L264)
...
- 版本信息
  - rdx --version 或 rdx version --json
  - JSON中包含工具版本、平台、入口点与兼容性信息
- 环境诊断
  - rdx doctor 或 rdx --json doctor
...
- 浏览与导出
  ...
  - rdx export screenshot --event-id 42 --out "<路径>"
...
章节来源
- [rdx/cli.py:48-52](file://rdx/cli.py#L48-L52)
- [rdx/core/contracts.py:129-164](file://rdx/core/contracts.py#L129-L164)
- [cli/run_cli.py:226-282](file://cli/run_cli.py#L226-L282)
- [rdx/cli.py:369-394](file://rdx/cli.py#L369-L394)

### 命令补全功能配置
- 支持的Shell：powershell、bash、zsh、fish
- 使用方法
  - 生成脚本：rdx completion <shell>
  - 将输出粘贴到Shell配置文件或注册为原生补全
- 注意
  - 补全词包含静态命令与动态工具名（来自工具目录）

章节来源
- [rdx/cli.py:567-691](file://rdx/cli.py#L567-L691)
...
## 故障排查指南
- 优先运行 rdx --json doctor 检查环境与守护进程状态
...
## 结论
rdx CLI通过“启动器 + 核心CLI + 守护进程”的清晰分层，提供了稳定、可扩展的调试与分析能力。其统一的JSON信封与可选的TSV投影使得人类可读性与机器可消费性兼顾。借助 doctor、version、completion等辅助命令，用户可以快速完成环境自检、版本确认与交互体验优化。遵循文档中的最佳实践与故障排查步骤，能够高效定位并解决问题。

[本节为总结，不直接分析具体文件]

## 附录
- 环境变量
  - RDX_TOOLS_ROOT：覆盖工具根目录（仅在从其他目录启动时需要）
  - RDX_CONTEXT_ID：默认上下文ID（可由命令行覆盖）
  - RDX_INTERMEDIATE_ROOT：隔离任务级中间产物根
  - RDX_RUNTIME_DLL_DIR、RDX_RENDERDOC_PATH：维护者/调试用途的路径覆盖
- 参考
