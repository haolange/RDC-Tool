...
## 简介
本文件面向使用 RDC-Tool（rdx）进行渲染调试与像素级分析的工程师，系统化说明以下能力：
- rdx pixel value：读取指定坐标的像素值，支持 RGBA 等颜色格式与多种数值类型。
- rdx pixel history：追踪某像素的历史变化，定位影响该像素的绘制调用或调度，支持时间序列分析。
...
- CLI 层提供 rdx pixel value/history 子命令与参数解析。
...
```mermaid
graph TB
CLI["CLI: rdx pixel value/history"] --> OPS["操作定义: get_pixel_value / get_pixel_history"]
...
图表来源
- [rdx/cli.py:1557-1566](file://rdx/cli.py#L1557-L1566)
- [rdx/operation_definitions.py:3807-3860](file://rdx/operation_definitions.py#L3807-L3860)
- [rdx/server_runtime.py:5136-5250](file://rdx/server_runtime.py#L5136-L5250)
- [rdx/core/render_service.py:800-873](file://rdx/core/render_service.py#L800-L873)
- [rdx/core/pipeline_service.py:302-320](file://rdx/core/pipeline_service.py#L302-L320)
...
- [rdx/cli.py:1557-1566](file://rdx/cli.py#L1557-L1566)
- [rdx/operation_definitions.py:3807-3860](file://rdx/operation_definitions.py#L3807-L3860)
- [rdx/operation_definitions.py:3746-3762](file://rdx/operation_definitions.py#L3746-L3762)
...
U->>CLI : "rdx pixel value --resource-id ... --x --y"
...
### 坐标系统与转换
- 纹理坐标（像素坐标）：
  - 由 rdx pixel value/history 的 x、y 指定，单位为 texel（纹理元素）。
...
### 使用示例
- 读取特定像素的颜色变化：
  - 使用 rdx pixel value 在多个事件上读取同一坐标的像素值，比较 RGBA 分量变化。
  - 关注 has_nan/has_inf，识别异常值来源。
- 追踪渲染错误的来源：
  - 使用 rdx pixel history 获取影响该像素的事件链，定位导致异常的绘制调用或着色器阶段。
- 对比不同帧的像素差异：
  - 使用区域读取与统计计算（compute_stats）或直方图（get_histogram）进行全局或局部对比。
  - 结合差异验证器（image diff verifier）生成热力图与指标。

章节来源
- [rdx/operation_definitions.py:3506-3520](file://rdx/operation_definitions.py#L3506-L3520)
- [rdx/core/verifiers.py:284-417](file://rdx/core/verifiers.py#L284-L417)
...
章节来源
- [rdx/core/render_service.py:198-233](file://rdx/core/render_service.py#L198-L233)
- [rdx/operation_definitions.py:3861-3916](file://rdx/operation_definitions.py#L3861-L3916)
- [rdx/server_runtime.py:5162-5178](file://rdx/server_runtime.py#L5162-L5178)
...
章节来源
- [rdx/core/render_service.py:828-834](file://rdx/core/render_service.py#L828-L834)
- [rdx/server_runtime.py:5162-5178](file://rdx/server_runtime.py#L5162-L5178)
- [rdx/core/pipeline_service.py:302-320](file://rdx/core/pipeline_service.py#L302-L320)
...
## 附录
- 相关命令与操作：
  - rdx pixel value -> rd.texture.get_pixel_value
  - rdx pixel history -> rd.texture.get_pixel_history
