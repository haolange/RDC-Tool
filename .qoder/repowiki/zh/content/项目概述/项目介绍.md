...
## 简介
RDC-Tool（发行包名 `rdc-tool`，Python 包 `rdc_tool`）是一个面向 RenderDoc .rdc 捕获文件的命令行工具集，目标是提供“本地优先”的 GPU 调试与分析体验，同时支持 Windows x64 本地回放与 Android 远程回放。它通过 JSON 优先的 `rd.*` 操作暴露能力，CLI 入口为 `rdc-tool`，并可通过 `rdc-tool tools list` 动态发现可用工具。
...
## 项目结构
仓库采用分层与领域划分相结合的组织方式：
- 顶层 CLI 启动器与包装：`cli/run_cli.py` 负责环境初始化、依赖检查、版本/诊断输出，并委托到 `rdc_tool.cli`。
- 命令适配层：`rdc_tool/cli.py` 解析参数、维护上下文、与守护进程通信、格式化输出（JSON/TSV）。
- 路由与注册：`rdc_tool/tool_router.py` 基于目录域将 `rd.<domain>.<action>` 映射到具体处理器，并执行前置条件校验。
- 核心引擎：`rdc_tool/core/engine.py` 提供统一的异步执行引擎，规范化输入输出、收集元数据与制品。
- 会话与后端：`rdc_tool/core/session_manager.py` 抽象本地/远程回放会话、打开捕获、创建无头输出、清理资源。
- 数据模型：`rdc_tool/models.py` 定义会话、捕获、事件树、管线快照、实验、报告等结构化类型。
- 文档：`docs/` 包含快速开始、会话模型、工具清单、安装与排障等。

```mermaid
graph TB
A["用户"] --> B["CLI 启动器<br/>cli/run_cli.py"]
B --> C["命令适配层<br/>rdc_tool/cli.py"]
C --> D["守护进程/运行时<br/>daemon + server_runtime"]
D --> E["操作路由<br/>rdc_tool/tool_router.py"]
E --> F["领域处理器<br/>handlers/*"]
F --> G["核心引擎<br/>rdc_tool/core/engine.py"]
G --> H["会话管理<br/>rdc_tool/core/session_manager.py"]
...
## 核心组件
- CLI 启动器：负责 Python 路径与环境准备、依赖缺失检测、版本与诊断输出，并将控制权交给 `rdc_tool.cli`。
...
- [rdc_tool/cli.py:234-394](file://rdc_tool/cli.py#L234-L394)
...
- [rdc_tool/core/session_manager.py:148-251](file://rdc_tool/core/session_manager.py#L148-L251)
- [rdc_tool/models.py:128-157](file://rdc_tool/models.py#L128-L157)

## 架构总览
RDC 采用“CLI → 守护进程/运行时 → 操作路由 → 领域处理器 → 核心引擎 → 会话/后端”的分层架构。CLI 仅负责交互与协议适配；真正的回放与 GPU 访问由会话管理器驱动 RenderDoc 本地或远程接口完成。操作路由集中管理工具发现、参数校验与前置条件，使新增工具无需改动上层逻辑。

```mermaid
sequenceDiagram
participant U as "用户"
participant L as "CLI 启动器<br/>cli/run_cli.py"
participant C as "命令适配层<br/>rdc_tool/cli.py"
participant R as "操作路由<br/>rdc_tool/tool_router.py"
participant E as "核心引擎<br/>rdc_tool/core/engine.py"
participant S as "会话管理<br/>rdc_tool/core/session_manager.py"
...
### 操作路由与前置条件（rdc_tool/tool_router.py）
...
图表来源
- [rdc_tool/core/session_manager.py:175-251](file://rdc_tool/core/session_manager.py#L175-L251)
- [rdc_tool/core/session_manager.py:309-440](file://rdc_tool/core/session_manager.py#L309-L440)
- [rdc_tool/core/session_manager.py:509-568](file://rdc_tool/core/session_manager.py#L509-L568)
...
章节来源
- [rdc_tool/models.py:128-157](file://rdc_tool/models.py#L128-L157)
- [rdc_tool/models.py:173-181](file://rdc_tool/models.py#L173-L181)
- [rdc_tool/models.py:280-301](file://rdc_tool/models.py#L280-L301)
- [rdc_tool/models.py:479-483](file://rdc_tool/models.py#L479-L483)
...
## 故障排查指南
- 环境诊断：使用 `rdc-tool --json doctor` 检查依赖、Python 布局、RenderDoc 组件、SPIRV 工具与目录完整性。
- 常见错误：
