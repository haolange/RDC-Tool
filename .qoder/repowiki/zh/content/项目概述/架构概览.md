# 架构概览

<cite>
**本文引用的文件**
- [README.md](file://README.md)
- [rdc_tool/cli.py](file://rdc_tool/cli.py)
- [rdc_tool/server.py](file://rdc_tool/server.py)
- [rdc_tool/core/engine.py](file://rdc_tool/core/engine.py)
- [rdc_tool/daemon/server.py](file://rdc_tool/daemon/server.py)
- [rdc_tool/daemon/worker.py](file://rdc_tool/daemon/worker.py)
- [rdc_tool/tool_router.py](file://rdc_tool/tool_router.py)
- [rdc_tool/handlers/session.py](file://rdc_tool/handlers/session.py)
- [rdc_tool/handlers/replay.py](file://rdc_tool/handlers/replay.py)
- [rdc_tool/core/session_manager.py](file://rdc_tool/core/session_manager.py)
- [rdc_tool/runtime_catalog.py](file://rdc_tool/runtime_catalog.py)
- [rdc_tool/config.py](file://rdc_tool/config.py)
- [rdc_tool/operation_definitions.py](file://rdc_tool/operation_definitions.py)
- [rdc_tool/replay_observation.py](file://rdc_tool/replay_observation.py)
</cite>

## 更新摘要
**变更内容**
- 更新了Android远程呈现部分，反映Android原生设备呈现完全集成到会话观察操作中
- 新增了对`rd.session.observe`操作中远程显示输出的详细说明
- 更新了架构图以展示Android远程呈现的完整流程
- 增强了会话管理器中`present_remote`方法的描述

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与可扩展性](#性能与可扩展性)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)

## 简介
本架构概览面向希望理解 RDC-Tool（rdc-tools）整体设计的开发者。文档聚焦四层职责分离：CLI 层、守护进程层、核心引擎层、处理器层；解释 JSON-first 的 rd.* 操作接口设计理念；说明会话管理与上下文隔离机制；对比本地回放与 Android 远程回放的实现差异；并通过架构图与交互图帮助快速建立全局认知。

**更新** 现在支持Android原生设备呈现完全集成到会话观察操作中，使用拥有远程回放连接进行显示输出。

## 项目结构
RDC-Tool 采用分层与按领域划分的组织方式：
- CLI 层：负责用户入口、参数解析、JSON 输出、与守护进程通信。
- 守护进程层：基于命名管道提供长驻服务，管理 Worker 进程生命周期、上下文状态、心跳与超时。
- 核心引擎层：统一的执行引擎，负责操作路由、参数校验、结果规范化、错误归一化、产物发布。
- 处理器层：按领域（session、replay、capture、buffer、pipeline 等）拆分处理逻辑，通过工具目录注册表动态发现并调用。

```mermaid
graph TB
subgraph "CLI 层"
CLI["rdc_tool/cli.py"]
end
subgraph "守护进程层"
Daemon["rdc_tool/daemon/server.py"]
Worker["rdc_tool/daemon/worker.py"]
end
subgraph "核心引擎层"
Engine["rdc_tool/core/engine.py"]
Router["rdc_tool/tool_router.py"]
Catalog["rdc_tool/runtime_catalog.py"]
SessionMgr["rdc_tool/core/session_manager.py"]
end
subgraph "处理器层"
H_Session["rdc_tool/handlers/session.py"]
H_Replay["rdc_tool/handlers/replay.py"]
ReplayObs["rdc_tool/replay_observation.py"]
end
CLI --> Daemon
Daemon --> Worker
Daemon --> Engine
Engine --> Router
Router --> Catalog
Router --> H_Session
Router --> H_Replay
H_Session --> SessionMgr
H_Replay --> ReplayObs
ReplayObs --> SessionMgr
```

**图表来源**
- [rdc_tool/cli.py:234-264](file://rdc_tool/cli.py#L234-L264)
- [rdc_tool/daemon/server.py:572-643](file://rdc_tool/daemon/server.py#L572-L643)
- [rdc_tool/core/engine.py:40-75](file://rdc_tool/core/engine.py#L40-L75)
- [rdc_tool/tool_router.py:131-154](file://rdc_tool/tool_router.py#L131-L154)
- [rdc_tool/runtime_catalog.py:17-28](file://rdc_tool/runtime_catalog.py#L17-L28)
- [rdc_tool/handlers/session.py:8-12](file://rdc_tool/handlers/session.py#L8-L12)
- [rdc_tool/handlers/replay.py:8-9](file://rdc_tool/handlers/replay.py#L8-L9)
- [rdc_tool/replay_observation.py:40-174](file://rdc_tool/replay_observation.py#L40-L174)
- [rdc_tool/core/session_manager.py:302-317](file://rdc_tool/core/session_manager.py#L302-L317)

**章节来源**
- [README.md:1-70](file://README.md#L1-L70)
- [rdc_tool/cli.py:1-120](file://rdc_tool/cli.py#L1-L120)
- [rdc_tool/daemon/server.py:102-164](file://rdc_tool/daemon/server.py#L102-L164)
- [rdc_tool/core/engine.py:1-75](file://rdc_tool/core/engine.py#L1-L75)
- [rdc_tool/tool_router.py:1-54](file://rdc_tool/tool_router.py#L1-L54)

## 核心组件
- CLI 适配器：将命令行参数转换为 JSON 请求，选择上下文，调用守护进程执行，统一输出 JSON/TSV。
- 守护进程：维护多上下文状态、Worker 进程、连接与会话生命周期，提供 exec/status/ping 等方法。
- 核心引擎：统一执行入口，负责操作解析、参数校验、结果标准化、错误映射、产物发布。
- 处理器路由：从工具目录加载 rd.* 操作定义，按 domain.action 分发到对应处理器，并执行前置条件检查。
- 会话管理器：封装本地/远程回放后端，管理 session、capture、controller、headless output 等资源。
- **更新** 回放观察器：专门处理无窗口回放观察，集成Android远程设备呈现功能。

**章节来源**
- [rdc_tool/cli.py:234-327](file://rdc_tool/cli.py#L234-L327)
- [rdc_tool/daemon/server.py:572-643](file://rdc_tool/daemon/server.py#L572-L643)
- [rdc_tool/core/engine.py:40-204](file://rdc_tool/core/engine.py#L40-L204)
- [rdc_tool/tool_router.py:89-154](file://rdc_tool/tool_router.py#L89-L154)
- [rdc_tool/core/session_manager.py:175-206](file://rdc_tool/core/session_manager.py#L175-L206)
- [rdc_tool/replay_observation.py:40-174](file://rdc_tool/replay_observation.py#L40-L174)

## 架构总览
下图展示了从 CLI 到处理器再到后端的完整调用链，以及守护进程的中间协调角色。

```mermaid
sequenceDiagram
participant U as "用户"
participant CLI as "CLI (cli.py)"
participant D as "守护进程 (daemon/server.py)"
participant W as "Worker (daemon/worker.py)"
participant E as "核心引擎 (core/engine.py)"
participant R as "路由 (tool_router.py)"
participant H as "处理器 (handlers/*)"
participant RO as "回放观察器 (replay_observation.py)"
participant SM as "会话管理器 (core/session_manager.py)"
U->>CLI : rdc-tool call rd.session.observe
CLI->>D : daemon_request("exec", {operation, args})
D->>W : request("exec", {operation, args, context_id})
W-->>D : result payload
D->>E : execute(operation, args, context)
E->>R : resolve(operation)
R->>H : dispatch(action, args, env)
H->>RO : handle("observe", args)
RO->>SM : present_remote(session_id, event_id, texture_id)
SM->>SM : PresentReplay(event_id, texture_id)
SM-->>RO : sequence number
RO-->>H : observation result with remote_display
H-->>E : handler result
E-->>D : normalized payload
D-->>CLI : ok/result/error
CLI-->>U : JSON/TSV 输出
```

**图表来源**
- [rdc_tool/cli.py:243-264](file://rdc_tool/cli.py#L243-L264)
- [rdc_tool/daemon/server.py:616-643](file://rdc_tool/daemon/server.py#L616-L643)
- [rdc_tool/daemon/worker.py:144-170](file://rdc_tool/daemon/worker.py#L144-L170)
- [rdc_tool/core/engine.py:40-75](file://rdc_tool/core/engine.py#L40-L75)
- [rdc_tool/tool_router.py:131-154](file://rdc_tool/tool_router.py#L131-L154)
- [rdc_tool/handlers/session.py:8-12](file://rdc_tool/handlers/session.py#L8-L12)
- [rdc_tool/replay_observation.py:120-132](file://rdc_tool/replay_observation.py#L120-L132)
- [rdc_tool/core/session_manager.py:302-317](file://rdc_tool/core/session_manager.py#L302-L317)

## 详细组件分析

### CLI 层：JSON-first 的用户入口
- 职责：解析参数、构造 JSON 请求、选择上下文、调用守护进程、统一输出 JSON/TSV。
- 关键流程：
  - 确保守护进程可用并获取状态。
  - 通过 daemon_request 发送 exec 请求，携带 operation、args、transport、remote 标志。
  - 支持 --format json|tsv，TSV 时自动注入 tabular projection。
  - 错误统一为 canonical_error，包含 code/category/message/details。
- JSON-first 设计：所有操作以 JSON 对象作为输入和输出，便于脚本化与跨语言调用；TSV 仅用于表格类数据投影。

```mermaid
flowchart TD
Start(["CLI 入口"]) --> EnsureDaemon["确保守护进程可用"]
EnsureDaemon --> BuildReq["构建 JSON 请求<br/>operation + args + transport + remote"]
BuildReq --> SendExec["daemon_request('exec')"]
SendExec --> Resp{"响应 ok?"}
Resp --> |否| Err["canonical_error 包装"]
Resp --> |是| Format{"--format tsv?"}
Format --> |是| TSV["渲染 TSV 投影"]
Format --> |否| JSON["输出 JSON"]
Err --> End(["退出"])
TSV --> End
JSON --> End
```

**图表来源**
- [rdc_tool/cli.py:234-264](file://rdc_tool/cli.py#L234-L264)
- [rdc_tool/cli.py:330-394](file://rdc_tool/cli.py#L330-L394)

**章节来源**
- [rdc_tool/cli.py:54-125](file://rdc_tool/cli.py#L54-L125)
- [rdc_tool/cli.py:234-327](file://rdc_tool/cli.py#L234-L327)
- [rdc_tool/cli.py:330-394](file://rdc_tool/cli.py#L330-L394)

### 守护进程层：长驻服务与上下文隔离
- 职责：监听命名管道、认证、维护上下文状态、管理 Worker 进程、心跳与空闲超时、清理资源。
- 关键特性：
  - 多上下文隔离：每个上下文独立状态文件与 Worker 实例。
  - 客户端附着与心跳：记录 attached_clients，按租约超时清理。
  - 生命周期监控：当所有者丢失或空闲超时时停止守护进程。
  - exec 转发：将操作请求转发给 Worker 进程执行，并刷新上下文快照。
- 上下文快照：同步 session_id、capture_file_id、active_event_id、frame_index 等运行时信息。

```mermaid
classDiagram
class DaemonRuntime {
+string pipe_name
+string token
+string daemon_context
+int owner_pid
+bool running
+dict state
+handle_request(request) Dict
+serve_forever() int
-_run_exec(operation, args, transport, remote) Dict
-_watch_lifecycle() void
}
class RuntimeWorkerProcess {
+is_running() bool
+request(method, params, timeout) Dict
+stop() void
-_spawn() void
-_await_ready() void
}
DaemonRuntime --> RuntimeWorkerProcess : "管理/复用"
```

**图表来源**
- [rdc_tool/daemon/server.py:102-164](file://rdc_tool/daemon/server.py#L102-L164)
- [rdc_tool/daemon/server.py:572-643](file://rdc_tool/daemon/server.py#L572-L643)
- [rdc_tool/daemon/worker.py:24-45](file://rdc_tool/daemon/worker.py#L24-L45)
- [rdc_tool/daemon/worker.py:144-170](file://rdc_tool/daemon/worker.py#L144-L170)

**章节来源**
- [rdc_tool/daemon/server.py:102-164](file://rdc_tool/daemon/server.py#L102-L164)
- [rdc_tool/daemon/server.py:532-570](file://rdc_tool/daemon/server.py#L532-L570)
- [rdc_tool/daemon/server.py:572-643](file://rdc_tool/daemon/server.py#L572-L643)
- [rdc_tool/daemon/worker.py:24-45](file://rdc_tool/daemon/worker.py#L24-L45)
- [rdc_tool/daemon/worker.py:121-170](file://rdc_tool/daemon/worker.py#L121-L170)

### 核心引擎层：统一执行与规范化
- 职责：解析操作、调用处理器、标准化输出、错误映射、产物发布、元数据注入。
- 关键点：
  - ExecutionContext 携带 transport、remote、trace_id、metadata、progress_reporter。
  - _normalize_output 兼容多种返回格式，统一为 canonical_success/canonical_error。
  - 自动收集 artifacts，支持远程模式下的产物发布。
  - 记录 duration_ms、trace_id、transport 等元信息。

```mermaid
flowchart TD
ExecStart["execute(operation, args, context)"] --> Resolve["registry.resolve(operation)"]
Resolve --> Handler{"找到处理器?"}
Handler --> |否| NotFound["NotFoundError"]
Handler --> |是| CallHandler["await handler(args, env)"]
CallHandler --> Normalize["_normalize_output(raw, ctx)"]
Normalize --> PublishArtifacts["publish_candidates(artifacts, remote)"]
PublishArtifacts --> Meta["注入 meta(trace_id, transport, duration_ms)"]
Meta --> Return["返回标准化结果"]
NotFound --> Return
```

**图表来源**
- [rdc_tool/core/engine.py:40-75](file://rdc_tool/core/engine.py#L40-L75)
- [rdc_tool/core/engine.py:77-204](file://rdc_tool/core/engine.py#L77-L204)

**章节来源**
- [rdc_tool/core/engine.py:21-75](file://rdc_tool/core/engine.py#L21-L75)
- [rdc_tool/core/engine.py:77-204](file://rdc_tool/core/engine.py#L77-L204)

### 处理器层：按领域划分与前置条件校验
- 职责：按 domain.action 路由到具体处理器，执行参数校验与前置条件检查。
- 关键点：
  - 工具目录由 operation_definitions.py 驱动，支持描述、分组、输入 schema、前置条件。
  - 前置条件包括 capture_file_id、session_id、remote_id、active_event_id、capability.remote 等。
  - 处理器内部可委托 server_runtime 的分发方法，如 _dispatch_session/_dispatch_replay。
- **更新** 会话处理器现在支持回放观察操作，通过专用的回放观察器处理Android远程呈现。

```mermaid
graph LR
Cat["runtime_catalog.py<br/>load_tool_catalog()"] --> Reg["tool_router.py<br/>build_operation_registry()"]
Reg --> Dom["domain handlers<br/>session/replay/..."]
Dom --> Pre["前置条件校验<br/>_enforce_prerequisites()"]
Pre --> Dispatch["实际处理器逻辑"]
Dispatch --> RO["回放观察器<br/>replay_observation.py"]
```

**图表来源**
- [rdc_tool/runtime_catalog.py:17-28](file://rdc_tool/runtime_catalog.py#L17-L28)
- [rdc_tool/tool_router.py:89-154](file://rdc_tool/tool_router.py#L89-L154)
- [rdc_tool/handlers/session.py:8-12](file://rdc_tool/handlers/session.py#L8-L12)
- [rdc_tool/handlers/replay.py:8-9](file://rdc_tool/handlers/replay.py#L8-L9)
- [rdc_tool/replay_observation.py:40-174](file://rdc_tool/replay_observation.py#L40-L174)

**章节来源**
- [rdc_tool/runtime_catalog.py:31-91](file://rdc_tool/runtime_catalog.py#L31-L91)
- [rdc_tool/tool_router.py:89-154](file://rdc_tool/tool_router.py#L89-L154)
- [rdc_tool/handlers/session.py:8-12](file://rdc_tool/handlers/session.py#L8-L12)
- [rdc_tool/handlers/replay.py:8-9](file://rdc_tool/handlers/replay.py#L8-L9)

### 会话管理与上下文隔离
- 会话模型：SessionManager 维护多个 SessionState，区分本地与远程后端，管理 controller、output、remote_server。
- 上下文隔离：守护进程按 daemon_context 隔离状态文件与 Worker 实例；CLI 通过 --daemon-context 选择上下文。
- 关键能力：
  - create_session：根据 backend_config 初始化本地或远程会话。
  - open_capture：打开捕获文件，设置 headless output，推断能力。
  - close_session：释放控制器、输出、远程连接等。
- **更新** 新增`present_remote`方法，通过拥有远程回放连接进行Android设备呈现，使用`PresentReplay`接口获取新鲜序列号确认。

```mermaid
stateDiagram-v2
[*] --> Idle
Idle --> Created : "create_session()"
Created --> CaptureOpen : "open_capture(rdc_path)"
CaptureOpen --> Active : "创建 headless output"
Active --> RemotePresented : "present_remote(event_id, texture_id)"
RemotePresented --> Active : "继续观察"
Active --> Closed : "close_session()"
Closed --> [*]
```

**图表来源**
- [rdc_tool/core/session_manager.py:175-206](file://rdc_tool/core/session_manager.py#L175-L206)
- [rdc_tool/core/session_manager.py:208-251](file://rdc_tool/core/session_manager.py#L208-L251)
- [rdc_tool/core/session_manager.py:253-282](file://rdc_tool/core/session_manager.py#L253-L282)
- [rdc_tool/core/session_manager.py:302-317](file://rdc_tool/core/session_manager.py#L302-L317)

**章节来源**
- [rdc_tool/core/session_manager.py:118-174](file://rdc_tool/core/session_manager.py#L118-L174)
- [rdc_tool/core/session_manager.py:175-206](file://rdc_tool/core/session_manager.py#L175-L206)
- [rdc_tool/core/session_manager.py:208-251](file://rdc_tool/core/session_manager.py#L208-L251)
- [rdc_tool/core/session_manager.py:253-282](file://rdc_tool/core/session_manager.py#L253-L282)

### 回放观察器：无窗口回放与Android远程呈现
- 职责：处理无窗口回放观察，支持事件应用、纹理导出和Android远程设备呈现。
- 关键特性：
  - 原子性操作：事件应用与图像导出在同一上下文中执行。
  - Android远程呈现：通过`present_remote`方法调用拥有连接的远程服务器。
  - 呈现状态报告：返回`remote_display`字段，包含status、event_id、texture_id、sequence等信息。
  - 序列号验证：确保每次呈现都获得新鲜的正序列号，防止重复使用旧确认。
- **更新** 完全集成到会话观察操作中，不再需要独立的预览窗口生命周期。

```mermaid
flowchart TD
ObserveStart["rd.session.observe"] --> Validate["验证参数和目标"]
Validate --> ApplyEvent["应用事件 SetFrameEvent"]
ApplyEvent --> GetTargets["获取输出目标"]
GetTargets --> PresentCheck{"Android远程?"}
PresentCheck --> |是| RemotePresent["present_remote(event_id, texture_id)"]
PresentCheck --> |否| LocalExport["本地PNG导出"]
RemotePresent --> CheckStatus{"呈现状态"}
CheckStatus --> |presented| ExportPNG["导出PNG"]
CheckStatus --> |unavailable| ExportPNG
CheckStatus --> |unsupported| ExportPNG
ExportPNG --> ReturnResult["返回结果包含remote_display"]
LocalExport --> ReturnResult
ReturnResult --> End(["完成"])
```

**图表来源**
- [rdc_tool/replay_observation.py:40-174](file://rdc_tool/replay_observation.py#L40-L174)
- [rdc_tool/core/session_manager.py:302-317](file://rdc_tool/core/session_manager.py#L302-L317)

**章节来源**
- [rdc_tool/replay_observation.py:1-174](file://rdc_tool/replay_observation.py#L1-L174)
- [rdc_tool/core/session_manager.py:302-317](file://rdc_tool/core/session_manager.py#L302-L317)

### 本地回放与 Android 远程回放的实现差异
- 本地回放：
  - 初始化 RenderDoc replay 子系统，打开 .rdc 文件，创建 controller 与 headless output。
  - 直接在本机进程内执行回放，资源在本地释放。
  - 无Android远程呈现功能，`remote_display.status`为`not_applicable`。
- Android 远程回放：
  - 通过 adb 将捕获文件推送到设备指定目录，使用 sha256 校验一致性。
  - 通过远程 RenderDoc 服务器打开捕获，创建 controller 与 headless output。
  - **更新** 完全集成到`rd.session.observe`操作中，通过`present_remote`方法进行设备呈现。
  - 支持进度回调与失败清理，确保远端临时文件正确删除。
  - 呈现状态通过`remote_display`字段报告，包含`presented`、`unavailable`、`unsupported`、`not_applicable`四种状态。

```mermaid
flowchart TD
Start(["打开捕获"]) --> Backend{"后端类型"}
Backend --> |本地| LocalInit["初始化本地回放子系统"]
LocalInit --> OpenLocal["OpenFile/OpenCapture"]
OpenLocal --> HeadlessLocal["创建 headless output"]
HeadlessLocal --> ObserveLocal["rd.session.observe<br/>remote_display: not_applicable"]
Backend --> |Android 远程| ADBPush["adb push 到设备"]
ADBPush --> Verify["sha256 校验"]
Verify --> RemoteOpen["远程服务器 OpenCapture"]
RemoteOpen --> HeadlessRemote["创建 headless output"]
HeadlessRemote --> ObserveRemote["rd.session.observe<br/>present_remote调用"]
ObserveLocal --> End(["就绪"])
ObserveRemote --> End
```

**图表来源**
- [rdc_tool/core/session_manager.py:301-307](file://rdc_tool/core/session_manager.py#L301-L307)
- [rdc_tool/core/session_manager.py:347-371](file://rdc_tool/core/session_manager.py#L347-L371)
- [rdc_tool/core/session_manager.py:373-440](file://rdc_tool/core/session_manager.py#L373-L440)
- [rdc_tool/core/session_manager.py:442-507](file://rdc_tool/core/session_manager.py#L442-L507)
- [rdc_tool/replay_observation.py:120-132](file://rdc_tool/replay_observation.py#L120-L132)

**章节来源**
- [rdc_tool/core/session_manager.py:301-307](file://rdc_tool/core/session_manager.py#L301-L307)
- [rdc_tool/core/session_manager.py:347-371](file://rdc_tool/core/session_manager.py#L347-L371)
- [rdc_tool/core/session_manager.py:373-440](file://rdc_tool/core/session_manager.py#L373-L440)
- [rdc_tool/core/session_manager.py:442-507](file://rdc_tool/core/session_manager.py#L442-L507)
- [rdc_tool/replay_observation.py:120-132](file://rdc_tool/replay_observation.py#L120-L132)

## 依赖关系分析
- CLI 依赖守护进程客户端与服务路径、工具目录、运行时路径。
- 守护进程依赖 Worker 进程、上下文状态、运行时路径。
- 核心引擎依赖操作注册表、产物发布器、错误映射。
- 处理器依赖 server_runtime 的分发方法与会话管理器。
- **更新** 回放观察器依赖会话管理器的`present_remote`方法进行Android远程呈现。
- 工具目录由 operation_definitions.py 驱动，保证公开接口的稳定性与可发现性。

```mermaid
graph TB
CLI["cli.py"] --> DaemonClient["daemon/client.py"]
CLI --> Catalog["runtime_catalog.py"]
Daemon["daemon/server.py"] --> Worker["daemon/worker.py"]
Daemon --> State["runtime_state/context_snapshot"]
Engine["core/engine.py"] --> Registry["operation_registry"]
Engine --> Publisher["artifact_publisher"]
Router["tool_router.py"] --> Handlers["handlers/*"]
Handlers --> SessionMgr["core/session_manager.py"]
Handlers --> ReplayObs["replay_observation.py"]
ReplayObs --> SessionMgr
```

**图表来源**
- [rdc_tool/cli.py:17-46](file://rdc_tool/cli.py#L17-L46)
- [rdc_tool/daemon/server.py:17-29](file://rdc_tool/daemon/server.py#L17-L29)
- [rdc_tool/core/engine.py:12-16](file://rdc_tool/core/engine.py#L12-L16)
- [rdc_tool/tool_router.py:10-30](file://rdc_tool/tool_router.py#L10-L30)
- [rdc_tool/handlers/session.py:5-12](file://rdc_tool/handlers/session.py#L5-L12)
- [rdc_tool/replay_observation.py:11-12](file://rdc_tool/replay_observation.py#L11-L12)

**章节来源**
- [rdc_tool/cli.py:17-46](file://rdc_tool/cli.py#L17-L46)
- [rdc_tool/daemon/server.py:17-29](file://rdc_tool/daemon/server.py#L17-L29)
- [rdc_tool/core/engine.py:12-16](file://rdc_tool/core/engine.py#L12-L16)
- [rdc_tool/tool_router.py:10-30](file://rdc_tool/tool_router.py#L10-L30)

## 性能与可扩展性
- 异步执行：核心引擎与处理器支持异步，避免阻塞事件循环。
- 资源隔离：守护进程按上下文隔离 Worker 与状态，减少干扰。
- 超时控制：Worker 请求与守护进程空闲超时防止资源泄漏。
- 产物发布：自动收集 artifacts，支持远程模式下的产物上传。
- **更新** Android远程呈现优化：通过拥有连接复用和序列号验证，确保呈现操作的可靠性和性能。
- 可扩展点：新增 rd.* 操作只需在 operation_definitions.py 中声明，并在对应处理器中实现逻辑。

## 故障排查指南
- 守护进程未运行：CLI 会尝试 ensure_daemon，若失败则返回 canonical_error。
- Worker 启动失败：守护进程会重试并清理状态，必要时重启 Worker。
- 远程复制失败：Android 推送后通过 sha256 校验，失败时抛出明确错误并清理远端文件。
- 会话冲突：重复 session_id 会报错，需确保唯一性。
- 前置条件缺失：工具目录中的 prerequisites 会在执行前校验，缺失时返回结构化错误。
- **更新** Android远程呈现故障：
  - `presented`状态：需要成功的原生GPU呈现和新鲜正序列号。
  - `unavailable`状态：表面、传输或渲染失败，不重用旧的完成序列。
  - `unsupported`状态：缺少原生绑定能力，需要安装匹配的运行时和辅助程序。
  - `not_applicable`状态：本地回放，不支持远程呈现。

**章节来源**
- [rdc_tool/cli.py:234-264](file://rdc_tool/cli.py#L234-L264)
- [rdc_tool/daemon/server.py:532-570](file://rdc_tool/daemon/server.py#L532-L570)
- [rdc_tool/core/session_manager.py:373-440](file://rdc_tool/core/session_manager.py#L373-L440)
- [rdc_tool/tool_router.py:89-128](file://rdc_tool/tool_router.py#L89-L128)
- [rdc_tool/replay_observation.py:120-132](file://rdc_tool/replay_observation.py#L120-L132)

## 结论
RDC-Tool 通过清晰的四层架构实现了 CLI、守护进程、核心引擎与处理器的职责分离，结合 JSON-first 的 rd.* 操作接口与工具目录驱动的设计，提供了稳定、可扩展且易于集成的调试与回放能力。会话管理与上下文隔离确保了多任务场景下的资源安全与状态清晰；本地与 Android 远程回放的双后端支持满足了不同平台的调试需求。

**更新** 最新的Android原生设备呈现功能已完全集成到会话观察操作中，通过`rd.session.observe`提供统一的API接口，使用拥有远程回放连接进行显示输出，支持完整的呈现状态报告和序列号验证机制。开发者可基于此架构快速扩展新操作、优化性能或集成新的后端。