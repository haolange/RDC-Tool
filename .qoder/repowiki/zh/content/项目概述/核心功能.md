...
## 简介
RDC-Tool（rdx-tools）是一个以 CLI 为主的 RenderDoc .rdc 回放与分析工具集，提供 JSON-first 的 rd.* 操作，覆盖事件浏览、资源探查、性能分析、导出、Shader 调试与预览窗口控制等能力。它通过会话管理实现多上下文隔离，并通过 headless 渲染与 readback 将 GPU 数据转换为可分析的图像或数值产物。
...
- [render_service.py:346-800](file://rdx/core/render_service.py#L346-L800)
- [debug_service.py:43-614](file://rdx/core/debug_service.py#L43-L614)
- [preview_window.py:192-437](file://rdx/preview_window.py#L192-L437)

## 架构总览
RDX 采用“处理器层 + 服务层 + 后端”的分层设计：
...
**图示来源**
- [render_service.py:357-646](file://rdx/core/render_service.py#L357-L646)
...
**图示来源**
- [session_manager.py:175-258](file://rdx/core/session_manager.py#L175-L258)
...
**章节来源**
- [session_manager.py:91-115](file://rdx/core/session_manager.py#L91-L115)
