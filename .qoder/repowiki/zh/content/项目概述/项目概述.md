...
- [rdc_tool/handlers/resource.py](file://rdc_tool/handlers/resource.py)
...
## 更新摘要
**所做更改**
- 更新了包命名空间：从 `rdx` 迁移到 `rdc_tool`，包括所有核心模块、处理器、守护进程组件和工具
- 更新了项目结构描述以反映新的 `rdc_tool` 包布局
- 更新了导入路径和模块引用以使用新的命名空间
- 保持了原有的架构设计和功能特性不变
...
## 简介
RDC-Tool（发行包名 `rdc-tool`，Python 包 `rdc_tool`）是一个面向 RenderDoc .rdc 捕获文件的命令行工具集，支持 Windows x64 本地回放与 Android 远程回放。它通过 JSON 优先的 rd.* 操作暴露能力，提供 GPU 调试、分析、会话管理、资源查询、事件浏览、管线状态对比、截图导出等能力。CLI 入口为 `rdc-tool`，内部通过守护进程（daemon）与工作进程协同，统一调度渲染后端（RenderDoc），并对外输出结构化结果（JSON/TSV）。
...
## 项目结构
仓库采用分层组织，已完成从 `rdx` 到 `rdc_tool` 的重大重构：
- 顶层 CLI 启动器与包装脚本
- rdc_tool 核心库：引擎、会话管理、处理器、运行时路径、配置、进度、超时策略等
- daemon 子模块：基于命名管道的守护进程与服务端处理
- handlers：按功能域划分的处理器（capture、resource、pipeline、shader 等）
- utils：工具模块（artifact_store、image_utils、scheduler）
...
```mermaid
graph TB
A["用户终端"] --> B["CLI 入口<br/>cli/run_cli.py / rdc_tool/cli.py"]
...
**图表来源**
- [cli/run_cli.py:42-86](file://cli/run_cli.py#L42-L86)
...
- [rdc_tool/core/engine.py:30-76](file://rdc_tool/core/engine.py#L30-L76)
- [rdc_tool/core/session_manager.py:176-251](file://rdc_tool/core/session_manager.py#L176-L251)
...
## 核心组件
- CLI 启动器与参数解析：负责环境初始化、依赖检查、版本/诊断信息输出，并将命令路由到 rdc_tool 主 CLI
...
**章节来源**
- [cli/run_cli.py:42-86](file://cli/run_cli.py#L42-L86)
- [rdc_tool/cli.py:54-118](file://rdc_tool/cli.py#L54-L118)
- [rdc_tool/daemon/server.py:102-165](file://rdc_tool/daemon/server.py#L102-L165)
- [rdc_tool/core/engine.py:30-76](file://rdc_tool/core/engine.py#L30-L76)
- [rdc_tool/core/session_manager.py:176-251](file://rdc_tool/core/session_manager.py#L176-L251)
...
sequenceDiagram
participant U as "用户"
participant CL as "CLI<br/>rdc_tool/cli.py"
...
U->>CL : 调用 rdc-tool capture open --file ...
...
- [rdc_tool/core/engine.py:40-76](file://rdc_tool/core/engine.py#L40-L76)
...
### CLI 启动器与主 CLI
- 启动器（cli/run_cli.py）：
  - 解析 RDC_TOOL_ROOT、初始化 Python 路径与环境变量
  - 依赖缺失检测，必要时输出最小化 doctor 结果
  - 引导渲染运行时后，委派给 rdc_tool.cli.main
- 主 CLI（rdc_tool/cli.py）：
...
- [rdc_tool/cli.py:62-118](file://rdc_tool/cli.py#L62-L118)
- [rdc_tool/cli.py:234-264](file://rdc_tool/cli.py#L234-L264)
...
- [rdc_tool/core/session_manager.py:384-441](file://rdc_tool/core/session_manager.py#L384-L441)
- [rdc_tool/core/session_manager.py:442-508](file://rdc_tool/core/session_manager.py#L442-L508)

**章节来源**
- [rdc_tool/core/session_manager.py:176-251](file://rdc_tool/core/session_manager.py#L176-L251)
- [rdc_tool/core/session_manager.py:301-508](file://rdc_tool/core/session_manager.py#L301-L508)
...
- [rdc_tool/handlers/resource.py:1-11](file://rdc_tool/handlers/resource.py#L1-L11)

## 依赖关系分析
- 构建与运行：
  - Python >= 3.11
  - 依赖：aiofiles、jinja2、numpy、Pillow、pydantic
  - 入口脚本：rdc-tool = rdc_tool.cli:main
- 外部运行时：
...
## 故障排查指南
- 环境不完整：
  - 使用 rdc-tool --json doctor 检查依赖、Python 布局、RenderDoc 文件完整性、SPIR-V 工具可用性
- 守护进程异常：
  - 通过 rdc-tool daemon status 查看运行状态与活跃请求数
  - 若出现未响应，尝试 rdc-tool daemon stop/start 重置
- 捕获打开失败：
...
- [rdc_tool/core/session_manager.py:347-441](file://rdc_tool/core/session_manager.py#L347-L441)
...
- 基础命令示例：
  - rdc-tool --version / rdc-tool version --json
  ...
  - rdc completion powershell
- 预览控制：
  - rdc-tool session preview on/status/off
  - 在 context status 中检查 preview.display

**章节来源**
- [docs/install.md:1-45](file://docs/install.md#L1-L45)
- [docs/quickstart.md:1-49](file://docs/quickstart.md#L1-L49)
