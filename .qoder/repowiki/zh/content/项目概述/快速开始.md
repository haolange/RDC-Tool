...
## 简介
本指南面向首次使用 RDC-Tool（rdc-tools）的用户，目标是帮助你在 Windows x64 平台上快速完成安装与环境验证，并通过常用命令打开 .rdc 捕获文件、查看事件列表、查询管线状态等，实现“最短时间上手”的核心工作流。工具以 JSON 为协议主格式，提供 TSV 表格投影用于列表类命令的输出。

## 项目结构
RDC-Tool 是一个自包含的 Windows x64 包，无需额外安装 Python 或虚拟环境。主要入口包括：
- bin/rdc-tool.cmd：Windows 启动器
- bin/rdc-tool：POSIX Shell 启动器
- cli/run_cli.py：Python CLI 启动器
- rdc_tool/cli.py：CLI 命令路由与执行逻辑
- scripts/rdc_tool_install.ps1：Windows 安装/升级/卸载/诊断脚本
- binaries/windows/x64：运行时二进制与内置 Python 环境

```mermaid
graph TB
A["用户终端"] --> B["bin/rdc-tool.cmd / bin/rdc-tool"]
B --> C["cli/run_cli.py"]
C --> D["rdc_tool/cli.py"]
D --> E["守护进程(daemon)"]
E --> F["渲染后端(RenderDoc)"]
...
- [rdc_tool/cli.py:953-1021](file://rdc_tool/cli.py#L953-L1021)
- [rdc_tool/cli.py:888-900](file://rdc_tool/cli.py#L888-L900)
- [rdc_tool/cli.py:689-691](file://rdc_tool/cli.py#L689-L691)

## 架构总览
rdc 通过 CLI 层将用户命令转换为对守护进程的请求，由守护进程协调渲染后端（RenderDoc）执行具体操作，并以统一 JSON 信封返回结果。TSV 仅作为列表类命令的表格投影。

```mermaid
sequenceDiagram
participant U as "用户"
participant CLI as "cli/run_cli.py"
participant RDC as "rdc_tool/cli.py"
participant DM as "守护进程"
participant RD as "RenderDoc"
U->>CLI : rdc-tool --version
CLI-->>U : 版本信息(JSON/文本)
U->>CLI : rdc-tool --json doctor
CLI->>RDC : 诊断
RDC->>DM : 状态检查
DM-->>RDC : 运行状态
RDC-->>U : 诊断结果(JSON)
U->>CLI : rdc-tool capture open --file sample.rdc --frame-index 0
CLI->>RDC : 解析参数
RDC->>DM : rd.core.init / rd.capture.open_file / rd.capture.open_replay / rd.replay.set_frame
DM->>RD : 打开捕获并设置帧
RD-->>DM : 会话ID/活动事件ID
DM-->>RDC : 成功
RDC-->>U : 打开结果(JSON)
U->>CLI : rdc-tool event list --format tsv
CLI->>RDC : 事件列表
RDC->>DM : rd.event.get_action_tree
DM-->>RDC : 事件树
RDC-->>U : 表格(TSV)/JSON
```
...
- [rdc_tool/cli.py:410-532](file://rdc_tool/cli.py#L410-L532)
- [rdc_tool/cli.py:1042-1171](file://rdc_tool/cli.py#L1042-L1171)
- [rdc_tool/cli.py:953-962](file://rdc_tool/cli.py#L953-L962)
...
- 安装：powershell -ExecutionPolicy Bypass -File scripts/rdc_tool_install.ps1 -Action install -InstallDir "$env:LOCALAPPDATA\Programs\rdc-tools" -AddToPath
- 升级：同上，将 Action 改为 upgrade
- 诊断：powershell -ExecutionPolicy Bypass -File scripts/rdc_tool_install.ps1 -Action doctor -InstallDir "$env:LOCALAPPDATA\Programs\rdc-tools"
- 卸载：powershell -ExecutionPolicy Bypass -File scripts/rdc_tool_install.ps1 -Action uninstall -InstallDir "$env:LOCALAPPDATA\Programs\rdc-tools"

章节来源
- [docs/install.md:1-31](file://docs/install.md#L1-L31)
- [scripts/rdc_tool_install.ps1:1-171](file://scripts/rdc_tool_install.ps1#L1-L171)

### 基础命令行用法
- 版本检查：rdc-tool --version 或 rdc-tool version --json
- 环境诊断：rdc-tool --json doctor
- 工具列表：rdc-tool tools list --json 或 rdc-tool tools search pipeline --json
- 上下文状态：rdc-tool context status --json
- 打开捕获：rdc-tool capture open --file "C:\path\sample.rdc" --frame-index 0
- 事件列表：rdc-tool event list --format tsv
- 管线状态：rdc-tool pipeline show --event-id 42 --format json
- 资源列表：rdc-tool resource list --format tsv
- 自动补全：rdc completion powershell
...
章节来源
- [rdc_tool/cli.py:953-972](file://rdc_tool/cli.py#L953-L972)
...
章节来源
- [rdc_tool/cli.py:1012-1021](file://rdc_tool/cli.py#L1012-L1021)
...
## 依赖关系分析
- 启动器：cli/run_cli.py 负责解析环境变量、设置工具根路径、检查依赖并调用 rdc_tool/cli.py。
- CLI：rdc_tool/cli.py 负责命令路由、参数解析、JSON/TSV 输出、守护进程通信。
- 守护进程：封装 RenderDoc 操作，提供会话生命周期管理与资源隔离。
- 工具清单：从 catalog 加载可用操作与描述，供 tools list/search/describe 使用。

```mermaid
graph LR
L["cli/run_cli.py"] --> C["rdc_tool/cli.py"]
C --> D["守护进程"]
C --> K["工具清单(catalog)"]
D --> R["RenderDoc"]
```
...
## 故障排除指南
- 首选诊断：运行 rdc-tool --json doctor，检查工具根路径、Python 布局、RenderDoc 组件、工具清单数量、启动器存在性与守护进程状态。
- 会话状态：使用 rdc-tool context status --json 查看当前上下文；如会话不正确，先清理再重新打开捕获。
