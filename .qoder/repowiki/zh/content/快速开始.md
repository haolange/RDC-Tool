...
```mermaid
graph TB
A["用户"] --> B["Windows 启动器 rdx.bat"]
A --> C["POSIX 启动器 bin/rdx"]
B --> D["PowerShell 启动器 scripts/rdx_bat_launcher.ps1"]
C --> E["Python 解释器优先使用内置 python.exe"]
D --> E
E --> F["CLI 引导 cli/run_cli.py"]
...
## 核心组件
- 启动器层
  - Windows 启动器 rdx.bat：负责调用 PowerShell 启动器，支持 --non-interactive 模式。
...
## 架构总览
下图展示了从用户输入到命令执行的完整流程，以及关键组件之间的交互。
...
- 工具根目录
  - 通过 RDX_TOOLS_ROOT 或脚本所在目录解析；确保 rdx.bat、bin/rdx、cli/run_cli.py 存在。
...
C --> L["启动器选择 Python<br/>bin/rdx / rdx.bat"]
...
## 结论
通过本指南，你已了解 RDC-Tool 的环境要求、Windows 安装流程、常用命令与冒烟测试方法，并掌握了基本的故障定位与恢复步骤。建议在日常使用中优先采用 JSON 输出进行自动化处理，TSV 用于人类可读的列表浏览。遇到问题时，先从 doctor 与 context status 入手，结合日志与冒烟测试快速定位。
...
- 入口点包括 Windows BAT、POSIX shell 与 Python CLI。
