...
## 更新摘要
**所做的更改**
- 更新了Windows启动器说明，从复杂的PowerShell系统简化为简单的bin/rdc-tool.cmd薄启动器
- 移除了对 legacy `rdc_bat_launcher.ps1` 的引用；当前 Windows 入口为 `bin/rdc-tool.cmd`。
- 更新了架构图表以反映新的启动流程
- 修正了安装和验证步骤中的入口点说明
- 更新了故障排除指南以匹配新的启动器架构
...
## 简介
本快速开始指南面向首次使用 RDC-Tool（rdc-tools）的用户，目标是帮助你在 Windows 平台上快速完成环境准备、安装与验证，并掌握常用命令行操作。你将学会：
- 检查系统与环境要求
...
- **Windows 启动器：bin/rdc-tool.cmd** - 简化的薄启动器，直接调用内置Python
- POSIX Shell 启动器：bin/rdc-tool
- Python CLI 引导：cli/run_cli.py
- 主 CLI 实现：rdc_tool/cli.py
- 安装脚本：scripts/rdc_tool_install.ps1
- 冒烟测试脚本：scripts/smoke_cli.sh

```mermaid
graph TB
A["用户"] --> B["Windows 启动器 bin/rdc-tool.cmd"]
A --> C["POSIX 启动器 bin/rdc-tool"]
B --> D["内置 Python 解释器"]
C --> E["Python 解释器优先使用内置 python.exe"]
D --> F["CLI 引导 cli/run_cli.py"]
E --> F
F --> G["主 CLI rdc_tool/cli.py"]
...
## 核心组件
- **启动器层**
  - **Windows 启动器 bin/rdc-tool.cmd** - 简化的薄启动器，直接设置环境变量并调用内置Python，无复杂PowerShell逻辑
  - POSIX 启动器 bin/rdc-tool：优先使用内置 Python，其次回退到系统 python3/python。
- CLI 层
  - cli/run_cli.py：解析工具根目录、设置环境变量、检查依赖、路由命令。
  - rdc_tool/cli.py：实现 version、doctor、tools、context、capture、vfs、event、pipeline、export 等命令。
- 运行时与资源
...
- 安装与验证
  - scripts/rdc_tool_install.ps1：复制文件、可选更新 PATH、执行 doctor。
...
sequenceDiagram
participant U as "用户"
participant CMD as "bin/rdc-tool.cmd"
participant BIN as "bin/rdc-tool"
...
- Python 环境要求
  ...
  - 若需自定义 Python，可通过环境变量 RDC_TOOL_PYTHON 指定可执行文件。
- 必需 Python 依赖
...
- 工具根目录
  - 通过 RDC_TOOL_ROOT 或脚本所在目录解析；确保 bin/rdc-tool.cmd、bin/rdc-tool、cli/run_cli.py 存在。
...
- [bin/rdc-tool:7-18](file://bin/rdc-tool#L7-L18)
- [rdc_tool/runtime_paths.py:14-63](file://rdc_tool/runtime_paths.py#L14-L63)
- [docs/install.md:1-39](file://docs/install.md#L1-L39)

### Windows 平台安装说明
- 获取安装包
  - 下载 rdc-tool-<version>-windows-x64.zip。
- 解压与安装
  - 解压后在 PowerShell 中执行安装脚本，可选择将安装目录加入 PATH。
  - 示例命令：
    - powershell -ExecutionPolicy Bypass -File scripts/rdc_tool_install.ps1 -Action install -InstallDir "$env:LOCALAPPDATA\Programs\rdc-tools" -AddToPath
    - 升级：将 -Action 改为 upgrade
    - 卸载：将 -Action 改为 uninstall
- 安装后验证
  - 使用安装脚本自带的 doctor 或直接运行 rdc-tool --json doctor。
  - 预期：返回 ok=true 的诊断信息，包含工具根目录、Python 布局、RenderDoc 资源、启动器存在性、目录结构等。

**更新** 现在使用简化的 bin/rdc-tool.cmd 启动器，不再需要PowerShell中间层。

**章节来源**
- [docs/install.md:5-39](file://docs/install.md#L5-L39)
- [scripts/rdc_tool_install.ps1:141-186](file://scripts/rdc_tool_install.ps1#L141-L186)
...
- 工具发现
  - rdc-tool tools list --json --limit 5
  - rdc-tool tools search pipeline --json
- 上下文状态
  - rdc-tool context status --json
- 打开捕获文件并预览
  - rdc-tool capture open --file "C:\path\sample.rdc" --frame-index 0 --preview
...
- 非交互模式（Windows）：rdc-tool --non-interactive --json doctor。
...
- 缺少依赖
  ...
  - 处理：确认内置 Python 可用或正确设置 RDC_TOOL_PYTHON；重新安装或修复 binaries。
- 会话状态不正确
...
- **新增：Windows启动器问题**
  - 现象：bin/rdc-tool.cmd 找不到或无法执行
  - 处理：确认安装目录包含 bin/rdc-tool.cmd；检查PATH设置；重新运行安装脚本
...
C --> L["启动器选择 Python<br/>bin/rdc-tool.cmd / bin/rdc-tool"]
...
- **新增：启动器优化** - 新的 bin/rdc-tool.cmd 启动器更轻量，减少了PowerShell开销，启动速度更快。
...
- 第二步：检查上下文
  - rdc-tool context status --json
  - 如状态异常，执行 rdc-tool context clear --json 后重新打开捕获。
- 第三步：查看日志
  - 冒烟测试日志：intermediate/logs/smoke_cli.log
  - 发现摘要：intermediate/logs/tool_smoke_findings.md
- 第四步：清理与重启
  - rdc-tool daemon stop
  - rdc-tool context clear
  - 重新执行冒烟测试或目标命令。
- **新增：Windows启动器故障排除**
  - 如果 rdc 命令无法识别，检查PATH是否正确设置
  - 确认 bin/rdc-tool.cmd 文件存在且可执行
...
## 结论
通过本指南，你已了解 RDC-Tool 的环境要求、Windows 安装流程、常用命令与冒烟测试方法，并掌握了基本的故障定位与恢复步骤。**更新后的架构更加简洁高效**，新的 bin/rdc-tool.cmd 薄启动器提供了更快的启动速度和更少的依赖。建议在日常使用中优先采用 JSON 输出进行自动化处理，TSV 用于人类可读的列表浏览。遇到问题时，先从 doctor 与 context status 入手，结合日志与冒烟测试快速定位。
...
### 版本与入口点
- 版本号来源于 rdc_tool/__init__.py，CLI 输出包含 schema_version、tool_version、platform、entrypoints 等信息。
- **更新后的入口点**：
  - Windows CMD：bin/rdc-tool.cmd（简化的薄启动器）
  - POSIX shell：bin/rdc-tool
...
- [rdc_tool/cli.py:535-556](file://rdc_tool/cli.py#L535-L556)
- [bin/rdc-tool.cmd:1-12](file://bin/rdc-tool.cmd#L1-L12)
