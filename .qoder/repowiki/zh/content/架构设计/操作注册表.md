...
## 简介
本文件面向“操作注册表”机制，系统性说明 RDX 工具运行时如何动态发现、注册、校验并调用操作（Operation）。重点包括：
...
## 项目结构
RDX 的操作体系由“定义层 + 目录层 + 路由层 + 处理器层 + 执行层”构成：
...
- [runtime_catalog.py:1-91](file://rdx/runtime_catalog.py#L1-L91)
...
- [models.py:100-125](file://rdx/models.py#L100-L125)
...
- [tool_router.py:56-154](file://rdx/tool_router.py#L56-L154)
- [runtime_catalog.py:38-91](file://rdx/runtime_catalog.py#L38-L91)
...
- 约定：
  ...
  - 领域处理器位于 rdx/handlers/<domain>.py，并提供 handle(action, args, env)。
...
**章节来源**
- [runtime_catalog.py:24-35](file://rdx/runtime_catalog.py#L24-L35)
- [runtime_catalog.py:38-91](file://rdx/runtime_catalog.py#L38-L91)
...
**章节来源**
- [operation_definitions.py:324-393](file://rdx/operation_definitions.py#L324-L393)
- [server_runtime.py:1-800](file://rdx/server_runtime.py#L1-L800)
...
## 结论
RDX 的操作注册表通过“集中式定义 + 目录构建 + 路由分发 + 严格校验”的架构，实现了高内聚、低耦合的动态操作发现与加载。OperationRegistry 提供简洁的注册与解析接口；ToolRouter 将目录与处理器解耦；ServerRuntime 统一了上下文、监控、错误与超时策略。该设计便于扩展新操作、维护向后兼容性与提升系统可观测性。
...
### 步骤二：实现领域处理器
- 若为新领域：在 rdx/handlers 下新建 <domain>.py，提供 handle(action, args, env) 异步函数。
