...
## 项目结构
围绕错误处理的关键模块如下：
- rdc_tool/core/errors.py：定义 CoreError 及其子类，提供 map_exception 将底层异常映射为统一错误对象。
- rdc_tool/core/contracts.py：定义统一的响应信封 canonical_success/canonical_error，用于 CLI 输出和跨进程通信。
- rdc_tool/cli.py：CLI 入口，负责捕获异常、构造错误载荷、渲染结果或错误、设置退出码。
- rdc_tool/timeout_policy.py：超时策略，决定不同操作的超时时长，间接影响错误传播（如超时导致的失败）。
- rdc_tool/handlers/core.py：处理器转发层，调用 server_runtime 执行核心操作。

```mermaid
graph TB
A["CLI 层<br/>rdc_tool/cli.py"] --> B["处理器转发<br/>rdc_tool/handlers/core.py"]
B --> C["运行时调度<br/>server_runtime._dispatch_core(...)"]
C --> D["核心错误模型<br/>rdc_tool/core/errors.py"]
C --> E["统一响应信封<br/>rdc_tool/core/contracts.py"]
A --> F["超时策略<br/>rdc_tool/timeout_policy.py"]
```
...
- [rdc_tool/timeout_policy.py:56-95](file://rdc_tool/timeout_policy.py#L56-L95)
...
- [rdc_tool/core/contracts.py:98-164](file://rdc_tool/core/contracts.py#L98-L164)
- [rdc_tool/cli.py:107-118](file://rdc_tool/cli.py#L107-L118)
- [rdc_tool/cli.py:385-394](file://rdc_tool/cli.py#L385-L394)
- [rdc_tool/timeout_policy.py:56-95](file://rdc_tool/timeout_policy.py#L56-L95)
...
participant H as "处理器<br/>rdc_tool/handlers/core.py"
participant SR as "运行时调度<br/>server_runtime"
participant ERR as "错误模型<br/>rdc_tool/core/errors.py"
participant CON as "统一信封<br/>rdc_tool/core/contracts.py"
participant T as "超时策略<br/>rdc_tool/timeout_policy.py"
U->>CLI : 调用命令
CLI->>T : 计算操作超时
