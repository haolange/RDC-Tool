...
- [rdx/runtim_catalog.py](file://rdx/runtime_catalog.py)
...
## 简介
本架构设计文档面向 RDC-Tool（RDX）系统，系统性阐述分层架构与扩展机制：CLI 层、守护进程层、核心引擎层与处理器层的职责分离；操作注册与动态发现；守护进程的命名管道通信与消息协议；会话状态管理与上下文隔离；错误处理与异常传播；以及可扩展性与性能考量。文档提供组件交互图与数据处理流程图，帮助读者快速理解并扩展系统。
...
```mermaid
graph TB
CLI["命令行入口<br/>cli/run_cli.py"] --> DaemonClient["守护进程客户端<br/>rdx/daemon/client.py"]
DaemonClient --> NamedPipe["命名管道<br/>\\\\.\\pipe\\{name}"]
NamedPipe --> DaemonServer["守护进程服务<br/>rdx/daemon/server.py"]
DaemonServer --> WorkerProc["工作进程<br/>rdx/daemon/worker.py"]
WorkerProc --> Runtime["运行时服务<br/>rdx/server_runtime.py"]
Runtime --> CoreEngine["核心引擎<br/>rdx/core/engine.py"]
CoreEngine --> Registry["操作注册表<br/>rdx/core/operation_registry.py"]
Registry --> ToolRouter["工具路由<br/>rdx/tool_router.py"]
ToolRouter --> Handlers["处理器集合<br/>rdx/handlers/*"]
...
- [rdx/daemon/worker.py:24-143](file://rdx/daemon/worker.py#L24-L143)
- [rdx/server_runtime.py:1-120](file://rdx/server_runtime.py#L1-L120)
- [rdx/core/engine.py:30-75](file://rdx/core/engine.py#L30-L75)
- [rdx/core/operation_registry.py:18-43](file://rdx/core/operation_registry.py#L18-L43)
...
- [rdx/core/operation_registry.py:18-43](file://rdx/core/operation_registry.py#L18-L43)
- [rdx/tool_router.py:131-154](file://rdx/tool_router.py#L131-L154)
- [rdx/daemon/server.py:102-643](file://rdx/daemon/server.py#L102-L643)
- [rdx/daemon/worker.py:24-203](file://rdx/daemon/worker.py#L24-L203)
- [rdx/server_runtime.py:1-120](file://rdx/server_runtime.py#L1-L120)
- [rdx/core/session_manager.py:148-293](file://rdx/core/session_manager.py#L148-L293)
- [rdx/context_snapshot.py:174-220](file://rdx/context_snapshot.py#L174-L220)
...
### 扩展点与插件机制
- 处理器扩展：在 rdx/handlers 下新增模块，实现 handle(action, args, env)，并在 tool_router 中注册域名映射。
...
- [rdx/core/session_manager.py:347-382](file://rdx/core/session_manager.py#L347-L382)
...
- 参考路径：
  - 操作定义：rdx/operation_definitions.py
  - 工具清单：rdx/runtime_catalog.py
  - 处理器目录：rdx/handlers/*
  - 守护进程：rdx/daemon/server.py
  - 工作进程：rdx/daemon/worker.py
  - 核心引擎：rdx/core/engine.py
  - 会话管理：rdx/core/session_manager.py
  - 上下文快照：rdx/context_snapshot.py
  - 运行时状态：rdx/runtime_state.py
