...
## 简介
本指南面向使用 rdx-tools 进行图形捕获回放与调试的工程师，覆盖以下能力：
...
- CLI 层（cli）：暴露 rdx 命令，负责参数解析、上下文选择、预览控制、结果输出。
- 文档与参考：工具清单、分组、参数契约、故障排查。

```mermaid
graph TB
CLI["CLI 入口<br/>rdx 命令"] --> Handlers["处理器层<br/>handlers/*"]
...
图表来源
- [cli.py:1120-1194](file://rdx/cli.py#L1120-L1194)
- [debug.py:8-10](file://rdx/handlers/debug.py#L8-L10)
- [debug_service.py:43-66](file://rdx/core/debug_service.py#L43-L66)
- [render_service.py:346-422](file://rdx/core/render_service.py#L346-L422)
...
图表来源
- [debug.py:8-10](file://rdx/handlers/debug.py#L8-L10)
- [debug_service.py:122-213](file://rdx/core/debug_service.py#L122-L213)
- [render_service.py:421-521](file://rdx/core/render_service.py#L421-L521)
- [pipeline_service.py:607-702](file://rdx/core/pipeline_service.py#L607-L702)
- [diff_service.py:21-50](file://rdx/core/diff_service.py#L21-L50)
...
图表来源
- [debug_service.py:94-213](file://rdx/core/debug_service.py#L94-L213)
...
- [cli.py:1197-1210](file://rdx/cli.py#L1197-L1210)
...
章节来源
- [render_service.py:235-338](file://rdx/core/render_service.py#L235-L338)
...
- [pipeline_service.py:175-373](file://rdx/core/pipeline_service.py#L175-L373)
...
### 调试会话管理、断点持久化、条件断点
- 会话与上下文
  - rdx context status/update/list/clear：查看/更新/列出/清除上下文。
...
章节来源
- [render_service.py:6-11](file://rdx/core/render_service.py#L6-L11)
- [debug_service.py:83-87](file://rdx/core/debug_service.py#L83-L87)
- [render_service.py:304-333](file://rdx/core/render_service.py#L304-L333)
- [perf_service.py:32-37](file://rdx/core/perf_service.py#L32-L37)
- [debug_service.py:204-213](file://rdx/core/debug_service.py#L204-L213)

## 故障排查指南
- 前置检查
  - 运行 rdx --json doctor 检查工具根路径、Python 运行时、RenderDoc DLL/PYD 布局、catalog 数量、启动器与守护进程状态。
  - 使用 rdx context status --json 检查活跃上下文；若捕获/会话不正确，执行 rdx context clear --json 后重新打开 .rdc。
- 远程生命周期
...
## 结论
rdx-tools 提供了完整的图形调试与诊断能力：从着色器级逐步执行、变量与调用栈检查，到管线状态快照、资源绑定验证、图像与管线差异对比，再到日志与运行时指标采集。通过上下文隔离、预览窗口与 artifacts 持久化，可实现高效的问题定位与复现。遵循最佳实践与故障排查步骤，可显著降低调试成本并提高准确性。

## 附录：常用命令与操作索引
- 环境与上下文
  - rdx --json doctor
  - rdx context status --json
  - rdx context update --key notes --value "..." --json
  - rdx context clear --json
- 捕获与会话
  - rdx capture open --file "<path>" --frame-index 0
