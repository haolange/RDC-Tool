...
## 简介
本指南面向RDC工具链中的“连接问题”，覆盖守护进程通信失败、命名管道连接、进程状态检查、端口占用检测、远程连接超时（含Android设备）、网络配置验证、防火墙与代理设置、IP与端口映射、连接状态监控、调试信息收集、常见错误日志分析与解决方案，以及连接测试与故障恢复流程。内容基于仓库中守护进程、客户端、工作进程、运行时调度与配置的源码实现进行系统化梳理。
...
**图表来源**
- [rdc_tool/daemon/client.py:118-120](file://rdc_tool/daemon/client.py#L118-L120)
- [rdc_tool/daemon/server.py:657-690](file://rdc_tool/daemon/server.py#L657-L690)
- [rdc_tool/daemon/worker.py:69-119](file://rdc_tool/daemon/worker.py#L69-L119)
- [rdc_tool/server.py:62-142](file://rdc_tool/server.py#L62-L142)
...
- [rdc_tool/runtime_worker.py:1-167](file://rdc_tool/runtime_worker.py#L1-L167)
...
- [rdc_tool/server.py:62-142](file://rdc_tool/server.py#L62-L142)
- [rdc_tool/config.py:15-35](file://rdc_tool/config.py#L15-L35)
...
**图表来源**
- [rdc_tool/daemon/server.py:166-170](file://rdc_tool/daemon/server.py#L166-L170)
- [rdc_tool/daemon/server.py:572-643](file://rdc_tool/daemon/server.py#L572-L643)
...
- [rdc_tool/daemon/client.py:467-515](file://rdc_tool/daemon/client.py#L467-L515)
- [rdc_tool/daemon/client.py:518-525](file://rdc_tool/daemon/client.py#L518-L525)
- [rdc_tool/daemon/client.py:539-552](file://rdc_tool/daemon/client.py#L539-L552)
...
**图表来源**
- [rdc_tool/daemon/worker.py:24-45](file://rdc_tool/daemon/worker.py#L24-L45)
- [rdc_tool/daemon/worker.py:57-136](file://rdc_tool/daemon/worker.py#L57-L136)
- [rdc_tool/daemon/worker.py:144-170](file://rdc_tool/daemon/worker.py#L144-L170)
...
**章节来源**
- [rdc_tool/daemon/server.py:532-570](file://rdc_tool/daemon/server.py#L532-L570)
- [rdc_tool/daemon/client.py:25-28](file://rdc_tool/daemon/client.py#L25-L28)
- [rdc_tool/daemon/worker.py:20-21](file://rdc_tool/daemon/worker.py#L20-L21)
- [rdc_tool/config.py:28-35](file://rdc_tool/config.py#L28-L35)
...
**章节来源**
- [rdc_tool/daemon/server.py:85-100](file://rdc_tool/daemon/server.py#L85-L100)
- [rdc_tool/daemon/client.py:362-399](file://rdc_tool/daemon/client.py#L362-L399)
...
**章节来源**
- [rdc_tool/daemon/server.py:222-225](file://rdc_tool/daemon/server.py#L222-L225)
- [rdc_tool/daemon/client.py:159-175](file://rdc_tool/daemon/client.py#L159-L175)
...
### 连接测试工具与故障恢复流程
- 测试工具：
  - “rdc-tool --json doctor”快速诊断环境与守护进程状态。
  - “rdc-tool context status --json”查看上下文与会话。
  - “rdc-tool tools search/pipeline”验证工具可用性。
- 恢复流程：
  - 清理上下文：“rdc-tool context clear --json”释放会话/预览/远端/快照。
  - 停止守护进程：“rdc-tool daemon stop --json”停止对应命名空间的守护进程与工作进程。
...
## 结论
RDC的连接链路围绕命名管道与子进程协作构建，具备完善的认证、心跳、租约与生命周期管理机制。排查连接问题时，应优先检查守护进程状态、token一致性、活动操作与最后活动时间，再结合远程后端配置与网络环境进行诊断。通过“doctor”、“context status”与状态文件收集调试信息，配合上下文清理与守护进程重启，可有效恢复连接与服务。
...
- 恢复命令：
  ...
  - rdc-tool daemon stop --json
  - rdc-tool capture open --file <path> [--remote-id "<id>"]
- 注意事项：
