...
## 简介
本指南面向在 Windows x64 上使用 rdc-tools（CLI-only RenderDoc .rdc runtime）的用户与自动化流程，聚焦“环境问题诊断”。内容覆盖：
...
- CLI 入口为 `rdc-tool`，支持 doctor、version、tools、daemon、context、capture、session、export 等命令。
...
- [rdc_tool/runtime_requirements.py:7-57](file://rdc_tool/runtime_requirements.py#L7-L57)
- [rdc_tool/runtime_bootstrap.py:105-130](file://rdc_tool/runtime_bootstrap.py#L105-L130)
- [scripts/rdc_tool_install.ps1:119-167](file://scripts/rdc_tool_install.ps1#L119-L167)
...
- 运行时引导：
  ...
  - 可通过环境变量 RDC_TOOL_RUNTIME_DLL_DIR、RDC_TOOL_RENDERDOC_PATH 覆盖默认路径。
...
- [rdc_tool/runtime_bootstrap.py:60-90](file://rdc_tool/runtime_bootstrap.py#L60-L90)
- [rdc_tool/core/errors.py:60-77](file://rdc_tool/core/errors.py#L60-L77)
...
## 故障排查指南
- 第一步：运行 rdc-tool --json doctor，关注以下字段：
...
## 结论
通过内置 Python、严格的布局校验、依赖检测与 RenderDoc/SPIR-V 工具可用性检查，rdc-tools 提供了完善的环境健康诊断能力。遇到问题时，优先使用 doctor 与安装脚本进行快速定位与修复；结合上下文状态与远程生命周期说明，可高效恢复工作流。
...
### 自动化脚本使用方法
- 安装/升级/卸载/诊断：
  - 使用 scripts/rdc_tool_install.ps1，参数 Action=install|upgrade|uninstall|doctor，InstallDir 指定安装目录，AddToPath 可选添加用户 PATH，DryRun 预览变更。
  - 升级后会自动执行 doctor 验证。
- Smoke 测试：
  - 通过 bash scripts/smoke_cli.sh 执行端到端检查；可传入 --rdc 与 --context 指定捕获与上下文。
- 环境变量：
  - RDC_TOOL_INTERMEDIATE_ROOT：隔离输出根目录，便于任务隔离与清理。
