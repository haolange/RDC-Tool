...
- [rdc_tool/progress.py](file://rdc_tool/progress.py)
...
## 更新摘要
**变更内容**
- 新增"会话守护进程超时"专门章节，详细说明60秒工作预算与65秒CLI传输截止时间的关系
- 更新了连接问题排查部分，包含新的会话超时指导
- 增强了性能注意事项中的超时策略说明
- 添加了关于避免每调用超时覆盖和编辑已安装包的建议
...
## 简介
本指南面向使用 RDC-Tool（RDC）的用户与运维人员，聚焦"可操作"的排障方法：从环境检查、连接问题、性能诊断到日志收集与调试工具使用，提供分步骤的诊断流程、常见错误解释与修复建议。文档中的每个技术点均对应仓库中的具体实现或配置，便于快速定位与验证。

## 项目结构
RDC 采用"CLI + 守护进程 + 工作进程 + 运行时状态/日志"的分层设计：
...
- [rdc_tool/config.py:15-130](file://rdc_tool/config.py#L15-L130)
...
- [rdc_tool/daemon/server.py:102-690](file://rdc_tool/daemon/server.py#L102-L690)
- [rdc_tool/daemon/client.py:31-515](file://rdc_tool/daemon/client.py#L31-L515)
- [rdc_tool/runtime_paths.py:14-123](file://rdc_tool/runtime_paths.py#L14-L123)
- [rdc_tool/runtime_state.py:104-500](file://rdc_tool/runtime_state.py#L104-L500)
- [rdc_tool/timeout_policy.py:56-104](file://rdc_tool/timeout_policy.py#L56-L104)
- [rdc_tool/progress.py:14-86](file://rdc_tool/progress.py#L14-L86)
- [rdc_tool/python_runtime.py:104-172](file://rdc_tool/python_runtime.py#L104-L172)
- [rdc_tool/runtime_requirements.py:7-57](file://rdc_tool/runtime_requirements.py#L7-L57)
...
- [rdc_tool/daemon/server.py:572-643](file://rdc_tool/daemon/server.py#L572-L643)
- [rdc_tool/daemon/client.py:467-515](file://rdc_tool/daemon/client.py#L467-L515)
- [rdc_tool/runtime_state.py:448-500](file://rdc_tool/runtime_state.py#L448-L500)
...
图表来源
- [rdc_tool/daemon/server.py:572-690](file://rdc_tool/daemon/server.py#L572-L690)
- [rdc_tool/daemon/server.py:166-225](file://rdc_tool/daemon/server.py#L166-L225)
...
图表来源
- [rdc_tool/daemon/client.py:467-515](file://rdc_tool/daemon/client.py#L467-L515)
- [rdc_tool/daemon/client.py:518-553](file://rdc_tool/daemon/client.py#L518-L553)
- [rdc_tool/daemon/client.py:554-607](file://rdc_tool/daemon/client.py#L554-L607)

章节来源
- [rdc_tool/daemon/client.py:467-515](file://rdc_tool/daemon/client.py#L467-L515)
- [rdc_tool/daemon/client.py:518-607](file://rdc_tool/daemon/client.py#L518-L607)
...
图表来源
- [rdc_tool/python_runtime.py:28-92](file://rdc_tool/python_runtime.py#L28-L92)
- [rdc_tool/python_runtime.py:104-172](file://rdc_tool/python_runtime.py#L104-L172)
- [rdc_tool/runtime_paths.py:14-123](file://rdc_tool/runtime_paths.py#L14-L123)
...
- **会话上下文超时**：`rd.session.*` 操作使用 60 秒工作预算，CLI 传输层额外添加 5 秒响应缓冲，形成 65 秒截止时间。
...
- [rdc_tool/core/perf_service.py:69-96](file://rdc_tool/core/perf_service.py#L69-L96)
...
章节来源
- [rdc_tool/core/perf_service.py:212-618](file://rdc_tool/core/perf_service.py#L212-L618)
- [docs/troubleshooting.md:29-35](file://docs/troubleshooting.md#L29-L35)
- [rdc_tool/config.py:27-43](file://rdc_tool/config.py#L27-L43)
- [rdc_tool/timeout_policy.py:56-104](file://rdc_tool/timeout_policy.py#L56-L104)
...
- Python 版本与布局不兼容
  ...
  - 参考：[内置 Python 校验:104-172](file://rdc_tool/python_runtime.py#L104-L172)、[路径解析:14-123](file://rdc_tool/runtime_paths.py#L14-L123)
- 系统依赖缺失
  - 现象：导入第三方库失败。
  - 诊断：使用依赖检测获取缺失包列表。
  - 解决：安装缺失依赖（如 pydantic、numpy、Pillow、jinja2、aiofiles）。
  - 参考：[依赖检测:7-57](file://rdc_tool/runtime_requirements.py#L7-L57)
- 权限问题
  - 现象：读写状态/日志失败、无法创建目录。
  - 诊断：检查 intermediate/runtime 目录权限与磁盘空间。
  - 解决：以管理员/具备写权限的账户运行，或调整目录 ACL。
  - 参考：[原子写入与目录创建:117-147](file://rdc_tool/io_utils.py#L117-L147)、[状态文件写入:404-416](file://rdc_tool/runtime_state.py#L404-L416)

章节来源
- [rdc_tool/python_runtime.py:104-172](file://rdc_tool/python_runtime.py#L104-L172)
- [rdc_tool/runtime_paths.py:14-123](file://rdc_tool/runtime_paths.py#L14-L123)
- [rdc_tool/runtime_requirements.py:7-57](file://rdc_tool/runtime_requirements.py#L7-L57)
- [rdc_tool/io_utils.py:117-147](file://rdc_tool/io_utils.py#L117-L147)
- [rdc_tool/runtime_state.py:404-416](file://rdc_tool/runtime_state.py#L404-L416)
...
- 守护进程通信失败
  ...
  - 参考：[守护进程状态与清理:554-607](file://rdc_tool/daemon/client.py#L554-L607)、[守护进程启动:623-733](file://rdc_tool/daemon/client.py#L623-L733)
- 远程连接超时
  - 现象：rd.remote.connect 或 open_replay 超时。
  - 诊断：根据操作类型查看超时策略；检查网络可达性与端口配置。
  - 解决：增大超时或修正网络/代理设置；避免重复连接导致句柄被消费。
  - 参考：[超时策略](file://rdc_tool/timeout_policy.py:56-104)、[远程生命周期说明](file://docs/troubleshooting.md:13-19)
- 网络配置错误
  ...
  - 参考：[后端配置](file://rdc_tool/config.py:15-25)、[Android 连接恢复](file://docs/troubleshooting.md:47-52)

章节来源
- [rdc_tool/daemon/client.py:554-733](file://rdc_tool/daemon/client.py#L554-L733)
- [rdc_tool/timeout_policy.py:56-104](file://rdc_tool/timeout_policy.py#L56-L104)
- [rdc_tool/config.py:15-25](file://rdc_tool/config.py#L15-L25)
...
### 会话守护进程超时
- **超时机制说明**
  - `rd.session.*` 操作具有 60 秒的工作预算（worker budget）
  - CLI 守护进程传输层额外添加共享的 5 秒响应缓冲，形成 65 秒的截止时间
  - 这与主机应用程序的外部进程超时是分开的
- **超时诊断**
  - 如果主机报告在 65 秒之前超时，验证其使用的是当前打包的 RDC-Tool
  - 确认外部 CLI 截止时间大于 65 秒
  - 检查会话观察操作的序列化行为（回放导航、目标选择、图像导出）
- **解决方案**
  - 不要添加每调用超时覆盖（per-call timeout override）
  - 不要就地编辑已安装的包
  - 更新并重新安装 RDC-Tool 构建，使策略和测试保持为单一事实来源
  - 确保主机应用程序的外部 CLI 等待时间足够长（RDC-Agent 默认为 120 秒）
- **相关测试验证**
  - 会话上下文超时测试验证了 `rd.session.get_context` 和 `rd.session.observe` 的操作
  - 工作进程执行超时使用 60 秒会话上下文窗口
  - 守护进程执行超时使用会话上下文窗口加上响应缓冲

章节来源
- [docs/troubleshooting.md:13-22](file://docs/troubleshooting.md#L13-L22)
- [docs/session-model.md:42-46](file://docs/session-model.md#L42-L46)
- [rdc_tool/timeout_policy.py:14-17](file://rdc_tool/timeout_policy.py#L14-L17)
- [tests/test_timeout_policy.py:72-82](file://tests/test_timeout_policy.py#L72-L82)

### 性能问题分析
- 内存泄漏检测
  - 方法：结合 recent_operations 与 metrics.operation_error_count 观察异常增长；对大对象导出（纹理/缓冲区）谨慎使用，优先按需读取。
  - 参考：[运行时状态指标](file://rdc_tool/runtime_state.py:104-149)、[纹理导出限制](file://docs/troubleshooting.md:29-35)
- I/O 瓶颈识别
  - 方法：关注 large readback、导出格式不支持导致的失败；检查磁盘空间与写入速度。
  - 参考：[原子写入与备份](file://rdc_tool/io_utils.py:67-115)、[导出失败处理](file://docs/troubleshooting.md:29-35)
- CPU 使用率优化
  - 方法：合理设置超时与并发；避免重复枚举/抓取 counters；使用 detect_hotspots 定位昂贵事件。
  - 参考：[性能计数器热点检测](file://rdc_tool/core/perf_service.py:493-618)、[超时策略](file://rdc_tool/timeout_policy.py:56-104)

章节来源
- [rdc_tool/runtime_state.py:104-149](file://rdc_tool/runtime_state.py#L104-L149)
- [rdc_tool/io_utils.py:67-115](file://rdc_tool/io_utils.py#L67-L115)
- [docs/troubleshooting.md:29-35](file://docs/troubleshooting.md#L29-L35)
...
### 日志收集与诊断信息
- 位置与内容
  - 运行时状态：intermediate/runtime/rdc_cli/runtime_state*.json
  - 运行时日志：intermediate/runtime/rdc_cli/runtime_logs*.jsonl
  - 守护进程状态：intermediate/runtime/rdc_cli/daemon_state*.json
- 采集方式
...
- 参考
  - [状态与日志路径](file://rdc_tool/runtime_paths.py:83-123)、[状态读取/写入](file://rdc_tool/runtime_state.py:388-500)、[守护进程状态](file://rdc_tool/daemon/client.py:257-307)、[冒烟脚本](file://scripts/smoke_cli.sh:178-197)

章节来源
- [rdc_tool/runtime_paths.py:83-123](file://rdc_tool/runtime_paths.py#L83-L123)
- [rdc_tool/runtime_state.py:388-500](file://rdc_tool/runtime_state.py#L388-L500)
- [rdc_tool/daemon/client.py:257-307](file://rdc_tool/daemon/client.py#L257-L307)
...
- 调试服务
  ...
  - 参考：[守护进程方法](file://rdc_tool/daemon/server.py:572-643)、[客户端心跳/附加](file://rdc_tool/daemon/client.py:754-800)
- 状态检查
...
- 性能监控
  ...
  - 参考：[性能服务](file://rdc_tool/core/perf_service.py:212-618)、[状态指标](file://rdc_tool/runtime_state.py:104-149)
- 最佳实践
...
章节来源
- [rdc_tool/daemon/server.py:572-643](file://rdc_tool/daemon/server.py#L572-L643)
- [rdc_tool/daemon/client.py:754-800](file://rdc_tool/daemon/client.py#L754-L800)
...
- validation_error / not_found / assertion_failed / runtime_error / permission_error / io_error / internal_error
  ...
  - 参考：[错误分类与映射](file://rdc_tool/core/errors.py:9-119)
- daemon_timeout
  - 含义：等待守护进程响应超时。
  - 处理：检查守护进程状态与活跃请求；清理残留状态并重试。
  - 参考：[客户端超时异常](file://rdc_tool/daemon/client.py:31-37)、[超时处理](file://rdc_tool/daemon/client.py:467-515)
...
- **会话超时优化**：对于 `rd.session.*` 操作，理解 60 秒工作预算与 65 秒传输截止时间的关系，确保主机应用的外部超时设置适当。
- 参考：[配置项](file://rdc_tool/config.py:27-43)、[超时策略](file://rdc_tool/timeout_policy.py:56-104)、[性能服务](file://rdc_tool/core/perf_service.py:212-618)
...
## 结论
通过统一的状态与日志、健壮的守护进程通信与超时策略、以及完善的性能计数器能力，RDC 提供了端到端的排障与调优手段。建议在日常使用中遵循"先查状态与日志、再定位连接与超时、最后进行性能分析"的流程，并结合本文提供的模板与步骤高效解决问题。

## 附录

### 问题报告模板与信息收集清单
- 基本信息
  - 操作系统与版本、RDC 版本、工具根目录（tools_root）
...
- 连接与超时
  ...
  - **会话超时特定信息**：如果是 `rd.session.*` 操作超时，记录是否在使用当前打包的 RDC-Tool，外部 CLI 截止时间是否大于 65 秒
- 状态与日志
...
- 参考
  ...
  - [状态与日志路径](file://rdc_tool/runtime_paths.py:83-123)
