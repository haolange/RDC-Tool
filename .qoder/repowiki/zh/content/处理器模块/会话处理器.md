...
```mermaid
graph TB
A["调用方/CLI"] --> B["session 处理器<br/>rdx/handlers/session.py"]
B --> C["服务器运行时<br/>rdx/server_runtime.py"]
C --> D["会话管理器<br/>rdx/core/session_manager.py"]
C --> E["上下文快照<br/>rdx/context_snapshot.py"]
C --> F["运行时状态<br/>rdx/runtime_state.py"]
C --> G["预览窗口<br/>rdx/preview_window.py"]
D --> H["RenderDoc API"]
G --> I["Windows 用户态窗口"]
```
...
- [rdx/server_runtime.py:106-120](file://rdx/server_runtime.py#L106-L120)
...
- [rdx/runtime_state.py:41-76](file://rdx/runtime_state.py#L41-L76)
- [rdx/preview_window.py:192-238](file://rdx/preview_window.py#L192-L238)
...
章节来源
- [rdx/handlers/session.py:8-12](file://rdx/handlers/session.py#L8-L12)
- [rdx/core/session_manager.py:148-259](file://rdx/core/session_manager.py#L148-L259)
- [rdx/context_snapshot.py:214-279](file://rdx/context_snapshot.py#L214-L279)
- [rdx/runtime_state.py:104-149](file://rdx/runtime_state.py#L104-L149)
- [rdx/preview_window.py:192-238](file://rdx/preview_window.py#L192-L238)
- [rdx/server_runtime.py:450-540](file://rdx/server_runtime.py#L450-L540)
...
图表来源
- [rdx/core/session_manager.py:118-146](file://rdx/core/session_manager.py#L118-L146)
- [rdx/core/session_manager.py:148-259](file://rdx/core/session_manager.py#L148-L259)
...
- [rdx/core/session_manager.py:509-569](file://rdx/core/session_manager.py#L509-L569)
- [rdx/models.py:128-157](file://rdx/models.py#L128-L157)
...
图表来源
- [rdx/context_snapshot.py:214-279](file://rdx/context_snapshot.py#L214-L279)
- [rdx/context_snapshot.py:367-444](file://rdx/context_snapshot.py#L367-L444)
- [rdx/context_snapshot.py:447-475](file://rdx/context_snapshot.py#L447-L475)
- [rdx/context_snapshot.py:486-541](file://rdx/context_snapshot.py#L486-L541)
...
- [rdx/runtime_state.py:448-472](file://rdx/runtime_state.py#L448-L472)
...
图表来源
- [rdx/preview_window.py:192-238](file://rdx/preview_window.py#L192-L238)
- [rdx/preview_window.py:274-301](file://rdx/preview_window.py#L274-L301)
...
- [rdx/server_runtime.py:977-1071](file://rdx/server_runtime.py#L977-L1071)
- [rdx/server_runtime.py:1102-1200](file://rdx/server_runtime.py#L1102-L1200)
...
- [rdx/preview_window.py:221-238](file://rdx/preview_window.py#L221-L238)
- [rdx/server_runtime.py:977-1071](file://rdx/server_runtime.py#L977-L1071)

## 结论
RDX的会话处理器通过清晰的层次划分与模块化设计，实现了稳健的会话生命周期管理、可靠的上下文快照机制以及灵活的预览窗口控制。借助服务器运行时的协调，各组件协同工作，既保证了性能与稳定性，也为GPU调试与分析提供了良好的扩展点。
...
- 创建会话
  ...
  - 参考路径：[创建会话:175-206](file://rdx/core/session_manager.py#L175-L206)
- 打开捕获
  - 本地：确保.rdc文件有效，使用OpenCaptureFile/OpenCapture。
  - 远程：先CopyCaptureToRemote，再OpenCapture，注意进度回调。
  - 参考路径：[打开本地捕获:347-372](file://rdx/core/session_manager.py#L347-L372)、[打开远程捕获:373-440](file://rdx/core/session_manager.py#L373-L440)
- 切换会话
  - 通过服务器运行时选择当前会话/捕获，并持久化状态。
  - 参考路径：[选择会话状态:910-922](file://rdx/server_runtime.py#L910-L922)
- 销毁会话
  - 关闭输出、控制器、捕获文件、远程连接，必要时关闭回放子系统。
  - 参考路径：[清理会话:517-569](file://rdx/core/session_manager.py#L517-L569)

### 上下文快照与持久化策略
- 保存与恢复
  - 使用save_context_snapshot/load_context_snapshot进行原子读写，支持多上下文隔离。
  - 参考路径：[保存快照:463-475](file://rdx/context_snapshot.py#L463-L475)、[加载快照:447-460](file://rdx/context_snapshot.py#L447-L460)
- 保留策略
  - 通过SnapshotRetentionPolicy限制总数与每类型数量，避免无限增长。
  - 参考路径：[保留策略](file://rdx/context_snapshot.py:39-43)(file://rdx/context_snapshot.py#L39-L43)、[裁剪产物:335-364](file://rdx/context_snapshot.py#L335-L364)
- 预览状态
  - 使用normalize_preview_state/display进行归一化，确保兼容不同来源的数据。
  - 参考路径：[预览状态归一化:130-171](file://rdx/context_snapshot.py#L130-L171)、[显示状态归一化:83-127](file://rdx/context_snapshot.py#L83-L127)

### 预览窗口控制与帧缓冲预览
- 启动与关闭
  - 使用PreviewWindowHost.start/close管理窗口线程与消息循环。
  - 参考路径：[启动窗口:221-238](file://rdx/preview_window.py#L221-L238)、[关闭窗口:243-252](file://rdx/preview_window.py#L243-L252)
- 帧缓冲适配
  - 根据帧缓冲尺寸与工作区计算窗口大小，支持强制刷新。
  - 参考路径：[应用帧缓冲几何:274-301](file://rdx/preview_window.py#L274-L301)
- 用户交互
  - 处理WM_SIZE/WM_ENTERSIZEMOVE/WM_EXITSIZEMOVE，支持手动覆盖尺寸。
  - 参考路径：[窗口过程:171-189](file://rdx/preview_window.py#L171-L189)、[用户调整:303-318](file://rdx/preview_window.py#L303-L318)

### GPU调试与分析工作流示例（路径引用）
- 初始化上下文与配置
  - 参考路径：[应用运行时配置:319-390](file://rdx/server_runtime.py#L319-L390)
- 打开捕获并创建会话
  - 参考路径：[打开捕获:208-251](file://rdx/core/session_manager.py#L208-L251)
- 设置帧与事件
  - 参考路径：[更新会话记录:846-897](file://rdx/server_runtime.py#L846-L897)
- 启用预览
  - 参考路径：[预览状态存储:523-540](file://rdx/server_runtime.py#L523-L540)、[预览窗口几何:274-301](file://rdx/preview_window.py#L274-L301)
- 记录操作与阶段
  - 参考路径：[记录开始/阶段/结束:977-1071](file://rdx/server_runtime.py#L977-L1071)
- 导出产物与快照
  - 参考路径：[合并产物:511-541](file://rdx/context_snapshot.py#L511-L541)、[保存快照:463-475](file://rdx/context_snapshot.py#L463-L475)
