...
## 项目结构
RDC 采用“目录即命名空间”的处理器组织方式：每个领域（如 buffer、capture、core、session 等）在 rdc_tool/handlers 下提供统一的 handle(action, args, env) 入口，由 tool_router 根据 catalog 动态路由到对应领域处理器，再由 server_runtime 将 action 分派到具体实现。核心引擎 CoreEngine 负责统一执行、规范化输出与错误归一化。
...
章节来源
- [operation_registry.py:18-43](file://rdc_tool/core/operation_registry.py#L18-L43)
- [engine.py:30-204](file://rdc_tool/core/engine.py#L30-L204)
- [contracts.py:98-164](file://rdc_tool/core/contracts.py#L98-L164)
...
- [handlers/capture.py:8-10](file://rdc_tool/handlers/capture.py#L8-L10)
- [handlers/buffer.py:8-10](file://rdc_tool/handlers/buffer.py#L8-L10)
- [handlers/session.py:8-12](file://rdc_tool/handlers/session.py#L8-L12)
- [handlers/util.py:8-10](file://rdc_tool/handlers/util.py#L8-L10)
...
图表来源
- [tool_router.py:131-154](file://rdc_tool/tool_router.py#L131-L154)
- [operation_registry.py:33-43](file://rdc_tool/core/operation_registry.py#L33-L43)
...
- 示例路径：
  - core 领域：[handle:8-10](file://rdc_tool/handlers/core.py#L8-L10)
  - capture 领域：[handle:8-10](file://rdc_tool/handlers/capture.py#L8-L10)
  - buffer 领域：[handle:8-10](file://rdc_tool/handlers/buffer.py#L8-L10)
  - session 领域（含特殊分支）：[handle:8-12](file://rdc_tool/handlers/session.py#L8-L12)
  - util 领域：[handle:8-10](file://rdc_tool/handlers/util.py#L8-L10)
...
- 参考路径：
  - 校验入口：[build_operation_registry:131-154](file://rdc_tool/tool_router.py#L131-L154)
  - 校验实现：[validate_operation_arguments:38-91](file://rdc_tool/runtime_catalog.py#L38-L91)
  - 操作定义与 schema：[OPERATIONS:1-800](file://rdc_tool/operation_definitions.py#L1-L800)
...
- 参考路径：
  - 前置条件判定：[_enforce_prerequisites:112-128](file://rdc_tool/tool_router.py#L112-L128)
  - 上下文快照读取：[_has_prerequisite:89-109](file://rdc_tool/tool_router.py#L89-L109)

章节来源
- [tool_router.py:80-128](file://rdc_tool/tool_router.py#L80-L128)
...
- 参考路径：
  - 资源句柄与运行时状态：[CaptureFileHandle/ReplayHandle/RemoteHandle/...:120-238](file://rdc_tool/server_runtime.py#L120-L238)
  - 会话/上下文状态存取：[_context_state/_store_context_state:450-468](file://rdc_tool/server_runtime.py#L450-L468)
  - 预览绑定与显示状态：[PreviewBinding/_preview_update_payload:217-228](file://rdc_tool/server_runtime.py#L217-L228), [file://rdc_tool/server_runtime.py#L482-L531]
...
- 参考路径：
  - 规范化输出：[_normalize_output:77-204](file://rdc_tool/core/engine.py#L77-L204)
  - 成功/失败契约：[canonical_success/canonical_error:98-164](file://rdc_tool/core/contracts.py#L98-L164)
  - 产物候选收集：[collect_artifact_candidates:191-221](file://rdc_tool/core/contracts.py#L191-L221)
...
- 参考路径：
  - 错误类定义：[CoreError 及子类:9-87](file://rdc_tool/core/errors.py#L9-L87)
  - 异常映射：[map_exception:90-119](file://rdc_tool/core/errors.py#L90-L119)
  - 引擎异常捕获：[execute:40-75](file://rdc_tool/core/engine.py#L40-L75)
...
- 参考路径：
  - 上下文状态存取：[_context_state/_store_context_state:450-468](file://rdc_tool/server_runtime.py#L450-L468)
  - 容量与占用检查：[_ensure_context_capacity/_occupying_context_ids:417-448](file://rdc_tool/server_runtime.py#L417-L448)
  - 预览状态更新：[_preview_update_payload/_store_preview_state:482-531](file://rdc_tool/server_runtime.py#L482-L531)

章节来源
- [server_runtime.py:417-531](file://rdc_tool/server_runtime.py#L417-L531)
...
- 参考路径：
  - 注册与路由：[build_operation_registry:131-154](file://rdc_tool/tool_router.py#L131-L154)
  - 执行与归一化：[CoreEngine.execute/_normalize_output:40-204](file://rdc_tool/core/engine.py#L40-L204)
  - 契约封装：[canonical_success/canonical_error:98-164](file://rdc_tool/core/contracts.py#L98-L164)
...
- 参考路径：
  - 上下文变量定义：[_CURRENT_CONTEXT_ID/_CURRENT_PROGRESS_REPORTER:99-103](file://rdc_tool/server_runtime.py#L99-L103)
  - 进度上报：[_progress:248-266](file://rdc_tool/server_runtime.py#L248-L266)

章节来源
- [server_runtime.py:99-103](file://rdc_tool/server_runtime.py#L99-L103)
- [server_runtime.py:248-266](file://rdc_tool/server_runtime.py#L248-L266)

### 完整实现示例（从简单查询到复杂操作）
- 简单查询（core.get_version）：
  - 处理器：[core.handle:8-10](file://rdc_tool/handlers/core.py#L8-L10)
  - 分派：server_runtime._dispatch_core("get_version", args)
  - 返回：由引擎规范化为 canonical_success，包含版本信息。
- 复杂操作（buffer.get_structured_data）：
  - 处理器：[buffer.handle:8-10](file://rdc_tool/handlers/buffer.py#L8-L10)
...
- 会话相关（session.observe）：
  - 处理器：[session.handle:8-12](file://rdc_tool/handlers/session.py#L8-L12)
...
- 参考路径：
  - 日志与指标：[core.get_logs/core.get_runtime_metrics:324-393](file://rdc_tool/operation_definitions.py#L324-L393)
  - 健康检查：[core.healthcheck:442-464](file://rdc_tool/operation_definitions.py#L442-L464)

章节来源
- [operation_definitions.py:324-464](file://rdc_tool/operation_definitions.py#L324-L464)

## 结论
通过统一的处理器接口、catalog 驱动的路由与校验、核心引擎的规范化输出与错误映射，RDC 提供了可扩展、可观测且高性能的操作执行框架。新增处理器只需遵循 handle(action, args, env) 约定，并在 server_runtime 中实现 _dispatch_* 分派逻辑，即可无缝接入现有生态。借助产物机制、进度上报与上下文状态管理，可轻松实现从简单查询到复杂工作流的多样化场景。
