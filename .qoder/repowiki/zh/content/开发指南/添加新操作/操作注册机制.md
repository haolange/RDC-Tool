...
## 简介
本文件系统性说明 RDX 工具链中“操作”（tool）的发现、注册、路由与执行机制。重点覆盖：
- 命名空间组织原则与模块划分
- 版本管理与指纹校验
- 依赖声明与前置条件检查
...
## 项目结构
RDX 将“操作”以声明式方式集中定义，并在运行时构建路由表，按命名空间分发到具体处理器。关键路径如下：
- 操作定义：`rdx/operation_definitions.py` 中的 `OPERATIONS` 列表
- 目录生成与校验：`spec/tool_catalog.json` 由代码生成并带指纹
- 目录加载与参数校验：`rdx/runtime_catalog.py`
- 路由与注册：`rdx/tool_router.py` 使用 `OperationRegistry`
- 领域处理器：`rdx/handlers/*` 下的各模块
- 运行时上下文与调度：`rdx/server_runtime.py`

```mermaid
graph TB
A["操作定义<br/>rdx/operation_definitions.py"] --> B["目录构建与校验<br/>rdx/runtime_catalog.py"]
B --> C["JSON 目录产物<br/>spec/tool_catalog.json"]
C --> D["路由与注册<br/>rdx/tool_router.py"]
D --> E["领域处理器<br/>rdx/handlers/*"]
E --> F["运行时调度<br/>rdx/server_runtime.py"]
```

**图表来源**
- [rdx/operation_definitions.py:1-120](file://rdx/operation_definitions.py#L1-L120)
- [rdx/runtime_catalog.py:17-35](file://rdx/runtime_catalog.py#L17-L35)
...
- [rdx/server_runtime.py:1-10](file://rdx/server_runtime.py#L1-L10)
...
**章节来源**
- [rdx/core/operation_registry.py:12-45](file://rdx/core/operation_registry.py#L12-L45)
- [rdx/runtime_catalog.py:17-91](file://rdx/runtime_catalog.py#L17-L91)
- [rdx/tool_router.py:34-57,131-154:34-57](file://rdx/tool_router.py#L34-L57)
...
- [rdx/server_runtime.py:90-120](file://rdx/server_runtime.py#L90-L120)
...
**章节来源**
- [rdx/operation_definitions.py:283-306](file://rdx/operation_definitions.py#L283-L306)
- [rdx/operation_definitions.py:465-489](file://rdx/operation_definitions.py#L465-L489)
- [rdx/operation_definitions.py:2268-2285](file://rdx/operation_definitions.py#L2268-L2285)

## 操作发现、加载与热重载
- 发现来源：`rdx/operation_definitions.py` 中的 `OPERATIONS` 列表
- 加载过程：
...
**图表来源**
- [rdx/runtime_catalog.py:17-35](file://rdx/runtime_catalog.py#L17-L35)
- [rdx/tool_router.py:56-57](file://rdx/tool_router.py#L56-L57)
- [rdx/tool_router.py:131-154](file://rdx/tool_router.py#L131-L154)
...
1. 定义：在 `rdx/operation_definitions.py` 中添加条目，包含 name、namespace、scope、prerequisites、input_schema 等
...
**章节来源**
- [rdx/runtime_catalog.py:17-21](file://rdx/runtime_catalog.py#L17-L21)
...
## 调试与故障排除
- 使用 `rdx --json doctor` 检查工具根目录、Python 运行时、RenderDoc 布局、目录计数、守护进程状态
- 使用 `rdx context status --json` 查看当前上下文，必要时 `rdx context clear --json` 清理并重开捕获
...
- 操作不存在：升级后旧名称不再执行，使用 `rdx tools search/describe` 查找新接口

**章节来源**
- [docs/troubleshooting.md:1-58](file://docs/troubleshooting.md#L1-L58)

## 结论
RDX 的操作注册机制以声明式定义为核心，通过目录生成、路由注册、参数校验与前置条件检查，实现了高内聚、低耦合的可扩展架构。命名空间清晰、作用域明确、依赖可验证，配合指纹与缓存机制，保障了系统的稳定性与性能。开发者只需遵循命名约定与依赖声明，即可快速添加新操作并被系统自动发现与加载。