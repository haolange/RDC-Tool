...
- 操作元数据集中定义在 rdc_tool/operation_definitions.py 的 OPERATIONS 数组中。
- 运行时通过 rdc_tool/runtime_catalog.py 加载并生成工具目录（catalog），并提供参数校验能力。
- rdc_tool/tool_router.py 负责将 rd.domain.action 解析为对应域处理器，并在调用前执行参数校验与前置条件检查。
- 各域处理器位于 rdc_tool/handlers/*，通常仅做轻量转发到 server_runtime 的具体实现。
- 统一注册表 rdc_tool/core/operation_registry.py 用于维护已注册操作名到异步处理器的映射。

```mermaid
graph TB
A["rdc_tool/operation_definitions.py<br/>OPERATIONS"] --> B["rdc_tool/runtime_catalog.py<br/>load_tool_catalog / validate_operation_arguments"]
B --> C["rdc_tool/tool_router.py<br/>build_operation_registry / _enforce_prerequisites"]
C --> D["rdc_tool/handlers/*.py<br/>handle(action, args, env)"]
...
- [tool_router.py:34-156](file://rdc_tool/tool_router.py#L34-L156)
...
- [handlers/capture.py:1-11](file://rdc_tool/handlers/capture.py#L1-L11)
...
- 若新增域，则需：
  - 创建 rdc_tool/handlers/<new_domain>.py，实现 handle。
...
**章节来源**
- [tool_router.py:64-77](file://rdc_tool/tool_router.py#L64-L77)
...
**章节来源**
- [tool_router.py:89-109](file://rdc_tool/tool_router.py#L89-L109)
- [operation_definitions.py:1-800](file://rdc_tool/operation_definitions.py#L1-L800)
...
- 在 rdc_tool/operation_definitions.py 的 OPERATIONS 数组末尾追加新条目。
- 关键字段：
...
- 若新增域，需：
  - 创建 rdc_tool/handlers/<new_domain>.py，实现 async handle(action, args, env)。
