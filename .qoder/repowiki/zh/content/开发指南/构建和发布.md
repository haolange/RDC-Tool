# 构建和发布

<cite>
**本文引用的文件**
- [pyproject.toml](file://pyproject.toml)
- [rdc_tool/__init__.py](file://rdc_tool/__init__.py)
- [scripts/package_release.py](file://scripts/package_release.py)
- [scripts/package_runtime.py](file://scripts/package_runtime.py)
- [scripts/release_gate.py](file://scripts/release_gate.py)
- [scripts/generate_release_checksums.py](file://scripts/generate_release_checksums.py)
- [scripts/verify_release_package.py](file://scripts/verify_release_package.py)
- [scripts/_shared.py](file://scripts/_shared.py)
- [scripts/generate_tool_reference.py](file://scripts/generate_tool_reference.py)
- [scripts/check_identity.py](file://scripts/check_identity.py)
- [scripts/check_markdown_health.py](file://scripts/check_markdown_health.py)
- [scripts/release_runtime.py](file://scripts/release_runtime.py)
- [spec/validate_catalog.py](file://spec/validate_catalog.py)
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml)
- [rdc_tool/runtime_requirements.py](file://rdc_tool/runtime_requirements.py)
- [rdc_tool/python_runtime.py](file://rdc_tool/python_runtime.py)
- [bin/rdc-tool.cmd](file://bin/rdc-tool.cmd)
- [spec/tool_catalog.json](file://spec/tool_catalog.json)
</cite>

## 更新摘要
**所做更改**
- 增强了发布门禁系统，集成隔离的运行时上下文以确保测试环境的纯净性
- 添加了发布前的变更日志验证检查，确保CHANGELOG.md包含当前版本号
- 改进了打包过程，使用临时暂存环境防止文件冲突和状态污染
- 实现了全面的发布身份验证，包括LICENSE字节比较和多文件版本一致性检查
- 增强了运行时清理机制，确保隔离环境中创建的上下文和守护进程被正确清理

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本指南面向RDC-Tool项目的构建与发布，覆盖以下主题：
- 构建系统配置与依赖管理（基于pyproject.toml）
- GitHub Actions CI工作流自动化（测试、验证、发布流程）
- 打包脚本使用方式（release包与runtime包）
- 版本管理策略（版本号规则与变更日志维护）
- 发布前检查清单（代码质量、测试验证、文档更新）
- 二进制打包与签名（Windows平台特殊处理）
- 自动化脚本说明（发布流水线）
- 回滚策略与发布后验证
- 第三方依赖许可证合规检查与SBOM生成

## 项目结构
本项目采用"源码+脚本+产物"的清晰分层：
- 源码位于 rdc_tool/ 与 cli/，通过 pyproject.toml 暴露命令行入口
- 构建与发布脚本集中在 scripts/，负责打包、校验、生成清单与校验和
- 运行时二进制与Python环境打包在 binaries/windows/x64/
- 工具目录规范与契约定义在 spec/ 与 docs/
- CI工作流配置在 .github/workflows/ci.yml，自动化测试和验证流程

```mermaid
graph TB
A["源码 rdc_tool/"] --> B["构建脚本 scripts/"]
C["配置 pyproject.toml"] --> B
D["运行时 binaries/windows/x64/"] --> E["发布产物 dist/"]
B --> E
F["契约 spec/"] --> B
G["文档 docs/"] --> B
H["CI工作流 .github/workflows/ci.yml"] --> I["自动化测试和验证"]
I --> J["契约验证"]
I --> K["工具参考生成"]
I --> L["单元测试执行"]
I --> M["Markdown健康检查"]
I --> N["身份验证"]
```

**图表来源**
- [pyproject.toml:1-45](file://pyproject.toml#L1-L45)
- [scripts/package_release.py:1-218](file://scripts/package_release.py#L1-L218)
- [scripts/package_runtime.py:1-358](file://scripts/package_runtime.py#L1-L358)
- [.github/workflows/ci.yml:1-45](file://.github/workflows/ci.yml#L1-L45)

**章节来源**
- [pyproject.toml:1-45](file://pyproject.toml#L1-L45)

## 核心组件
- 构建系统与依赖声明：pyproject.toml 定义了构建后端、包名、版本、Python版本要求、依赖与可选依赖，并注册了命令行入口。
- **增强** GitHub Actions CI工作流：自动化测试、验证和发布流程，包括契约验证、工具参考生成、单元测试、Markdown健康检查和身份验证。
- 版本来源：包版本由 rdc_tool/__init__.py 中的 __version__ 提供，打包脚本会读取该值用于产物命名与元数据。
- Runtime打包：scripts/package_runtime.py 将外部RenderDoc二进制与捆绑的CPython运行时复制到 binaries/windows/x64/，并生成 manifest.runtime.json。
- Release打包：scripts/package_release.py 从仓库根复制受控文件到临时暂存区，生成RELEASE_MANIFEST.json、LICENSE_INVENTORY.json、SBOM.json，并压缩为zip。
- **增强** 发布门禁：scripts/release_gate.py 执行结构与内容检查、运行公共命令契约测试、校验manifest与打包一致性、生成报告，并使用隔离的运行时上下文。
- **增强** 包验证：scripts/verify_release_package.py 解压并验证发布包的完整性、入口点、许可证清单、SBOM、公共命令契约等，包括全面的身份验证。
- **新增** 身份验证：scripts/check_identity.py 检查废弃的身份引用，确保代码库中不包含旧的rdx-tools相关标识。
- **新增** Markdown健康检查：scripts/check_markdown_health.py 验证文档编码、链接完整性和内容一致性。
- **新增** 契约验证：spec/validate_catalog.py 验证tool_catalog.json的结构和内容正确性。
- **新增** 隔离运行时：scripts/release_runtime.py 提供隔离的运行时上下文管理，确保测试环境的纯净性和资源清理。
- 辅助工具：scripts/_shared.py 提供路径解析、子进程调用、JSON读写等通用能力；scripts/generate_tool_reference.py 从代码生成工具参考文档；scripts/generate_release_checksums.py 生成SHA256校验和。

**章节来源**
- [pyproject.toml:1-45](file://pyproject.toml#L1-L45)
- [.github/workflows/ci.yml:1-45](file://.github/workflows/ci.yml#L1-L45)
- [rdc_tool/__init__.py:1-4](file://rdc_tool/__init__.py#L1-L4)
- [scripts/package_runtime.py:1-358](file://scripts/package_runtime.py#L1-L358)
- [scripts/package_release.py:1-218](file://scripts/package_release.py#L1-L218)
- [scripts/release_gate.py:1-701](file://scripts/release_gate.py#L1-L701)
- [scripts/verify_release_package.py:1-390](file://scripts/verify_release_package.py#L1-L390)
- [scripts/check_identity.py:1-29](file://scripts/check_identity.py#L1-L29)
- [scripts/check_markdown_health.py:1-243](file://scripts/check_markdown_health.py#L1-L243)
- [scripts/release_runtime.py:1-77](file://scripts/release_runtime.py#L1-L77)
- [spec/validate_catalog.py:1-162](file://spec/validate_catalog.py#L1-L162)
- [scripts/_shared.py:1-92](file://scripts/_shared.py#L1-L92)
- [scripts/generate_tool_reference.py:1-140](file://scripts/generate_tool_reference.py#L1-L140)
- [scripts/generate_release_checksums.py:1-56](file://scripts/generate_release_checksums.py#L1-L56)

## 架构总览
下图展示了从源码到发布产物的完整流程，包括GitHub Actions CI工作流的自动化测试和验证，以及Runtime打包、Release打包、门禁校验与包验证。

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant CI as "GitHub Actions CI"
participant PR as "package_runtime.py"
participant PB as "package_release.py"
participant RG as "release_gate.py"
participant VP as "verify_release_package.py"
participant IR as "isolated_release_runtime"
participant Dist as "dist/"
Note over CI : CI工作流自动执行
CI->>CI : 契约验证 (validate_catalog.py)
CI->>CI : 工具参考生成 (generate_tool_reference.py --check)
CI->>CI : 单元测试执行 (pytest -m "unit or contract")
CI->>CI : Markdown健康检查 (check_markdown_health.py)
CI->>CI : 身份验证 (check_identity.py)
Dev->>PR : 准备渲染器二进制与Python环境
PR-->>Dev : 写入 binaries/windows/x64/manifest.runtime.json
Dev->>PB : 构建release包
PB-->>Dist : 生成 rdc-tool-<version>-windows-x64.zip
PB-->>Dist : 生成 SHA256SUMS
Dev->>RG : 运行发布门禁
RG->>IR : 创建隔离运行时上下文
RG-->>Dev : 输出 intermediate/logs/release_gate_report.md
Dev->>VP : 验证发布包
VP->>IR : 使用隔离环境进行验证
VP-->>Dev : PASS/FAIL 结果
```

**图表来源**
- [.github/workflows/ci.yml:12-26](file://.github/workflows/ci.yml#L12-L26)
- [scripts/package_runtime.py:289-353](file://scripts/package_runtime.py#L289-L353)
- [scripts/package_release.py:169-213](file://scripts/package_release.py#L169-L213)
- [scripts/release_gate.py:521-696](file://scripts/release_gate.py#L521-L696)
- [scripts/verify_release_package.py:351-385](file://scripts/verify_release_package.py#L351-L385)
- [scripts/release_runtime.py:49-77](file://scripts/release_runtime.py#L49-L77)

## 详细组件分析

### 构建系统与依赖管理（pyproject.toml）
- 构建后端：使用 setuptools.build_meta，要求 setuptools>=68 与 wheel。
- 包信息：name=rdc-tools，version来自 rdc_tool/__init__.py，requires-python>=3.11，license=Apache-2.0。
- 依赖：aiofiles、jinja2、numpy、Pillow、pydantic。
- 可选依赖：dev包含pytest。
- 入口点：注册 rdc 命令指向 rdc_tool.cli:main。
- 包发现：setuptools.packages.find 包含 rdc*。
- pytest配置：测试路径tests，忽略.git、intermediate、binaries，定义标记unit/contract/fixture_integration/gpu_live。
- **增强** CI环境支持：CI工作流使用Python 3.14进行构建和测试。

**章节来源**
- [pyproject.toml:1-45](file://pyproject.toml#L1-L45)
- [.github/workflows/ci.yml:18-21](file://.github/workflows/ci.yml#L18-L21)
- [rdc_tool/__init__.py:1-4](file://rdc_tool/__init__.py#L1-L4)

### GitHub Actions CI工作流（.github/workflows/ci.yml）
**增强** CI工作流提供了完整的自动化测试和验证流程：

- **触发条件**：push到main分支或创建pull request时自动触发
- **并发控制**：使用concurrency组避免重复构建，支持取消进行中的任务
- **权限设置**：仅授予contents: read权限，遵循最小权限原则

#### 契约验证作业（contract job）
- 运行环境：windows-latest
- 步骤：
  1. 检出代码（支持LFS）
  2. 设置Python 3.14环境
  3. 安装开发依赖（pip install -e ".[dev]"）
  4. 执行契约验证（python spec/validate_catalog.py）
  5. 生成并检查工具参考文档（python scripts/generate_tool_reference.py --check）
  6. 运行单元测试和契约测试（pytest tests -q -m "unit or contract"）
  7. 执行Markdown健康检查（python scripts/check_markdown_health.py）
  8. 执行身份验证（python scripts/check_identity.py）

#### 冒烟测试作业（doctor-no-capture job）
- 运行环境：windows-latest
- 功能：验证打包的CLI在没有捕获环境下的基本功能
- 测试内容：
  - 执行 rdc-tool --version 验证版本信息
  - 执行 rdc-tool --json doctor 验证医生命令
  - 验证返回结果的result_kind字段
  - 检查退出码是否符合预期（0或2）

```mermaid
flowchart TD
Start(["CI触发"]) --> Checkout["检出代码"]
Checkout --> SetupPy["设置Python环境"]
SetupPy --> Install["安装依赖"]
Install --> Contract{"契约验证"}
Contract --> |通过| ToolRef["工具参考生成检查"]
Contract --> |失败| Fail["构建失败"]
ToolRef --> |通过| UnitTests["运行单元测试"]
ToolRef --> |失败| Fail
UnitTests --> |通过| MarkdownCheck["Markdown健康检查"]
UnitTests --> |失败| Fail
MarkdownCheck --> |通过| IdentityCheck["身份验证"]
MarkdownCheck --> |失败| Fail
IdentityCheck --> |通过| SmokeTest["冒烟测试"]
IdentityCheck --> |失败| Fail
SmokeTest --> |通过| Success["构建成功"]
SmokeTest --> |失败| Fail
Success --> End(["完成"])
Fail --> End
```

**图表来源**
- [.github/workflows/ci.yml:12-45](file://.github/workflows/ci.yml#L12-L45)

**章节来源**
- [.github/workflows/ci.yml:1-45](file://.github/workflows/ci.yml#L1-L45)

### Runtime包构建（scripts/package_runtime.py）
- 输入参数：
  - --source：渲染器源目录（相对或绝对路径）
  - --python-home：CPython home路径
  - --site-packages-source：site-packages源路径（默认 .venv/Lib/site-packages）
  - --python-version：覆盖捆绑Python版本元数据
- 行为要点：
  - 过滤允许的文件后缀（*.dll, *.json），排除调试符号与头文件
  - 复制Python基础文件（python.exe、pythonw.exe、python3.dll、vcruntime等）
  - 复制DLLs与Lib标准库，跳过顶层测试与缓存目录
  - 选择性捆绑site-packages，依据 rdc_tool/runtime_requirements.py 的白名单/黑名单
  - 生成 python{tag}._pth 以设置导入路径
  - 生成 manifest.runtime.json，记录所有文件及其sha256与size
- 错误处理：
  - 缺失python.exe或版本探测失败时报错
  - site-packages不能指向输出树内部，防止循环引用
  - 复制权限错误时提示并返回非零退出码

```mermaid
flowchart TD
Start(["开始"]) --> ParseArgs["解析参数"]
ParseArgs --> CheckSource{"是否提供渲染器源?"}
CheckSource --> |是| CopySrc["复制允许的二进制与JSON"]
CheckSource --> |否| SkipSrc["跳过复制渲染器源"]
CopySrc --> BundlePy{"是否提供Python Home?"}
SkipSrc --> BundlePy
BundlePy --> |是| Bundle["捆绑Python运行时<br/>复制基础文件/DLLs/Lib"]
BundlePy --> |否| UseExisting["复用已存在的捆绑Python元数据"]
Bundle --> SitePkgs["选择性复制site-packages"]
UseExisting --> SitePkgs
SitePkgs --> PTH["生成 _pth 与导入路径"]
PTH --> Manifest["生成 manifest.runtime.json"]
Manifest --> End(["结束"])
```

**图表来源**
- [scripts/package_runtime.py:289-353](file://scripts/package_runtime.py#L289-L353)
- [rdc_tool/runtime_requirements.py:1-73](file://rdc_tool/runtime_requirements.py#L1-L73)

**章节来源**
- [scripts/package_runtime.py:1-358](file://scripts/package_runtime.py#L1-L358)
- [rdc_tool/runtime_requirements.py:1-73](file://rdc_tool/runtime_requirements.py#L1-L73)

### Release包构建（scripts/package_release.py）
- **增强** 行为要点：
  - 从仓库根复制受控目录与文件（bin、binaries、cli、docs、policy、rdc、scripts、spec等）
  - 排除测试、缓存、中间产物与特定后缀（.pyc/.pdb/.lib等）
  - **新增** 使用临时暂存环境：通过 tempfile.TemporaryDirectory 创建隔离的暂存目录，防止文件冲突
  - 生成 RELEASE_MANIFEST.json（包含public_commands、entrypoints、files列表）
  - 生成 LICENSE_INVENTORY.json（扫描site-packages的*.dist-info/METADATA提取名称、版本、许可证）
  - 生成 SBOM.json（组件清单，schema=rdc-tools.sbom.v1）
  - 压缩为 rdc-tool-<version>-windows-x64.zip，并生成 SHA256SUMS
  - 输出 release_report.md 摘要
  - **增强** 版本校验：若传入--version与包内TOOL_VERSION不一致则报错退出
  - **增强** 清理机制：无论成功还是失败，都会清理临时暂存环境

```mermaid
flowchart TD
S(["开始"]) --> V["版本校验"]
V --> T["创建临时暂存环境"]
T --> C["收集受控文件到暂存区"]
C --> M["生成 RELEASE_MANIFEST.json"]
M --> L["生成 LICENSE_INVENTORY.json"]
L --> B["生成 SBOM.json"]
B --> Z["压缩为 zip"]
Z --> Ck["生成 SHA256SUMS"]
Ck --> R["生成 release_report.md"]
R --> Cl["清理临时环境"]
Cl --> E(["结束"])
```

**图表来源**
- [scripts/package_release.py:89-108](file://scripts/package_release.py#L89-L108)
- [scripts/package_release.py:169-213](file://scripts/package_release.py#L169-L213)

**章节来源**
- [scripts/package_release.py:1-218](file://scripts/package_release.py#L1-L218)

### 发布门禁（scripts/release_gate.py）
- **增强** 结构检查：必需目录与文件存在性
- **增强** 内容扫描：禁止调试框架引用、用户文档不得包含pip/uv/venv引导命令、禁止bin/rdc-tool.cmd示例命令
- **增强** 契约校验：
  - tool_catalog.json与代码定义一致，指纹匹配
  - 工具参考文档由 generate_tool_reference.py 生成且未过期
  - 公共命令帮助输出必须显示 usage: rdc，且不包含bin/rdc-tool.cmd示例
- **增强** 运行时校验：
  - 校验 manifest.runtime.json 完整性（文件数量、大小、sha256）
  - 校验捆绑Python布局（python_runtime.validate_bundled_python_layout）
- **增强** 公共命令冒烟：
  - 运行 rdc-tool --help/--version/tools list/context status/vfs ls 等
  - 负向用例期望特定错误码（如session_required、projection_not_supported）
- **新增** 变更日志验证：检查 CHANGELOG.md 是否包含当前版本号
- **新增** 隔离运行时上下文：使用 isolated_release_runtime 确保测试环境的纯净性
- **增强** 报告：输出 intermediate/logs/release_gate_report.md，整体PASS/FAIL

```mermaid
sequenceDiagram
participant Gate as "release_gate.py"
participant IR as "isolated_release_runtime"
participant FS as "文件系统"
participant Py as "Python/CLI"
participant Cat as "tool_catalog.json"
participant Doc as "docs/tool-reference.md"
Gate->>IR : 创建隔离运行时上下文
IR-->>Gate : 返回隔离状态路径
Gate->>FS : 检查必需目录/文件
Gate->>Cat : 校验公开表面与指纹
Gate->>Doc : 校验工具参考文档新鲜度
Gate->>Py : 运行公共命令契约测试
Gate->>FS : 校验manifest.runtime.json
Gate->>IR : 清理隔离环境和上下文
Gate-->>Gate : 汇总结果并写报告
```

**图表来源**
- [scripts/release_gate.py:29-105](file://scripts/release_gate.py#L29-L105)
- [scripts/release_gate.py:521-696](file://scripts/release_gate.py#L521-L696)
- [scripts/release_runtime.py:49-77](file://scripts/release_runtime.py#L49-L77)

**章节来源**
- [scripts/release_gate.py:1-701](file://scripts/release_gate.py#L1-L701)
- [scripts/generate_tool_reference.py:68-135](file://scripts/generate_tool_reference.py#L68-L135)
- [rdc_tool/python_runtime.py:104-172](file://rdc_tool/python_runtime.py#L104-L172)
- [scripts/release_runtime.py:1-77](file://scripts/release_runtime.py#L1-L77)

### 发布包验证（scripts/verify_release_package.py）
- **增强** 前置检查：禁止预GA载荷与MCP公开入口泄露
- **增强** 解压定位：找到包含 bin/rdc-tool.cmd 的包根目录
- **增强** 清单校验：RELEASE_MANIFEST.json的public_commands与entrypoints必须匹配预期
- **增强** 许可证与SBOM：LICENSE_INVENTORY.json与SBOM.json必须存在且包含Apache-2.0
- **增强** 身份验证：
  - **新增** LICENSE字节比较：确保LICENSE文件内容与源代码完全一致
  - **新增** 多文件版本一致性检查：验证RELEASE_MANIFEST.json、SBOM.json、LICENSE_INVENTORY.json、pyproject.toml、METADATA文件中的版本号都一致
  - **新增** 变更日志验证：检查CHANGELOG.md包含当前版本号
  - **新增** 文件完整性验证：验证manifest中列出的文件大小和sha256与实际文件匹配
- **增强** 公共命令契约：doctor/version/tools list/context/vfs等命令成功执行
- **增强** 负向契约：期望的错误码与result_kind正确
- **增强** 隔离运行时：使用 isolated_release_runtime 确保验证环境的纯净性
- **增强** 结果：输出PASS/FAIL

```mermaid
flowchart TD
Start(["开始"]) --> CheckPre["前置检查"]
CheckPre --> Extract["解压包"]
Extract --> FindRoot["查找包根目录"]
FindRoot --> VerifyManifest["验证清单"]
VerifyManifest --> VerifyLicense["验证许可证清单"]
VerifyLicense --> VerifyIdentity["验证发布身份"]
VerifyIdentity --> IsolatedRun["隔离运行时验证"]
IsolatedRun --> Doctor["医生检查"]
Doctor --> Launcher["启动器检查"]
Launcher --> Version["版本检查"]
Version --> Tools["工具检查"]
Tools --> Contracts["契约检查"]
Contracts --> Cleanup["清理临时环境"]
Cleanup --> End(["结束"])
```

**图表来源**
- [scripts/verify_release_package.py:351-385](file://scripts/verify_release_package.py#L351-L385)
- [scripts/verify_release_package.py:158-188](file://scripts/verify_release_package.py#L158-L188)

**章节来源**
- [scripts/verify_release_package.py:1-390](file://scripts/verify_release_package.py#L1-L390)

### Windows平台特殊处理
- 启动器：bin/rdc-tool.cmd 调用 PowerShell 脚本执行实际逻辑，支持 --non-interactive
- 环境变量：PATH与PATHEXT注入，确保bat与powershell可被正确调用
- 发布门禁与验证中均通过cmd.exe调用bin/rdc-tool.cmd进行物理启动器测试

**章节来源**
- [bin/rdc-tool.cmd:1-18](file://bin/rdc-tool.cmd#L1-L18)
- [scripts/release_gate.py:117-173](file://scripts/release_gate.py#L117-L173)
- [scripts/verify_release_package.py:67-107](file://scripts/verify_release_package.py#L67-L107)

### 新增：隔离运行时上下文（scripts/release_runtime.py）
**新增** 隔离运行时上下文管理，确保发布验证过程的纯净性和安全性：
- **临时状态管理**：使用 tempfile.mkdtemp 创建临时目录作为隔离状态根
- **环境变量隔离**：设置 RDC_TOOL_INTERMEDIATE_ROOT 和 RDC_TOOL_ROOT 环境变量
- **上下文清理**：自动检测并清理所有创建的上下文和守护进程
- **异常处理**：确保即使发生异常也能正确清理资源
- **环境恢复**：在finally块中恢复原始环境变量

**章节来源**
- [scripts/release_runtime.py:1-77](file://scripts/release_runtime.py#L1-L77)

### 新增：身份验证（scripts/check_identity.py）
**新增** 身份验证脚本确保代码库中不包含废弃的rdx-tools相关标识：
- 检查README.md、AGENTS.md和docs目录下的所有markdown文件
- 检测废弃的身份引用模式（rdx-tools、`rdx`、bin/rdx、rdx_install）
- 检查是否存在废弃的入口文件（rdx、bin/rdx、bin/rdx.cmd、scripts/rdx_install.ps1）
- 输出详细的失败信息和统计结果

**章节来源**
- [scripts/check_identity.py:1-29](file://scripts/check_identity.py#L1-L29)

### 新增：Markdown健康检查（scripts/check_markdown_health.py）
**新增** Markdown健康检查脚本确保文档质量和一致性：
- UTF-8 BOM编码检查
- 可疑字符模式检测（mojibake片段）
- 本地链接完整性验证
- 必需文档存在性检查
- 导航链接完整性验证
- 工具计数一致性检查
- 会话工具引用检查
- 远程生命周期语义检查
- Agent自测指导检查

**章节来源**
- [scripts/check_markdown_health.py:1-243](file://scripts/check_markdown_health.py#L1-L243)

### 新增：契约验证（spec/validate_catalog.py）
**新增** 契约验证脚本确保tool_catalog.json的正确性：
- 工具计数一致性检查
- 工具名称唯一性验证
- 工具名称前缀验证（必须以rd.开头）
- 兼容性别名移除检查
- 可读性验证（无占位符文本、无乱码）
- Schema验证（前置条件、投影支持等）
- 源路径格式验证

**章节来源**
- [spec/validate_catalog.py:1-162](file://spec/validate_catalog.py#L1-L162)

## 依赖关系分析
- 构建依赖：setuptools、wheel（由pyproject.toml指定）
- 运行依赖：aiofiles、jinja2、numpy、Pillow、pydantic（由pyproject.toml指定）
- **增强** CI依赖：Python 3.14、pytest、开发依赖包
- 捆绑site-packages过滤：依据 rdc_tool/runtime_requirements.py 的白名单/黑名单，避免测试与开发工具进入发布包
- 契约与文档：spec/tool_catalog.json 与 docs/tool-reference.md 需保持同步，由 generate_tool_reference.py 生成

```mermaid
graph LR
A["pyproject.toml"] --> B["依赖安装"]
B --> C["rdc_tool/ 运行时"]
C --> D["scripts/* 构建脚本"]
D --> E["binaries/windows/x64/ 运行时产物"]
E --> F["dist/ 发布包"]
G["spec/tool_catalog.json"] --> H["docs/tool-reference.md"]
H --> D
I[".github/workflows/ci.yml"] --> J["CI环境"]
J --> K["契约验证"]
J --> L["工具参考生成"]
J --> M["单元测试执行"]
J --> N["Markdown健康检查"]
J --> O["身份验证"]
```

**图表来源**
- [pyproject.toml:13-27](file://pyproject.toml#L13-L27)
- [rdc_tool/runtime_requirements.py:15-42](file://rdc_tool/runtime_requirements.py#L15-L42)
- [scripts/generate_tool_reference.py:68-135](file://scripts/generate_tool_reference.py#L68-L135)
- [.github/workflows/ci.yml:12-26](file://.github/workflows/ci.yml#L12-L26)

**章节来源**
- [pyproject.toml:1-45](file://pyproject.toml#L1-L45)
- [rdc_tool/runtime_requirements.py:1-73](file://rdc_tool/runtime_requirements.py#L1-L73)
- [spec/tool_catalog.json:1-200](file://spec/tool_catalog.json#L1-L200)

## 性能考虑
- 打包阶段：
  - 仅复制受控文件，跳过测试与缓存，减少zip体积与构建时间
  - 使用ZIP_DEFLATED压缩级别9，平衡体积与速度
  - **增强** 临时暂存环境：使用内存文件系统减少磁盘IO开销
- 校验阶段：
  - 使用流式sha256计算，避免大文件内存占用
  - 并行化建议：可在CI中将release_gate与verify_release_package作为独立任务并行执行
- **增强** CI优化：
  - 使用concurrency组避免重复构建
  - 仅运行必要的测试集（unit or contract）
  - 缓存Python依赖以提高构建速度
- 运行时：
  - 捆绑Python的stdlib与site-packages按需裁剪，减少加载开销
  - **增强** 隔离运行时：确保每次验证都在干净的Python环境中执行

## 故障排查指南
- 版本不匹配：当--version与包内TOOL_VERSION不一致时，release打包会提前退出
- 缺少manifest：release_gate会检查manifest.runtime.json是否存在且有效，否则失败
- Python环境异常：package_runtime在探测python.exe或版本失败时会抛出明确错误
- 契约失败：release_gate与verify_release_package对公共命令的期望返回值与result_kind进行严格校验，失败时输出详细上下文
- 许可证与SBOM：verify_release_package要求LICENSE_INVENTORY.json与SBOM.json存在且包含Apache-2.0
- **增强** 身份验证失败：
  - **新增** LICENSE字节不匹配：检查LICENSE文件内容是否与源代码完全一致
  - **新增** 版本不一致：检查多个文件中的版本号是否一致
  - **新增** 变更日志缺失：确认CHANGELOG.md包含当前版本号
- **增强** CI故障排查：
  - 契约验证失败：检查tool_catalog.json的schema和可读性
  - 工具参考过时：重新生成工具参考文档
  - Markdown健康检查失败：检查文档编码、链接和必需内容
  - 身份验证失败：清理废弃的rdx-tools相关标识
  - 冒烟测试失败：检查Windows环境下CLI的基本功能
- **增强** 隔离运行时问题：
  - **新增** 上下文清理失败：检查是否有残留的守护进程或上下文
  - **新增** 环境变量污染：确认RDC_TOOL_*环境变量正确设置和清理

**章节来源**
- [scripts/package_release.py:169-177](file://scripts/package_release.py#L169-L177)
- [scripts/release_gate.py:255-283](file://scripts/release_gate.py#L255-L283)
- [scripts/package_runtime.py:112-126](file://scripts/package_runtime.py#L112-L126)
- [scripts/verify_release_package.py:158-188](file://scripts/verify_release_package.py#L158-L188)
- [.github/workflows/ci.yml:12-45](file://.github/workflows/ci.yml#L12-L45)
- [scripts/release_runtime.py:17-47](file://scripts/release_runtime.py#L17-L47)

## 结论
RDC-Tool的构建与发布体系围绕"受控复制、清单校验、契约测试、SBOM生成"展开，确保发布包的可追溯性与稳定性。**增强的发布门禁系统**通过集成隔离的运行时上下文，确保了测试环境的纯净性和可靠性。**改进的打包过程**使用临时暂存环境防止文件冲突，而**全面的身份验证机制**通过LICENSE字节比较和多文件版本一致性检查，进一步增强了发布的安全性。配合**增强的CI工作流**，通过契约验证、工具参考生成、单元测试、Markdown健康检查和身份验证等多重保障，可以在提交和合并请求时自动识别问题。通过release_gate与verify_release_package的双重保障，结合CI的自动化检查，可以实现安全、可回滚、可审计的发布过程。遵循本文档的流程与检查清单，配合CI工作流的自动化验证，可以显著提高软件质量和发布效率。

## 附录

### 构建与发布步骤（推荐顺序）
1. **增强** CI验证：
   - 提交代码到main分支或创建pull request
   - 等待CI工作流自动执行契约验证、工具参考生成、单元测试、Markdown健康检查和身份验证
   - 解决CI报告中发现的问题
2. 准备Runtime：
   - 执行 package_runtime.py，提供渲染器源与Python home，生成 binaries/windows/x64/ 与 manifest.runtime.json
3. 构建Release包：
   - 执行 package_release.py，生成 dist/ 下的zip与SHA256SUMS
4. 运行发布门禁：
   - 执行 release_gate.py，查看 intermediate/logs/release_gate_report.md
5. 验证发布包：
   - 执行 verify_release_package.py，确认PASS
6. 生成额外校验和（可选）：
   - 执行 generate_release_checksums.py，输出 logs/release_checksums.sha256

**章节来源**
- [.github/workflows/ci.yml:12-45](file://.github/workflows/ci.yml#L12-L45)
- [scripts/package_runtime.py:289-353](file://scripts/package_runtime.py#L289-L353)
- [scripts/package_release.py:169-213](file://scripts/package_release.py#L169-L213)
- [scripts/release_gate.py:521-696](file://scripts/release_gate.py#L521-L696)
- [scripts/verify_release_package.py:351-385](file://scripts/verify_release_package.py#L351-L385)
- [scripts/generate_release_checksums.py:29-50](file://scripts/generate_release_checksums.py#L29-L50)

### 版本管理策略
- 版本号来源：rdc_tool/__init__.py 的 __version__
- 版本传播：package_release.py 读取 TOOL_VERSION 并用于产物命名与元数据
- **增强** 变更日志：CHANGELOG.md 为必需文件，release_gate会检查其存在并验证包含当前版本号
- **增强** 版本一致性：verify_release_package.py 验证多个文件中的版本号一致性
- 建议：每次发布前更新 __version__ 与 CHANGELOG.md，并确保工具参考文档由 generate_tool_reference.py 生成

**章节来源**
- [rdc_tool/__init__.py:1-4](file://rdc_tool/__init__.py#L1-L4)
- [scripts/package_release.py:19-21](file://scripts/package_release.py#L19-L21)
- [scripts/release_gate.py:543-545](file://scripts/release_gate.py#L543-L545)
- [scripts/verify_release_package.py:158-180](file://scripts/verify_release_package.py#L158-L180)
- [scripts/generate_tool_reference.py:68-135](file://scripts/generate_tool_reference.py#L68-L135)

### 发布前检查清单
- 结构：必需目录与文件存在
- 契约：tool_catalog.json与代码定义一致，工具参考文档新鲜
- 公共命令：rdc-tool --help/--version/tools list/context status/vfs ls 等正常
- 运行时：manifest.runtime.json完整，捆绑Python布局有效
- 文档：用户文档不包含pip/uv/venv引导命令，不使用bin/rdc-tool.cmd示例命令
- 安全：无调试框架引用，无MCP公开入口泄露
- **增强** CI检查：
  - 契约验证通过（validate_catalog.py）
  - 工具参考文档最新（generate_tool_reference.py --check）
  - 单元测试和契约测试通过（pytest -m "unit or contract"）
  - Markdown健康检查通过（check_markdown_health.py）
  - 身份验证通过（check_identity.py）
- **新增** 变更日志验证：CHANGELOG.md包含当前版本号
- **新增** 身份验证：LICENSE字节比较和多文件版本一致性检查

**章节来源**
- [scripts/release_gate.py:29-105](file://scripts/release_gate.py#L29-L105)
- [scripts/release_gate.py:543-652](file://scripts/release_gate.py#L543-L652)
- [.github/workflows/ci.yml:12-26](file://.github/workflows/ci.yml#L12-L26)
- [scripts/verify_release_package.py:158-188](file://scripts/verify_release_package.py#L158-L188)

### 二进制打包与签名（Windows）
- 打包：package_runtime.py 复制允许的二进制与JSON，生成manifest.runtime.json
- 签名：当前脚本未内置签名步骤；建议在CI中于生成zip后对二进制与zip进行签名，并将签名文件纳入发布资产
- 启动器：bin/rdc-tool.cmd 通过PowerShell调用实际逻辑，支持非交互模式

**章节来源**
- [scripts/package_runtime.py:24-42](file://scripts/package_runtime.py#L24-L42)
- [bin/rdc-tool.cmd:1-18](file://bin/rdc-tool.cmd#L1-L18)

### 回滚策略与发布后验证
- 回滚策略：
  - 保留上一版本的发布包与校验和，便于快速回滚
  - 若发现严重问题，撤销发布并重新执行构建与验证流程
- **增强** 发布后验证：
  - 在目标环境执行 rdc-tool --json doctor 与关键工具命令
  - 核对版本与入口点是否与发布包一致
  - 检查日志与上下文快照是否正常生成
  - **新增** 身份验证：验证LICENSE文件和版本信息的一致性

**章节来源**
- [scripts/verify_release_package.py:158-188](file://scripts/verify_release_package.py#L158-L188)

### 第三方依赖许可证合规与通知生成
- 许可证清单：package_release.py 扫描 site-packages 的 METADATA，生成 LICENSE_INVENTORY.json
- SBOM：同时生成 SBOM.json，包含组件名称、版本与许可证
- **增强** 合规检查：verify_release_package.py 验证SBOM与许可证清单的存在与正确性，包括Apache-2.0许可证验证
- 通知：可将 LICENSE_INVENTORY.json 与 SBOM.json 作为发布附件，供法务与合规团队审阅

**章节来源**
- [scripts/package_release.py:111-159](file://scripts/package_release.py#L111-L159)
- [scripts/verify_release_package.py:202-224](file://scripts/verify_release_package.py#L202-L224)

### GitHub Actions CI工作流详解
**增强** CI工作流提供了完整的质量保证流程：

#### 工作流配置
- **触发条件**：push到main分支或创建pull request
- **并发控制**：使用ci-${{ github.workflow }}-${{ github.ref }}组避免重复构建
- **权限设置**：仅授予contents: read权限

#### 契约验证作业
- **Python环境**：使用Python 3.14
- **依赖安装**：安装开发依赖（pip install -e ".[dev]"）
- **验证步骤**：
  1. 契约验证：python spec/validate_catalog.py
  2. 工具参考检查：python scripts/generate_tool_reference.py --check
  3. 单元测试：python -m pytest tests -q -m "unit or contract"
  4. Markdown健康检查：python scripts/check_markdown_health.py
  5. 身份验证：python scripts/check_identity.py

#### 冒烟测试作业
- **运行环境**：Windows latest
- **测试内容**：
  - CLI版本检查：.\bin\rdc-tool.cmd --version
  - 医生命令检查：.\bin\rdc-tool.cmd --json doctor
  - 响应验证：检查result_kind字段和退出码

**章节来源**
- [.github/workflows/ci.yml:1-45](file://.github/workflows/ci.yml#L1-L45)

### 隔离运行时上下文详解
**新增** 隔离运行时上下文管理，确保发布验证过程的纯净性和安全性：

#### 临时状态管理
- 使用 tempfile.mkdtemp 创建临时目录作为隔离状态根
- 设置 RDC_TOOL_INTERMEDIATE_ROOT 环境变量指向临时目录
- 设置 RDC_TOOL_ROOT 环境变量指向工具根目录

#### 上下文清理机制
- 自动检测所有创建的上下文和守护进程
- 执行 context clear 和 daemon stop 命令清理资源
- 确保即使发生异常也能正确清理所有资源

#### 异常处理与环境恢复
- 在finally块中恢复原始环境变量
- 保留临时状态以便故障诊断
- 提供详细的清理失败信息

**章节来源**
- [scripts/release_runtime.py:17-77](file://scripts/release_runtime.py#L17-L77)

### 发布身份验证详解
**增强** 发布身份验证机制，确保发布包的完整性和一致性：

#### LICENSE字节比较
- 直接比较LICENSE文件的字节内容，而非仅检查文件名
- 防止LICENSE文件被替换或篡改
- 确保发布包中的LICENSE与源代码完全一致

#### 多文件版本一致性检查
- 验证RELEASE_MANIFEST.json、SBOM.json、LICENSE_INVENTORY.json中的版本号
- 检查pyproject.toml中的项目版本
- 验证bundled distribution的METADATA文件版本
- 确保所有版本信息保持一致

#### 变更日志验证
- 检查CHANGELOG.md是否包含当前版本号
- 验证格式符合预期（## {version}）
- 确保发布历史记录的完整性

#### 文件完整性验证
- 验证manifest中列出的文件大小和sha256与实际文件匹配
- 检查文件路径的有效性和可达性
- 确保发布包内容的完整性

**章节来源**
- [scripts/verify_release_package.py:158-188](file://scripts/verify_release_package.py#L158-L188)