...
## 更新摘要
**变更内容**
- 更新了开发环境设置部分，反映从 `rdx` 命名空间到 `rdc_tool` 命名空间的迁移
- 更新了所有导入语句和模块引用，现在统一使用 `rdc_tool` 命名空间
- 更新了构建系统和发布流程中的包结构说明
- 更新了测试配置和环境变量设置
- 保持了原有的文档结构和一致性
...
## 简介
本指南面向希望理解并扩展 RDC-Tool（rdc-tools）的开发者。内容涵盖如何添加新操作、实现处理器、编写测试（单元、集成、冒烟）、Fixture 策略、代码规范与最佳实践、构建与发布流程、调试技巧、常见问题与贡献流程，以及完整的开发环境搭建说明。

RDC-Tool 是一个以 JSON 为契约的 CLI 工具集，通过守护进程驱动本地或远程回放能力，暴露统一的 rd.* 操作接口，支持工具发现、上下文管理、捕获/回放、资源与事件浏览、着色器调试、导出与对比等能力。
...
## 项目结构
仓库采用"功能域 + 运行时"分层组织：
- rdc_tool：核心运行时、CLI 适配、守护进程客户端、工具路由、配置与路径、运行时状态等
...
```mermaid
graph TB
A["CLI 入口<br/>rdc_tool/cli.py"] --> B["守护进程客户端<br/>daemon/client.py"]
...
**章节来源**
- [rdc_tool/operation_definitions.py:1-120](file://rdc_tool/operation_definitions.py#L1-L120)
- [rdc_tool/core/operation_registry.py:1-45](file://rdc_tool/core/operation_registry.py#L1-L45)
- [rdc_tool/cli.py:1-120](file://rdc_tool/cli.py#L1-L120)
- [scripts/smoke_cli.sh:1-120](file://scripts/smoke_cli.sh#L1-L120)
- [scripts/package_release.py:1-120](file://scripts/package_release.py#L1-L120)

## 架构总览
RDC-Tool 采用"CLI -> 守护进程 -> 操作处理器 -> 核心服务/后端"的分层架构。CLI 仅负责参数解析、协议封装与结果渲染；具体业务逻辑在守护进程中由操作处理器完成，并通过核心服务访问 RenderDoc 或其他后端。
...
U->>CLI : 调用 rdc <command> [--json/--format]
...
**图表来源**
- [rdc_tool/cli.py:230-330](file://rdc_tool/cli.py#L230-L330)
- [rdc_tool/operation_definitions.py:200-300](file://rdc_tool/operation_definitions.py#L200-L300)
...
**图表来源**
- [rdc_tool/cli.py:410-560](file://rdc_tool/cli.py#L410-L560)
- [rdc_tool/cli.py:230-395](file://rdc_tool/cli.py#L230-L395)
...
- 包构建
  ...
  - 压缩为 rdc-tool-<version>-windows-x64.zip，并输出 SHA256SUMS 与 release_report.md。
- 版本与平台
...
Install["rdc_tool_install.ps1"] --> User["用户安装/升级/卸载"]
```
...
- [rdc_tool/cli.py:267-330](file://rdc_tool/cli.py#L267-L330)
- [rdc_tool/cli.py:330-395](file://rdc_tool/cli.py#L330-L395)

## 结论
RDC-Tool 通过清晰的 JSON 契约、统一的 CLI 与守护进程架构、完善的冒烟与打包流程，提供了稳定可扩展的 RenderDoc 工具运行时。开发者应遵循操作定义规范、处理器实现约定与测试分层策略，配合 Fixture 与隔离机制，持续增强工具能力并保持向后兼容。
...
### 开发环境搭建
**已更新** 开发环境设置现已完全迁移到新的 `rdc_tool` 包结构

- 基础要求
  - Python >= 3.11，建议使用仓库提供的自包含包或本地虚拟环境。
  - Windows x64 环境下可直接使用二进制包；非 Windows 环境需自行准备 RenderDoc 绑定。
- 包结构与导入
  - 所有模块现在使用 `rdc_tool` 命名空间而非之前的 `rdx` 命名空间
  - 主要入口点：`rdc_tool.cli:main` 作为命令行工具入口
  - 包配置：`pyproject.toml` 中定义了 `rdc_tool` 包的发现规则
- 安装与验证
  - 解压包后运行安装脚本进行安装/升级/卸载/doctor。
  - 使用 `rdc-tool --json doctor` 验证环境与依赖。
- 运行冒烟
  - 使用 `bash scripts/smoke_cli.sh` 进行基础冒烟；如需捕获链路，传入 `--rdc` 指定捕获路径。
- 环境变量配置
  - `RDC_TOOL_ROOT`：工具根目录，默认为脚本所在目录的父目录
  - `RDC_TOOL_INTERMEDIATE_ROOT`：中间输出目录，用于测试和冒烟结果
  - `RDC_TOOL_ARTIFACT_DIR`：工件存储目录
  - `RDC_TOOL_RENDERDOC_PATH`：RenderDoc Python 模块路径
  - `RDC_TOOL_RUNTIME_DLL_DIR`：运行时 DLL 目录
...
- [cli/run_cli.py:16-44](file://cli/run_cli.py#L16-L44)
...
**章节来源**
- [rdc_tool/operation_definitions.py:324-424](file://rdc_tool/operation_definitions.py#L324-L424)
...
### 命名空间迁移指南
**新增** 从 `rdx` 到 `rdc_tool` 的迁移说明

- 包结构变更
  - 主包名从 `rdx` 更改为 `rdc_tool`
  - 所有导入语句需要从 `import rdx` 或 `from rdx` 更新为 `import rdc_tool` 或 `from rdc_tool`
  - 包入口点从 `rdx.cli:main` 更新为 `rdc_tool.cli:main`
- 配置文件更新
  - `pyproject.toml` 中的包发现规则已更新为包含 `rdc_tool*`
  - 脚本入口点配置已更新为 `rdc-tool = "rdc_tool.cli:main"`
- 测试环境配置
  - 测试夹具中的导入语句已更新为使用 `rdc_tool` 命名空间
  - 环境变量配置保持不变，仍使用 `RDC_TOOL_*` 前缀
- 开发工作流
  - 所有开发工具和脚本现在都基于新的 `rdc_tool` 包结构
  - 冒烟测试和发布流程已适配新的包结构

**章节来源**
- [pyproject.toml:21-34](file://pyproject.toml#L21-L34)
- [tests/conftest.py:19-21](file://tests/conftest.py#L19-L21)
- [cli/run_cli.py:44-46](file://cli/run_cli.py#L44-L46)