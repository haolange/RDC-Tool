...
## 简介
本指南面向希望在本地快速搭建 RDC-Tool（rdx-tools）开发环境的开发者。项目提供自包含的 Windows x64 发布包，默认无需安装系统 Python；同时支持在源码环境下进行开发与测试。文档涵盖：
...
H["安装脚本<br/>rdx_install.ps1"] --> I["目标目录<br/>LOCALAPPDATA\\Programs\\rdx-tools"]
I --> B
```

图表来源
- [rdx/cli.py:412-513](file://rdx/cli.py#L412-L513)
- [rdx/runtime_bootstrap.py:105-130](file://rdx/runtime_bootstrap.py#L105-L130)
- [scripts/rdx_install.ps1:141-167](file://scripts/rdx_install.ps1#L141-L167)
...
## 核心组件
- CLI 入口与健康检查：通过 rdx 命令暴露操作，doctor 子命令用于环境自检（Python、RenderDoc 布局、工具链、守护进程状态）。
...
- [rdx/runtime_requirements.py:7-13](file://rdx/runtime_requirements.py#L7-L13)
- [rdx/cli.py:452-489](file://rdx/cli.py#L452-L489)
...
步骤要点：
- 指定安装目录（默认 LOCALAPPDATA\Programs\rdx-tools）。
...
- 运行时引导会优先使用环境变量 RDX_RUNTIME_DLL_DIR 与 RDX_RENDERDOC_PATH 指定路径；未设置时使用默认位置。
- 在 Windows 上，引导过程会将二进制目录注册为 DLL 搜索目录，避免动态库加载失败。
- Vulkan 层配置文件 renderdoc.json 定义了层名称、库路径、API 版本及启用/禁用环境变量。

章节来源
- [rdx/runtime_bootstrap.py:42-57](file://rdx/runtime_bootstrap.py#L42-L57)
- [rdx/runtime_bootstrap.py:70-89](file://rdx/runtime_bootstrap.py#L70-L89)
...
- 常用环境变量：
  - RDX_RENDERDOC_PATH：指定 RenderDoc Python 模块路径
  - RDX_RUNTIME_DLL_DIR：指定运行时 DLL 目录
  - RDX_ARTIFACT_STORE / RDX_DATA_DIR / RDX_REPORT_DIR：分别覆盖工件、数据库、报告输出路径
  - RDX_LOG_LEVEL：日志级别
  - RDX_GPU_VENDOR：GPU 厂商过滤
  - RDX_SPIRV_TOOLS_PATH：SPIR-V 工具路径
  - RDX_HEADLESS：无头模式开关
  - 其他 bisect、快照保留、运行时限制相关变量

章节来源
- [rdx/config.py:16-131](file://rdx/config.py#L16-L131)
- [rdx/config.py:135-186](file://rdx/config.py#L135-L186)

### 本地开发工作流程
- 快速开始：从 resources/tools 或使用 bin/rdx 执行示例命令，查看上下文状态、打开捕获、浏览事件与资源。
...
### 调试技巧
- 使用 rdx --json doctor 获取环境诊断信息（工具根、Python 运行时、RenderDoc 布局、目录、守护进程状态）。
- 使用 rdx context status --json 查看当前上下文与会话状态；必要时清理上下文并重新打开捕获。
...
章节来源
- [rdx/config.py:82-131](file://rdx/config.py#L82-L131)

## 故障排查指南
- 首查：执行 rdx --json doctor，检查 Python、RenderDoc 布局、工具链与守护进程状态。
...
- 路径配置：通过 RDX_RUNTIME_DLL_DIR 与 RDX_RENDERDOC_PATH 显式指定路径，避免默认路径冲突。
