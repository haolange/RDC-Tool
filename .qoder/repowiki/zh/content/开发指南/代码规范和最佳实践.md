...
## 简介
本指南面向 RDC-Tool（rdx-tools）项目的 Python 代码质量与一致性，覆盖编码规范、命名约定、格式与注释标准；阐述分层架构、模块组织与依赖管理；定义错误处理、日志记录与异常类型；提供性能优化建议（内存、I/O、并发）；给出代码审查检查清单；说明重构原则与向后兼容性保证；总结常见反模式与避免方法；并明确文档编写标准与工具使用。

## 项目结构
- CLI 入口与命令路由：通过 rdx 命令暴露 JSON-first 操作，CLI 负责参数解析、输出格式化、与守护进程通信。
...
```mermaid
graph TB
CLI["CLI 入口<br/>rdx/cli.py"] --> Server["运行时调度<br/>rdx/server.py"]
Server --> Router["操作路由<br/>rdx/tool_router.py"]
Router --> Handlers["领域处理器<br/>rdx/handlers/*"]
Server --> Contracts["统一契约<br/>rdx/core/contracts.py"]
Server --> Errors["错误映射<br/>rdx/core/errors.py"]
CLI --> IO["安全I/O<br/>rdx/io_utils.py"]
CLI --> Config["配置与环境<br/>rdx/config.py"]
```
...
- [rdx/io_utils.py:19-161](file://rdx/io_utils.py#L19-L161)
...
- [rdx/tool_router.py:34-154](file://rdx/tool_router.py#L34-L154)
- [rdx/config.py:15-186](file://rdx/config.py#L15-L186)
- [rdx/core/contracts.py:98-164](file://rdx/core/contracts.py#L98-L164)
- [rdx/io_utils.py:19-161](file://rdx/io_utils.py#L19-L161)
- [rdx/core/errors.py:9-119](file://rdx/core/errors.py#L9-L119)
...
## 架构总览
RDX 采用“CLI -> 守护进程 -> 运行时 -> 操作引擎 -> 领域处理器”的分层模型。CLI 仅关注用户交互与输出；运行时负责上下文、进度、预览同步与生命周期；操作引擎根据 catalog 路由到具体 handler；handler 调用底层能力并返回统一契约载荷。

```mermaid
sequenceDiagram
participant U as "用户"
participant C as "CLI<br/>rdx/cli.py"
participant S as "运行时调度<br/>rdx/server.py"
participant R as "操作路由<br/>rdx/tool_router.py"
participant H as "领域处理器<br/>rdx/handlers/*"
...
图表来源
- [rdx/cli.py:188-221](file://rdx/cli.py#L188-L221)
- [rdx/cli.py:243-264](file://rdx/cli.py#L243-L264)
...
- 环境变量：from_env 支持大量 RDX_* 变量覆盖默认值，便于 CI/CD 与部署定制。
- 建议：新增配置项应遵循现有命名与分组，保持默认值合理且可覆盖。

章节来源
- [rdx/config.py:15-186](file://rdx/config.py#L15-L186)

### 统一契约与制品（rdx/core/contracts.py）
...
章节来源
- [rdx/core/contracts.py:98-164](file://rdx/core/contracts.py#L98-L164)
- [rdx/core/contracts.py:46-95](file://rdx/core/contracts.py#L46-L95)
- [rdx/core/contracts.py:224-240](file://rdx/core/contracts.py#L224-L240)

### I/O 安全（rdx/io_utils.py）
...
## 依赖关系分析
- 包元数据：pyproject.toml 声明依赖 aiofiles、jinja2、numpy、Pillow、pydantic；entry point rdx=rdx.cli:main；pytest markers 区分 unit/contract/fixture_integration/gpu_live。
- 模块耦合：
...
```mermaid
graph LR
CLI["rdx/cli.py"] --> DAEMON["rdx.daemon.client"]
CLI --> PATHS["rdx.runtime_paths"]
CLI --> CAT["rdx.runtime_catalog"]
CLI --> CT["rdx.core.contracts"]
CLI --> IO["rdx.io_utils"]
SERVER["rdx/server.py"] --> ROUTER["rdx/tool_router.py"]
ROUTER --> HANDLERS["rdx/handlers/*"]
CONFIG["rdx/config.py"] --> PATHS
```
...
- [rdx/config.py:12-13](file://rdx/config.py#L12-L13)
...
章节来源
- [rdx/core/contracts.py:31-39](file://rdx/core/contracts.py#L31-L39)
- [rdx/io_utils.py:67-161](file://rdx/io_utils.py#L67-L161)
- [rdx/server.py:84-145](file://rdx/server.py#L84-L145)
...
- [rdx/server.py:115-141](file://rdx/server.py#L115-L141)
