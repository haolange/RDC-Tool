...
## 核心组件
- 冒烟测试脚本 smoke_cli.sh
  - 通过 bin/rdc-tool 执行 doctor、tools list/search、capture open/status、vfs ls/tree 等关键路径。
...
- 发布门禁 release_gate.py
  ...
  - 调用 public 入口 rdc 和 bin/rdc-tool.cmd 执行一组正向/负向用例，确保 CLI 契约稳定。
...
- 发布包校验 verify_release_package.py
  ...
  - 在隔离环境中运行 rdc 的 doctor/version/tools list/context/vfs 等契约用例，并检查负面契约。
...
- 测试框架 pyproject.toml 与 tests/conftest.py
  ...
  - conftest 设置 RDC_TOOL_INTERMEDIATE_ROOT、清理运行时状态，保证测试隔离。
...
- 功能要点
  ...
  - 定位 RDC 启动器 bin/rdc-tool，准备 intermediate 日志与 findings。
...
```mermaid
flowchart TD
Start(["开始"]) --> Parse["解析参数<br/>--tools-root/--rdc_tool/--context/--timeout"]
Parse --> CheckRDC{"bin/rdc-tool 存在?"}
CheckRDC --> |否| ExitErr["退出并记录错误"]
CheckRDC --> |是| InitLogs["初始化日志与 findings"]
...
- 功能要点
  - 结构检查：必需目录/文件存在性。
  - 代码扫描：禁止引用扩展路径、调试框架术语；用户文档禁止 venv/pip 引导与 bin/rdc-tool.cmd 命令示例。
  - 公开表面：catalog 名称集合与指纹一致；工具参考文档与代码定义一致；不暴露 MCP 公开入口。
  - 清单与运行时：manifest.runtime.json 完整性校验；捆绑 Python 布局校验。
  - 入口契约：rdc-tool --help/--version/--json doctor/tools list/context status/list/update/clear、vfs ls、bin/rdc-tool.cmd 非交互 doctor。
...
participant CLI as "rdc_tool/bin/rdc-tool.cmd"
...
- 功能要点
  ...
  - 在隔离环境执行 rdc 契约用例（doctor/version/tools list/context/vfs），并断言负面契约。
  - 扫描预 GA 标记与 MCP 暴露面，防止未发布内容进入发布包。
- 关键流程
...
- 测试隔离
  - 通过 RDC_TOOL_INTERMEDIATE_ROOT 指定隔离中间目录。
...
## 依赖关系分析
- 冒烟脚本依赖 bin/rdc-tool 与可选 /usr/bin/timeout。
- 发布门禁依赖：
...
- 发布包校验依赖 zip 包内的 RELEASE_MANIFEST.json、LICENSE_INVENTORY.json、SBOM.json 及 rdc_tool/bin/rdc-tool.cmd。
- 共享工具被门禁与校验脚本共同使用，降低重复实现。

```mermaid
graph LR
Smoke["smoke_cli.sh"] --> RDC["bin/rdc-tool"]
...
- 资源隔离
  - 每个任务使用独立的 RDC_TOOL_INTERMEDIATE_ROOT，避免状态冲突。
...
- 执行方式
  - 在 pytest 命令中加入 --cov=rdc-tool --cov-report=html --cov-report=xml。
