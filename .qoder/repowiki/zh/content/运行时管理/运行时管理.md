# 运行时管理

<cite>
**本文引用的文件**
- [runtime_paths.py](file://rdc_tool/runtime_paths.py)
- [python_runtime.py](file://rdc_tool/python_runtime.py)
- [config.py](file://rdc_tool/config.py)
- [timeout_policy.py](file://rdc_tool/timeout_policy.py)
- [runtime_requirements.py](file://rdc_tool/runtime_requirements.py)
- [runtime_bootstrap.py](file://rdc_tool/runtime_bootstrap.py)
- [server_runtime.py](file://rdc_tool/server_runtime.py)
- [runtime_worker.py](file://rdc_tool/runtime_worker.py)
- [runtime_state.py](file://rdc_tool/runtime_state.py)
- [runtime_catalog.py](file://rdc_tool/runtime_catalog.py)
- [operation_definitions.py](file://rdc_tool/operation_definitions.py)
- [context_snapshot.py](file://rdc_tool/context_snapshot.py)
- [io_utils.py](file://rdc_tool/io_utils.py)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障诊断指南](#故障诊断指南)
9. [结论](#结论)
10. [附录：配置示例与最佳实践](#附录配置示例与最佳实践)

## 简介
本仓库实现了一个面向 RenderDoc 的“运行时管理系统”，负责在独立工具包中启动、引导并协调 Python 运行时、RenderDoc 原生模块、会话与上下文状态、操作路由、超时策略、依赖检查与路径解析等。其目标是让上层 CLI/服务端/工作进程以统一接口调用图形捕获与回放能力，同时保证环境一致性、资源隔离、可恢复性与可观测性。

## 项目结构
- 运行时路径与布局：集中管理工具根、二进制、Python 运行时、中间产物与日志目录，确保跨平台一致的路径解析与临时文件组织。
- 运行时引导：准备 Python 环境、注册 DLL 搜索路径、注入 sys.path，探测 renderdoc 模块可用性。
- 配置系统：通过 dataclass 定义多类配置项，支持环境变量覆盖与运行时动态调整。
- 依赖检查：声明必需依赖、排除打包列表、检测缺失依赖。
- 超时策略：按操作类型与重量级分组计算超时，提供传输层与执行层超时换算。
- 状态持久化：上下文/会话/捕获句柄的状态序列化、并发安全写入、最近操作摘要与指标统计。
- 工作进程：独立的 worker 进程封装异步事件循环，接收 JSON 指令并调度到 server 的操作分发器。
- 服务与编排：server_runtime 聚合各服务（会话管理、渲染、流水线、性能、事件图、补丁引擎），维护全局运行时状态与上下文快照。

```mermaid
graph TB
A["CLI/调用方"] --> B["worker 进程<br/>runtime_worker.py"]
B --> C["server 运行期<br/>server_runtime.py"]
C --> D["路径与布局<br/>runtime_paths.py / python_runtime.py"]
C --> E["配置与环境<br/>config.py"]
C --> F["依赖检查<br/>runtime_requirements.py"]
C --> G["超时策略<br/>timeout_policy.py"]
C --> H["状态持久化<br/>runtime_state.py / context_snapshot.py"]
C --> I["操作目录与校验<br/>runtime_catalog.py / operation_definitions.py"]
C --> J["IO 原子写/日志行追加<br/>io_utils.py"]
```

**图表来源**
- [runtime_worker.py:65-167](file://rdc_tool/runtime_worker.py#L65-L167)
- [server_runtime.py:1-120](file://rdc_tool/server_runtime.py#L1-L120)
- [runtime_paths.py:14-123](file://rdc_tool/runtime_paths.py#L14-L123)
- [python_runtime.py:28-92](file://rdc_tool/python_runtime.py#L28-L92)
- [config.py:15-186](file://rdc_tool/config.py#L15-L186)
- [runtime_requirements.py:1-73](file://rdc_tool/runtime_requirements.py#L1-L73)
- [timeout_policy.py:1-104](file://rdc_tool/timeout_policy.py#L1-L104)
- [runtime_state.py:41-57](file://rdc_tool/runtime_state.py#L41-L57)
- [runtime_catalog.py:13-91](file://rdc_tool/runtime_catalog.py#L13-L91)
- [io_utils.py:67-161](file://rdc_tool/io_utils.py#L67-L161)

**章节来源**
- [runtime_paths.py:14-123](file://rdc_tool/runtime_paths.py#L14-L123)
- [python_runtime.py:28-92](file://rdc_tool/python_runtime.py#L28-L92)
- [config.py:15-186](file://rdc_tool/config.py#L15-L186)
- [runtime_requirements.py:1-73](file://rdc_tool/runtime_requirements.py#L1-L73)
- [timeout_policy.py:1-104](file://rdc_tool/timeout_policy.py#L1-L104)
- [runtime_worker.py:65-167](file://rdc_tool/runtime_worker.py#L65-L167)
- [server_runtime.py:1-120](file://rdc_tool/server_runtime.py#L1-L120)
- [runtime_state.py:41-57](file://rdc_tool/runtime_state.py#L41-L57)
- [runtime_catalog.py:13-91](file://rdc_tool/runtime_catalog.py#L13-L91)
- [io_utils.py:67-161](file://rdc_tool/io_utils.py#L67-L161)

## 核心组件
- 路径管理系统：统一解析工具根、二进制目录、内置 Python 运行时、中间产物与日志目录，并提供创建与强制设置环境变量的能力。
- 依赖检查系统：声明必需依赖、过滤打包前缀、检测当前环境是否满足要求。
- Python 运行时管理：读取 manifest 描述内置 Python 布局，校验关键文件/目录存在性，返回当前 Python 细节。
- 配置管理系统：集中定义后端、回放、工作进程、工件存储、数据库、二分查找、报告、置信度权重、快照保留、运行时限制等；支持从环境变量加载与运行时动态应用。
- 超时策略：按操作名与重量级前缀选择不同超时，区分连接、打开回放、关闭、重负载请求等场景，并提供传输层与执行层超时换算。
- 状态与快照：上下文/会话/捕获句柄的持久化、并发锁、最近操作裁剪、指标汇总、预览状态归一化。
- 工作进程与服务：worker 进程通过 stdin 接收 JSON 指令，调用 server 的操作分发器执行；server 维护全局运行时状态、服务实例与上下文容量控制。

**章节来源**
- [runtime_paths.py:14-123](file://rdc_tool/runtime_paths.py#L14-L123)
- [runtime_requirements.py:1-73](file://rdc_tool/runtime_requirements.py#L1-L73)
- [python_runtime.py:28-172](file://rdc_tool/python_runtime.py#L28-L172)
- [config.py:15-186](file://rdc_tool/config.py#L15-L186)
- [timeout_policy.py:1-104](file://rdc_tool/timeout_policy.py#L1-L104)
- [runtime_state.py:104-500](file://rdc_tool/runtime_state.py#L104-L500)
- [context_snapshot.py:130-200](file://rdc_tool/context_snapshot.py#L130-L200)
- [runtime_worker.py:65-167](file://rdc_tool/runtime_worker.py#L65-L167)
- [server_runtime.py:196-468](file://rdc_tool/server_runtime.py#L196-L468)

## 架构总览
下图展示了从调用方到 worker、server、子系统的交互流程，以及路径、配置、超时、状态等横切关注点如何贯穿其中。

```mermaid
sequenceDiagram
participant Client as "调用方"
participant Worker as "worker 进程"
participant Server as "server 运行期"
participant Paths as "路径/布局"
participant Config as "配置"
participant Timeout as "超时策略"
participant State as "状态/快照"
participant IO as "IO 原子写"
Client->>Worker : "JSON 指令(method/exec/status/shutdown)"
Worker->>Server : "dispatch_operation(operation, args, transport, remote, context_id)"
Server->>Timeout : "operation_timeout_s / transport_timeout_s"
Server->>Paths : "ensure_runtime_dirs / binaries_root / runtime_root"
Server->>Config : "_apply_runtime_config / _serialize_runtime_config"
Server->>State : "load/save_context_state / append_runtime_log"
Server-->>Worker : "结果(ok/result/error/meta/artifacts)"
Worker-->>Client : "响应(JSON)"
Note over Server,IO : "状态与日志通过原子写入与追加保证一致性"
```

**图表来源**
- [runtime_worker.py:97-161](file://rdc_tool/runtime_worker.py#L97-L161)
- [server_runtime.py:277-390](file://rdc_tool/server_runtime.py#L277-L390)
- [timeout_policy.py:56-104](file://rdc_tool/timeout_policy.py#L56-L104)
- [runtime_paths.py:112-123](file://rdc_tool/runtime_paths.py#L112-L123)
- [runtime_state.py:448-472](file://rdc_tool/runtime_state.py#L448-L472)
- [io_utils.py:138-161](file://rdc_tool/io_utils.py#L138-L161)

## 详细组件分析

### 路径管理系统
- 工具根解析：优先使用环境变量覆盖，否则回退到脚本所在包的父目录；提供强制设置环境变量的方法。
- 二进制与 Python：导出 binaries_root、bundled_python_root、pymodules_dir 等固定布局路径。
- 中间产物与日志：intermediate_root 可由环境变量覆盖，默认位于 tools_root/intermediate；提供 runtime_root、cli_runtime_dir、worker_state_dir、artifacts_dir、pytest_dir、logs_dir 等便捷访问。
- 目录初始化：ensure_runtime_dirs 一次性创建所有必要目录。

```mermaid
flowchart TD
Start(["进入路径解析"]) --> CheckEnv{"RDC_TOOL_ROOT 已设置?"}
CheckEnv --> |是| UseEnv["使用环境变量路径(展开用户目录并解析)"]
CheckEnv --> |否| UseFallback["使用脚本包父目录作为回退"]
UseEnv --> Validate{"是否为有效目录?"}
UseFallback --> Validate
Validate --> |是| ReturnRoot["返回工具根路径"]
Validate --> |否| RaiseErr["抛出运行时错误"]
```

**图表来源**
- [runtime_paths.py:14-57](file://rdc_tool/runtime_paths.py#L14-L57)

**章节来源**
- [runtime_paths.py:14-123](file://rdc_tool/runtime_paths.py#L14-L123)

### 依赖检查系统
- 必需依赖清单：pydantic、numpy、Pillow、jinja2、aiofiles。
- 打包排除：排除测试、开发工具与构建相关包的前缀，避免将无关内容打入运行时 site-packages。
- 可用性与缺失检测：基于 importlib.util.find_spec 判断模块是否存在，收集缺失依赖列表。

```mermaid
flowchart TD
S(["开始"]) --> ForEach["遍历必需依赖"]
ForEach --> Check["尝试 find_spec(import_name)"]
Check --> |找到| Next["下一个依赖"]
Check --> |未找到| AddMissing["加入缺失列表"]
AddMissing --> Next
Next --> Done{"是否全部检查完毕?"}
Done --> |否| ForEach
Done --> |是| Return["返回缺失依赖列表"]
```

**图表来源**
- [runtime_requirements.py:7-57](file://rdc_tool/runtime_requirements.py#L7-L57)

**章节来源**
- [runtime_requirements.py:1-73](file://rdc_tool/runtime_requirements.py#L1-L73)

### Python 运行时的环境管理、模块加载与细节处理
- 内置 Python 布局：从 manifest.runtime.json 读取 bundled_python 元数据，合并默认值，解析出 python.exe、pythonw.exe、DLL、stdlib、site-packages、pth 等路径，并进行安全校验（防止逃逸 binaries_root）。
- 验证：校验必需元数据字段与关键路径/目录存在性；若 stdlib 为 zip 则检查文件，否则检查目录与关键标记文件。
- 当前 Python 详情：返回当前进程的 executable、version、prefix、base_prefix。
- 运行时引导：bootstrap_renderdoc_runtime 设置 RDC_TOOL_RUNTIME_DLL_DIR 与 RDC_TOOL_RENDERDOC_PATH，将 pymodules_dir 插入 sys.path 首位，将 binaries_dir 前置 PATH，并在 Windows 上注册 DLL 目录；可选探测 renderdoc 导入。

```mermaid
sequenceDiagram
participant Boot as "引导器"
participant FS as "文件系统"
participant Py as "Python 解释器"
Boot->>Boot : "读取 manifest.runtime.json"
Boot->>FS : "解析并校验路径(防逃逸)"
Boot->>Py : "设置 RDC_TOOL_* 环境变量"
Boot->>Py : "sys.path.insert(0, pymodules_dir)"
Boot->>Py : "PATH 前置 binaries_dir"
Boot->>Py : "Windows : add_dll_directory(binaries/pymodules)"
Boot->>Py : "可选 : import renderdoc"
```

**图表来源**
- [python_runtime.py:28-172](file://rdc_tool/python_runtime.py#L28-L172)
- [runtime_bootstrap.py:105-131](file://rdc_tool/runtime_bootstrap.py#L105-L131)

**章节来源**
- [python_runtime.py:28-172](file://rdc_tool/python_runtime.py#L28-L172)
- [runtime_bootstrap.py:18-131](file://rdc_tool/runtime_bootstrap.py#L18-L131)

### 配置管理系统
- 配置模型：BackendConfig、ReplayConfig、WorkerConfig、ArtifactConfig、DatabaseConfig、BisectConfig、ConfidenceWeightsConfig、SnapshotRetentionConfig、RuntimeLimitsConfig、AdaptiveBisectConfig、PatchConfig、ReportConfig、RdcToolConfig。
- 环境变量覆盖：from_env 支持大量 RDC_TOOL_* 环境变量，如 RDC_TOOL_RENDERDOC_PATH、RDC_TOOL_ARTIFACT_STORE、RDC_TOOL_DATA_DIR、RDC_TOOL_REPORT_DIR、RDC_TOOL_LOG_LEVEL、RDC_TOOL_GPU_VENDOR、RDC_TOOL_SPIRV_TOOLS_PATH、RDC_TOOL_HEADLESS、各类 bisect 与运行时限制变量。
- 运行时应用：_apply_runtime_config 支持在运行期动态更新 artifact_dir、log_level、snapshot_retention、bisect、confidence_weights、adaptive_bisect、runtime_limits 等，并持久化到 _runtime.config。
- 配置序列化：_serialize_runtime_config 输出当前配置摘要，便于对外暴露或记录。

```mermaid
classDiagram
class RdcToolConfig {
+backend : BackendConfig
+replay : ReplayConfig
+worker : WorkerConfig
+artifact : ArtifactConfig
+database : DatabaseConfig
+bisect : BisectConfig
+patch : PatchConfig
+report : ReportConfig
+confidence_weights : ConfidenceWeightsConfig
+snapshot_retention : SnapshotRetentionConfig
+runtime_limits : RuntimeLimitsConfig
+adaptive_bisect : AdaptiveBisectConfig
+renderdoc_module_path : str?
+log_level : str
+from_env() RdcToolConfig
}
class BackendConfig
class ReplayConfig
class WorkerConfig
class ArtifactConfig
class DatabaseConfig
class BisectConfig
class ConfidenceWeightsConfig
class SnapshotRetentionConfig
class RuntimeLimitsConfig
class AdaptiveBisectConfig
class PatchConfig
class ReportConfig
RdcToolConfig --> BackendConfig
RdcToolConfig --> ReplayConfig
RdcToolConfig --> WorkerConfig
RdcToolConfig --> ArtifactConfig
RdcToolConfig --> DatabaseConfig
RdcToolConfig --> BisectConfig
RdcToolConfig --> ConfidenceWeightsConfig
RdcToolConfig --> SnapshotRetentionConfig
RdcToolConfig --> RuntimeLimitsConfig
RdcToolConfig --> AdaptiveBisectConfig
RdcToolConfig --> PatchConfig
RdcToolConfig --> ReportConfig
```

**图表来源**
- [config.py:15-186](file://rdc_tool/config.py#L15-L186)

**章节来源**
- [config.py:15-186](file://rdc_tool/config.py#L15-L186)
- [server_runtime.py:277-390](file://rdc_tool/server_runtime.py#L277-L390)

### 超时策略
- 基础超时：针对远程连接、本地/远程打开回放、本地打开文件、会话上下文、像素历史等定义默认秒数。
- 重量级前缀：HEAVY_TIMEOUT_PREFIXES 与 VERY_HEAVY_TIMEOUT_PREFIXES 用于识别重负载操作，分别赋予更长的超时。
- 操作超时计算：operation_timeout_s 根据操作名与参数决定基础超时；transport_timeout_s 在此基础上增加缓冲时间；daemon_exec_timeout_s 与 worker_exec_timeout_s 分别用于不同执行层；harness_timeout_s 取最大值保证最低阈值。

```mermaid
flowchart TD
Op["传入 operation 与 args"] --> Type{"操作类型匹配"}
Type --> |rd.remote.connect| T1["remote_connect_timeout_ms -> 转换为秒"]
Type --> |rd.capture.open_replay| T2{"options.remote_id 非空?"}
T2 --> |是| T3["REMOTE_OPEN_REPLAY_TIMEOUT_S"]
T2 --> |否| T4["LOCAL_OPEN_REPLAY_TIMEOUT_S"]
Type --> |rd.capture.open_file| T5["LOCAL_OPEN_FILE_TIMEOUT_S"]
Type --> |rd.session.*| T6["SESSION_CONTEXT_TIMEOUT_S"]
Type --> |关闭/断开| T7["90.0 秒"]
Type --> |重/极重前缀| T8["VERY_HEAVY/HEAVY 超时"]
Type --> |其他| T9["DEFAULT_DAEMON_REQUEST_TIMEOUT_S"]
T1 --> Out["返回 base 超时"]
T3 --> Out
T4 --> Out
T5 --> Out
T6 --> Out
T7 --> Out
T8 --> Out
T9 --> Out
```

**图表来源**
- [timeout_policy.py:8-104](file://rdc_tool/timeout_policy.py#L8-L104)

**章节来源**
- [timeout_policy.py:1-104](file://rdc_tool/timeout_policy.py#L1-L104)

### 状态与快照、临时文件处理
- 上下文状态：默认 schema_version、backend、captures/sessions/recovery/preview/limits/metrics/recent_operations 等；支持加载、保存、清理、列出上下文 ID。
- 并发安全：每个上下文对应一个互斥锁，并使用 msvcrt.locking 对 .lock 文件进行锁定，确保多进程/多线程写入安全。
- 最近操作裁剪：按 trace_id 去重并按更新时间排序，保留最近 N 条。
- 指标统计：操作计数、错误计数、恢复计数、拒绝计数、活跃会话/捕获/操作计数、最近操作耗时分布与百分位。
- 临时文件：io_utils 提供原子写入与交换、JSON 安全序列化、追加 JSONL 日志；atomic_swap_path 支持重试与备份恢复。

```mermaid
sequenceDiagram
participant Srv as "server"
participant St as "runtime_state"
participant IO as "io_utils"
Srv->>St : "save_context_state(state, ctx, limits)"
St->>St : "normalize_context_state(...)"
St->>IO : "atomic_write_json(path, state)"
Srv->>St : "append_runtime_log(ctx, entry)"
St->>IO : "atomic_append_jsonl(path, entry)"
```

**图表来源**
- [runtime_state.py:388-472](file://rdc_tool/runtime_state.py#L388-L472)
- [io_utils.py:117-161](file://rdc_tool/io_utils.py#L117-L161)

**章节来源**
- [runtime_state.py:104-500](file://rdc_tool/runtime_state.py#L104-L500)
- [io_utils.py:13-161](file://rdc_tool/io_utils.py#L13-L161)
- [context_snapshot.py:130-200](file://rdc_tool/context_snapshot.py#L130-L200)

### 工作进程与服务协调机制
- 工作进程：main 解析 --context-id，设置 RDC_TOOL_CONTEXT_ID，启动父进程守护线程，发送 ready 消息，使用 asyncio.Runner 与单线程 executor 保持持久回放线程；stdin 循环读取 JSON 指令，调用 server.dispatch_operation 执行 exec/clear_context/status/shutdown。
- 服务运行期：server_runtime 维护全局 _runtime（包含 config/logs/captures/replays/remotes/contexts/previews/shader_debugs 等），提供配置应用、上下文容量检查、快照同步、度量更新、后端推断与投影等。
- 操作目录：runtime_catalog 从 operation_definitions 加载公开操作定义，生成目录载荷，支持 describe_operation 与 validate_operation_arguments 进行输入校验。

```mermaid
sequenceDiagram
participant W as "worker"
participant S as "server"
participant C as "catalog"
participant O as "operations"
W->>S : "exec(rd.xxx, params, ...)"
S->>C : "describe_operation(name)"
C->>O : "读取 input_schema"
S->>S : "validate_operation_arguments(name, args)"
S->>S : "执行具体 handler(内部服务)"
S-->>W : "结果(ok/result/error/meta/artifacts)"
```

**图表来源**
- [runtime_worker.py:97-161](file://rdc_tool/runtime_worker.py#L97-L161)
- [runtime_catalog.py:17-91](file://rdc_tool/runtime_catalog.py#L17-L91)
- [operation_definitions.py:1-200](file://rdc_tool/operation_definitions.py#L1-L200)

**章节来源**
- [runtime_worker.py:65-167](file://rdc_tool/runtime_worker.py#L65-L167)
- [server_runtime.py:196-468](file://rdc_tool/server_runtime.py#L196-L468)
- [runtime_catalog.py:13-91](file://rdc_tool/runtime_catalog.py#L13-L91)
- [operation_definitions.py:1-200](file://rdc_tool/operation_definitions.py#L1-L200)

## 依赖关系分析
- 模块耦合：
  - server_runtime 强依赖 runtime_paths、config、timeout_policy、runtime_state、context_snapshot、io_utils、runtime_catalog、operation_definitions。
  - runtime_worker 依赖 server 的 dispatch_operation 与 runtime_shutdown。
  - python_runtime 与 runtime_bootstrap 共同协作完成内置 Python 与 RenderDoc 模块的环境准备。
  - runtime_requirements 被依赖检查流程使用，不反向依赖上层。
- 外部依赖：
  - 运行时必需 Python 包：pydantic、numpy、Pillow、jinja2、aiofiles。
  - Windows 特定：msvcrt 用于文件锁；os.add_dll_directory 用于 DLL 搜索路径注册。
- 潜在循环：未发现直接循环依赖；catalog 仅读取 operation_definitions，server 通过 catalog 做校验，单向依赖清晰。

```mermaid
graph LR
SR["server_runtime.py"] --> RP["runtime_paths.py"]
SR --> CFG["config.py"]
SR --> TP["timeout_policy.py"]
SR --> RS["runtime_state.py"]
SR --> CS["context_snapshot.py"]
SR --> IOU["io_utils.py"]
SR --> RC["runtime_catalog.py"]
RC --> OD["operation_definitions.py"]
RW["runtime_worker.py"] --> SR
PB["runtime_bootstrap.py"] --> PR["python_runtime.py"]
PR --> RP
```

**图表来源**
- [server_runtime.py:1-120](file://rdc_tool/server_runtime.py#L1-L120)
- [runtime_worker.py:65-167](file://rdc_tool/runtime_worker.py#L65-L167)
- [runtime_bootstrap.py:105-131](file://rdc_tool/runtime_bootstrap.py#L105-L131)
- [python_runtime.py:28-92](file://rdc_tool/python_runtime.py#L28-L92)
- [runtime_catalog.py:13-91](file://rdc_tool/runtime_catalog.py#L13-L91)
- [operation_definitions.py:1-200](file://rdc_tool/operation_definitions.py#L1-L200)

**章节来源**
- [server_runtime.py:1-120](file://rdc_tool/server_runtime.py#L1-L120)
- [runtime_worker.py:65-167](file://rdc_tool/runtime_worker.py#L65-L167)
- [runtime_bootstrap.py:105-131](file://rdc_tool/runtime_bootstrap.py#L105-L131)
- [python_runtime.py:28-92](file://rdc_tool/python_runtime.py#L28-L92)
- [runtime_catalog.py:13-91](file://rdc_tool/runtime_catalog.py#L13-L91)
- [operation_definitions.py:1-200](file://rdc_tool/operation_definitions.py#L1-L200)

## 性能考量
- 超时分级：对重负载操作自动延长超时，避免阻塞整体流程；传输层额外缓冲，提升稳定性。
- 上下文容量：限制最大上下文数、每上下文会话数、捕获文件数量与大小估计，防止资源耗尽。
- 状态裁剪：recent_operations 限制长度，减少内存占用与序列化开销。
- 原子写入：避免并发竞争导致的损坏；重试与备份提高可靠性。
- 单线程回放：worker 使用单线程 executor 维持持久回放线程，避免频繁创建销毁带来的原生上下文失效。

[本节为通用指导，无需特定文件引用]

## 故障诊断指南
- 路径问题：
  - 确认 RDC_TOOL_ROOT 指向有效目录；tools_root 会警告覆盖情况。
  - 检查 ensure_runtime_dirs 是否创建了 intermediate/runtime/cli_runtime_dir/worker_state/artifacts/pytest/logs。
- Python/RenderDoc 引导失败：
  - 查看 bootstrap 报告的 dll_dirs_registered 与 dll_dir_errors；确认 Windows 下 add_dll_directory 可用。
  - 使用 validate_bundled_python_layout 检查内置 Python 元数据与关键路径。
- 依赖缺失：
  - 使用 missing_dependencies 获取缺失包列表；确保 site-packages 中包含必需依赖。
- 超时与卡死：
  - 根据 operation 名称与重量级前缀评估预期超时；必要时通过参数 timeout_ms 调整远程连接超时。
- 状态不一致：
  - 检查 runtime_state.json 与 runtime_logs.jsonl；使用 read_runtime_logs 拉取最近日志；利用 summarize_operation_durations 分析耗时分布。
- 上下文超限：
  - 当达到 max_contexts 时会抛出 context_limit_exceeded；清理不再使用的上下文或增大限制。

**章节来源**
- [runtime_paths.py:14-123](file://rdc_tool/runtime_paths.py#L14-L123)
- [runtime_bootstrap.py:70-131](file://rdc_tool/runtime_bootstrap.py#L70-L131)
- [python_runtime.py:104-172](file://rdc_tool/python_runtime.py#L104-L172)
- [runtime_requirements.py:45-73](file://rdc_tool/runtime_requirements.py#L45-L73)
- [timeout_policy.py:56-104](file://rdc_tool/timeout_policy.py#L56-L104)
- [runtime_state.py:448-500](file://rdc_tool/runtime_state.py#L448-L500)
- [server_runtime.py:427-468](file://rdc_tool/server_runtime.py#L427-L468)

## 结论
该运行时管理系统通过清晰的分层与职责划分，实现了路径解析、依赖检查、Python 环境引导、配置管理、超时策略、状态持久化与工作进程协调的完整闭环。其设计强调可移植性、可恢复性与可观测性，适合在多种部署环境中稳定运行，并为上层工具提供一致的图形捕获与回放能力。

[本节为总结，无需特定文件引用]

## 附录：配置示例与最佳实践
- 环境变量建议：
  - RDC_TOOL_ROOT：指向工具根目录，便于多版本共存与调试。
  - RDC_TOOL_INTERMEDIATE_ROOT：自定义中间产物与日志位置，便于磁盘规划与清理。
  - RDC_TOOL_RENDERDOC_PATH：指定 pymodules 目录，便于替换或扩展 RenderDoc Python 绑定。
  - RDC_TOOL_ARTIFACT_STORE、RDC_TOOL_DATA_DIR、RDC_TOOL_REPORT_DIR：分别配置工件存储、数据库与报告输出路径。
  - RDC_TOOL_LOG_LEVEL：控制日志级别，便于生产与调试切换。
  - RDC_TOOL_GPU_VENDOR、RDC_TOOL_SPIRV_TOOLS_PATH、RDC_TOOL_HEADLESS：按需配置后端与工具链。
  - RDC_TOOL_BISECT_*、RDC_TOOL_MAX_*：调整二分策略与运行时限制，平衡性能与资源。
- 运行时配置：
  - 通过 _apply_runtime_config 动态调整 artifact_dir、log_level、snapshot_retention、bisect、confidence_weights、adaptive_bisect、runtime_limits，无需重启。
- 最佳实践：
  - 启动时调用 ensure_runtime_dirs 确保目录存在。
  - 使用 bootstrap_renderdoc_runtime(probe_import=True) 快速验证环境。
  - 对重负载操作预留足够超时，结合 harness_timeout_s 设定上限。
  - 定期清理 intermediate/runtime 下的旧状态与日志，避免磁盘膨胀。
  - 使用 validate_bundled_python_layout 在发布前自检内置 Python 完整性。

[本节为通用指导，无需特定文件引用]