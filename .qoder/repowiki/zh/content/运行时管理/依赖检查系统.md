...
## 简介
本文件系统性阐述 RDX 工具的依赖检查与验证体系，覆盖环境验证（操作系统、Python 运行时、RenderDoc）、依赖项检测（必需组件、可选功能、平台特定要求）、兼容性检查（版本冲突与向后兼容）、缺失恢复策略与安装指导，以及诊断方法与配置扩展。目标是帮助开发者与用户快速定位并解决环境问题，确保工具在 Windows x64 环境下稳定运行。
...
- [rdx/core/renderdoc_status.py:57-104](file://rdx/core/renderdoc_status.py#L57-L104)
...
**章节来源**
- [rdx/runtime_bootstrap.py:18-130](file://rdx/runtime_bootstrap.py#L18-L130)
- [rdx/python_runtime.py:14-172](file://rdx/python_runtime.py#L14-L172)
- [rdx/runtime_requirements.py:7-73](file://rdx/runtime_requirements.py#L7-L73)
- [rdx/core/renderdoc_status.py:10-104](file://rdx/core/renderdoc_status.py#L10-L104)
...
**图示来源**
- [rdx/runtime_bootstrap.py:105-130](file://rdx/runtime_bootstrap.py#L105-L130)
- [rdx/runtime_paths.py:42-57](file://rdx/runtime_paths.py#L42-L57)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)
...
- 环境变量
  ...
  - RDX_RUNTIME_DLL_DIR：覆盖二进制目录（默认 binaries/windows/x64）。
  - RDX_RENDERDOC_PATH：覆盖 pymodules 目录（默认 binaries/windows/x64/pymodules）。
  - RDX_INTERMEDIATE_ROOT：覆盖中间产物目录。
- 清单与层配置
...
## 故障排查指南
- 常见问题
  - 无法导入 renderdoc：检查 RDX_RENDERDOC_PATH 是否正确指向 pymodules，确认 binaries 目录已在 PATH，且 add_dll_directory 已成功注册。
...
- 诊断步骤
  ...
  - 检查环境变量 RDX_TOOLS_ROOT、RDX_RUNTIME_DLL_DIR、RDX_RENDERDOC_PATH。
  - 查看 runtime_bootstrap 返回的 dll_dirs_registered 与 dll_dir_errors。
- 解决方案
  - 重新安装或升级 rdx-tools，确保 binaries 与 pymodules 完整。
  - 修正 PATH 与 PYTHONPATH，确保模块与 DLL 可被找到。
  - 根据 renderdoc_status 的错误详情，调整 Vulkan 层配置或驱动版本。

**章节来源**
- [rdx/runtime_bootstrap.py:70-130](file://rdx/runtime_bootstrap.py#L70-L130)
- [rdx/python_runtime.py:104-172](file://rdx/python_runtime.py#L104-L172)
- [rdx/core/renderdoc_status.py:57-104](file://rdx/core/renderdoc_status.py#L57-L104)
