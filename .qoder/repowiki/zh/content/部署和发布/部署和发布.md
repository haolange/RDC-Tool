...
- Windows安装流程（已简化）
  ...
  - **PowerShell安装脚本**：rdc_tool_install.ps1 处理所有安装、升级、卸载和医生检查操作，包括PATH管理和文件复制。
...
- 包信息：name=rdc-tools，requires-python>=3.11，license=Apache-2.0，作者 RDC。
- 依赖：aiofiles、jinja2、numpy、Pillow、pydantic。
- 可选依赖：dev=pytest。
- 入口点：rdc = rdc_tool.cli:main。
- 包发现：include=["rdc*"]，package-dir 指向根目录。
...
- 文件筛选：
  ...
  - 允许根：bin、binaries、cli、docs、policy、rdc、scripts、spec 以及若干根级文件（README、LICENSE、CHANGELOG、THIRD_PARTY_NOTICES、pyproject.toml、install.cmd 等）。
- 清单与 SBOM：
...
### 发布前质量检查（门禁）
- 结构与必需文件：检查 rdc、bin、cli、spec、policy、docs、tests、运行时目录、intermediate 子目录等是否存在；必需文件如 CHANGELOG.md、THIRD_PARTY_NOTICES.md、docs/tool-reference.md 等。
- 文本扫描规则：
  - 禁止引用 extensions 路径、调试框架术语。
  - 用户文档不得出现 uv.lock、uv sync、python -m venv、pip install 等引导命令。
  - 用户文档示例不得使用 bin/rdc-tool.cmd 作为命令示例，应使用 rdc。
- 公开契约：
...
- 许可证合规：
  - LICENSE_INVENTORY.json 与 SBOM.json 由打包脚本自动生成，包含 rdc-tools 与各 site-packages 组件的 license。
...
- **简化后的Windows安装流程**
  ...
  - **rdc_tool_install.ps1**：处理所有安装相关操作，包括文件复制、PATH管理、doctor检查
...
- **简化安装流程**：install.cmd 作为薄包装器，rdc_tool_install.ps1 处理所有安装逻辑，bin/rdc-tool.cmd 直接调用捆绑Python。
...
- 常用命令
  ...
  - **高级安装选项**：powershell -File scripts/rdc_tool_install.ps1 -Action upgrade -InstallDir "$env:LOCALAPPDATA\Programs\rdc-tools" -AddToPath -DryRun
...
- **简化安装流程说明**
  ...
  - rdc_tool_install.ps1：完整安装逻辑，支持 install、upgrade、uninstall、doctor 四种操作
