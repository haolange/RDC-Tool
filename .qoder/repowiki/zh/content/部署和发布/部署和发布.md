...
- 用户入口：rdx.bat、bin/rdx、cli/run_cli.py
...
D["规范 spec/*"] --> B
E["文档 docs/*"] --> B
F["入口 rdx.bat / bin/rdx / cli/run_cli.py"] --> B
B --> G["打包 package_release.py"]
B --> H["运行时打包 package_runtime.py"]
B --> I["发布门禁 release_gate.py"]
B --> J["包验证 verify_release_package.py"]
G --> K["dist/*.zip + SHA256SUMS"]
H --> L["manifest.runtime.json"]
I --> M["intermediate/logs/release_gate_report.md"]
J --> N["验证通过/失败"]
```
...
- 安装与运行
  - Windows 启动器：rdx.bat 调用 PowerShell 启动器脚本，设置环境变量并转发参数。
  - 安装脚本：rdx_install.ps1 支持 install/upgrade/uninstall/doctor，可写入 PATH，并在安装后执行 doctor。
...
## 架构总览
下图展示从源码到发布包的端到端流水线，以及质量门禁与验证环节。
...
- 文件筛选：
  ...
  - 允许根：bin、binaries、cli、docs、policy、rdx、scripts、spec 以及若干根级文件（README、LICENSE、CHANGELOG、THIRD_PARTY_NOTICES、pyproject.toml、rdx.bat 等）。
- 清单与 SBOM：
...
- 签名：
  - 仓库未包含代码签名步骤；可在 CI 中集成 signtool 或其他签名工具，对 rdx.bat、bin/rdx、关键 dll 进行签名，并将签名信息纳入发布工件。
- 文档更新：
...
- 典型顺序
  ...
  4) 包验证：scripts/verify_release_package.py
     - 参数：--zip <路径>
  5) 安装与冒烟：scripts/rdx_install.ps1、scripts/smoke_cli.sh
- 定制要点
...
- 回滚策略
  ...
  - 安装脚本回滚：rdx_install.ps1 支持 uninstall，可清理安装目录与 PATH 条目。
- 故障恢复
...
- [scripts/verify_release_package.py:301-332](file://scripts/verify_release_package.py#L301-L332)
- [scripts/package_runtime.py:288-350](file://scripts/package_runtime.py#L288-L350)
- [scripts/rdx_install.ps1:135-171](file://scripts/rdx_install.ps1#L135-L171)
...
- 通过 rdx_install.ps1 与 smoke_cli.sh 支持安装与冒烟验证。
遵循本文流程与定制指南，可实现稳定、可追溯、可回滚的发布体系。
...
- 常用命令
  ...
  - 安装：powershell -File scripts/rdx_install.ps1 -Action install -AddToPath
  - 冒烟：bash scripts/smoke_cli.sh --tools-root . --context smoke-<时间戳>
- 定制要点
  - 调整允许/排除规则：修改 package_release.py 的排除与允许集合。
  - 调整站点包策略：修改 package_runtime.py 的白名单与过滤逻辑。
  - 扩展契约检查：在 release_gate.py 添加新规则并在 main 中注册。
  - 文档约束：在 release_gate.py 中维护 USER_DOCS 与禁止规则。
