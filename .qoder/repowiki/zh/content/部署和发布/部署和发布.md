基于我对代码的分析，我现在了解了安装脚本的命名约定变更。让我更新部署和发布文档以反映这些变更：
...
## 更新摘要
**所做更改**
- 更新了Windows安装流程部分，反映新的 rdc-tool 命名约定
- 将 PowerShell 安装脚本从 `scripts/rdx_install.ps1` 重命名为 `scripts/rdc_tool_install.ps1`
- 更新了安装脚本中的目录结构和文件路径逻辑
- 增强了安装脚本的安全检查和错误处理
- 更新了自动化发布脚本使用指南以反映新的命名约定
...
- 源码与工具：rdc、cli、spec、policy
- 构建与发布脚本：scripts/*
- 运行时与平台产物：binaries/windows/x64/python、pymodules
- 用户入口：bin/rdc-tool.cmd、bin/rdc-tool、cli/run_cli.py
- 安装入口：install.cmd（简化版Windows安装器）
- 构建元数据：pyproject.toml、rdc_tool/__init__.py
- 发布产物与中间产物：dist、intermediate

```mermaid
graph TB
A["源码 rdc"] --> B["脚本 scripts/*"]
C["运行时 binaries/windows/x64"] --> B
D["规范 spec/*"] --> B
E["文档 docs/*"] --> B
F["入口 bin/rdc-tool.cmd / cli/run_cli.py"] --> B
G["安装器 install.cmd"] --> H["PowerShell安装脚本 rdc_tool_install.ps1"]
...
- 构建与打包
  ...
  - 版本来源：rdc_tool/__init__.py 提供 __version__，被打包脚本读取用于产物命名与清单。
...
- Windows安装流程（已简化并使用新命名约定）
  - **简化后的安装器**：install.cmd 现在是一个薄包装器，仅负责调用 PowerShell 安装脚本，不再包含复杂的运行时代码。
  - **PowerShell安装脚本**：rdc_tool_install.ps1（已从 rdx_install.ps1 重命名）处理所有安装、升级、卸载和医生检查操作，包括PATH管理和文件复制。
  - **薄启动器**：bin/rdc-tool.cmd 直接调用捆绑的Python和CLI，不经过任何中间层。
- 质量门禁与验证
  - 发布门禁：release_gate.py 执行目录/文件存在性、公开命令契约、目录扫描规则、catalog 一致性、工具参考文档一致性、用户文档约束、Smoke 报告、包完整性等。
  - 包验证：verify_release_package.py 解压后校验清单、许可证清单/SBOM、doctor/launcher/version/tools list 契约、禁止暴露项（mcp/tests/.rdc_tool/pre-GA 标记）。
  - 校验和生成：generate_release_checksums.py 对指定文件或目录计算 SHA256 并输出。
- 安装与运行
  - **简化安装流程**：install.cmd 接受参数并传递给 rdc_tool_install.ps1，支持 install、upgrade、uninstall、doctor 操作。
...
participant PS as "PowerShell安装脚本 rdc_tool_install.ps1"
...
- [scripts/verify_release_package.py:301-332](file://scripts/verify_release_package.py#L301-L332)
- [install.cmd:1-11](file://install.cmd#L1-L11)
- [scripts/rdc_tool_install.ps1:157-187](file://scripts/rdc_tool_install.ps1#L157-L187)

## 详细组件分析

### 构建系统与依赖管理
- 构建后端：setuptools.build_meta，要求 setuptools>=68 与 wheel。
- 包信息：name=rdc-tool，requires-python>=3.11，license=Apache-2.0，作者 RDC。
- 依赖：aiofiles、jinja2、numpy、Pillow、pydantic。
- 可选依赖：dev=pytest。
- 入口点：rdc-tool = rdc_tool.cli:main。
- 包发现：include=["rdc_tool*"]，package-dir 指向根目录。
...
### 版本管理策略
- 版本号来源：rdc_tool/__init__.py 中的 __version__，当前为 1.0.0。
- 版本传播：
...
- 建议实践：
  - 每次发布前统一更新 rdc_tool/__init__.py 的版本号。
...
- 文件筛选：
  ...
  - 允许根：bin、binaries、cli、docs、policy、rdc_tool、scripts、spec 以及若干根级文件（README、LICENSE、CHANGELOG、THIRD_PARTY_NOTICES、pyproject.toml、install.cmd 等）。
- 清单与 SBOM：
...
- 压缩与校验：
  - 输出 dist/rdc-tool-{version}-windows-x64.zip。
...
### 发布前质量检查（门禁）
- 结构与必需文件：检查 rdc_tool、bin、cli、spec、policy、docs、tests、运行时目录、intermediate 子目录等是否存在；必需文件如 CHANGELOG.md、THIRD_PARTY_NOTICES.md、docs/tool-reference.md 等。
- 文本扫描规则：
  - 禁止引用 extensions 路径、调试框架术语。
  - 用户文档不得出现 uv.lock、uv sync、python -m venv、pip install 等引导命令。
  - 用户文档示例不得使用 bin/rdc-tool.cmd 作为命令示例，应使用 rdc-tool。
- 公开契约：
  - 运行 rdc-tool --help/--version/--json doctor/tools list/context status/list/update/clear/vfs ls 等，校验输出格式与契约。
...
- 负向契约：拒绝 mcp/tests/.rdc_tool/pre-GA 标记泄露。
...
V-->>V : 检查禁止项(mcp/tests/.rdc_tool/pre-GA)
V-->>V : 输出 PASS/FAIL
```
...
- 签名：
  - 仓库未包含代码签名步骤；可在 CI 中集成 signtool 或其他签名工具，对 install.cmd、bin/rdc-tool.cmd、关键 dll 进行签名，并将签名信息纳入发布工件。
- 文档更新：
  - 发布前需确保 docs/release-notes.md 更新完毕。
  - 若新增/变更公开操作，需保证 docs/tool-reference.md 与代码定义一致（门禁会校验）。
- 许可证合规：
  - LICENSE_INVENTORY.json 与 SBOM.json 由打包脚本自动生成，包含 rdc-tool 与各 site-packages 组件的 license。
...
- 典型顺序
  ...
  2) 打包发布：scripts/package_release.py
     - 参数：--out-dir（默认 dist）、--version（应与 rdc_tool/__init__.py 一致）
     - 输出：dist/rdc-tool-{version}-windows-x64.zip、SHA256SUMS、release_report.md、清单/SBOM
  ...
  4) 包验证：scripts/verify_release_package.py
     - 参数：--zip <路径>
  5) **简化安装流程**：install.cmd 和 scripts/rdc_tool_install.ps1
     - install.cmd 参数：无（自动调用 install 操作）
     - rdc_tool_install.ps1 参数：-Action {install|upgrade|uninstall|doctor}、-InstallDir、-AddToPath、-DryRun
- **简化后的Windows安装流程（使用新命名约定）**
  - **install.cmd**：薄包装器，仅调用 PowerShell 安装脚本，不再包含复杂逻辑
  - **rdc_tool_install.ps1**：处理所有安装相关操作，包括文件复制、PATH管理、doctor检查（已从 rdx_install.ps1 重命名）
  - **bin/rdc-tool.cmd**：薄启动器，直接调用捆绑Python和CLI
- 定制要点
...
- 回滚策略
  ...
  - **简化安装回滚**：rdc_tool_install.ps1 支持 uninstall，可清理安装目录与 PATH 条目，且不会误删非RDC目录。
- 故障恢复
...
- [scripts/verify_release_package.py:301-332](file://scripts/verify_release_package.py#L301-L332)
- [scripts/package_runtime.py:288-350](file://scripts/package_runtime.py#L288-L350)
- [scripts/rdc_tool_install.ps1:168-187](file://scripts/rdc_tool_install.ps1#L168-L187)
...
- 脚本间依赖：
  ...
  - **简化安装依赖**：install.cmd 仅依赖 rdc_tool_install.ps1，不再包含复杂逻辑。

```mermaid
graph LR
PR["pyproject.toml"] --> DEP["依赖声明"]
DEP --> RUNTIME["运行时依赖"]
PR --> ENTRY["入口点 rdc-tool"]
...
RG --> VF
INST["install.cmd"] --> PS["rdc_tool_install.ps1"]
PS --> PATH["PATH管理"]
PS --> COPY["文件复制"]
PS --> DOCTOR["Doctor检查"]
```
...
## 故障排查指南
- 常见问题
  - 版本不匹配：package_release.py 会拒绝与 rdc_tool/__init__.py 不一致的 --version。
  ...
  - 用户文档违规：出现 uv/pip/venv 引导命令或 bin/rdc-tool.cmd 示例，需按规则修正。
  - **安装问题**：PowerShell 权限不足、PATH设置冲突、捆绑Python缺失、旧版目录结构残留。
- 诊断步骤
...
- **简化安装流程**：install.cmd 作为薄包装器，rdc_tool_install.ps1（已从 rdx_install.ps1 重命名）处理所有安装逻辑，bin/rdc-tool.cmd 直接调用捆绑Python。
...
- 常用命令
  ...
  - 包验证：python scripts/verify_release_package.py --zip dist/rdc-tool-*.zip
  - **简化安装**：install.cmd（自动调用安装）或 powershell -File scripts/rdc_tool_install.ps1 -Action install -AddToPath
  - **高级安装选项**：powershell -File scripts/rdc_tool_install.ps1 -Action upgrade -InstallDir "$env:LOCALAPPDATA\Programs\rdc-tool" -AddToPath -DryRun
...
- **简化安装流程说明**
  ...
  - rdc_tool_install.ps1：完整安装逻辑，支持 install、upgrade、uninstall、doctor 四种操作（已从 rdx_install.ps1 重命名）
...
- 定制要点
  ...
  - **安装定制**：修改 rdc_tool_install.ps1 中的安装目录默认值、PATH管理策略、文件复制规则。
