...
```mermaid
graph TB
A["pyproject.toml<br/>定义包名/版本/依赖"] --> B["rdx/__init__.py<br/>导出 __version__"]
...
- 版本源与语义化约定
  ...
  - rdx/__init__.py 暴露 __version__，供打包脚本与 CLI 使用。
- 发布打包与清单
...
## 架构总览
版本管理的关键流程如下：
- 版本定义：pyproject.toml 与 rdx/__init__.py 共同维护版本。
...
- 版本来源
  ...
  - rdx/__init__.py 的 __version__ 被打包脚本导入，确保构建期与运行期一致。
- 语义化版本
...
ReadPyproj --> ReadInit["读取 rdx/__init__.py __version__"]
...
### 版本信息的集中管理与自动更新机制
- 集中管理
  - 版本集中在 pyproject.toml 与 rdx/__init__.py 两处；打包脚本从 rdx.__version__ 获取版本，避免分散维护。
- 自动更新
...
```mermaid
graph LR
P["pyproject.toml"] --> V["rdx/__init__.py"]
...
## 故障排查指南
- 版本不一致
  - 若 package_release.py 检测到请求版本与包版本不一致，将返回错误码；需统一 pyproject.toml 与 rdx/__init__.py 的版本。
- 清单缺失或不完整
