...
## 更新摘要
**所做更改**
- 更新了版本号从1.0.0到1.0.1的所有分发元数据文件
- 增强了发布验证过程中的版本一致性检查，确保RELEASE_MANIFEST.json、SBOM.json、LICENSE_INVENTORY.json、pyproject.toml、捆绑分发元数据和CHANGELOG.md之间的对齐
- 添加了详细的版本一致性验证流程说明
- 更新了变更日志和发布说明以反映新版本特性
...
## 简介
本章节概述 RDC-Tool 项目的版本管理策略与规范。该项目采用集中式版本定义、可复现的发布产物与严格的发布门禁，确保版本号、依赖与二进制产物在构建、打包、校验环节保持一致性与可追溯性。当前版本已从1.0.0升级到1.0.1，增强了版本一致性验证机制，通过多重校验确保所有分发元数据文件的版本信息完全对齐。版本信息通过包元数据与运行时清单共同表达，并通过脚本自动化生成发布清单、许可证清单与软件物料清单（SBOM），配合测试与冒烟验证形成完整的发布流水线。

## 项目结构
围绕版本管理的核心位置如下：
- 包版本与依赖声明：pyproject.toml（当前版本：1.0.1）
- 运行时版本常量：rdc_tool/__init__.py（当前版本：1.0.1）
- 发布打包与清单生成：scripts/package_release.py
- 运行时二进制清单：binaries/windows/x64/manifest.runtime.json
- 变更记录：CHANGELOG.md（包含1.0.1版本记录）
...
```mermaid
graph TB
A["pyproject.toml<br/>版本: 1.0.1"] --> B["rdc_tool/__init__.py<br/>__version__: 1.0.1"]
...
I["CHANGELOG.md<br/>1.0.1版本记录"] --> J["docs/release-notes.md<br/>发布说明"]
...
- [CHANGELOG.md:5-11](file://CHANGELOG.md#L5-L11)
- [docs/release-notes.md:3-10](file://docs/release-notes.md#L3-L10)
...
## 核心组件
- 版本源与语义化约定
  - 包版本在 pyproject.toml 中声明为1.0.1，遵循语义化版本（主版本.次版本.修订号）。
  - rdc_tool/__init__.py 暴露 __version__ 为1.0.1，供打包脚本与 CLI 使用。
- 发布打包与清单
...
- 变更记录与发布说明
  - CHANGELOG.md 记录1.0.1版本的修复内容，包括恢复完整 Apache-2.0 LICENSE 并收入新 zip。
  - docs/release-notes.md 描述1.0.1版本的变更，强调版本元数据一致性验证。
- 版本一致性验证
...
## 架构总览
版本管理的关键流程如下：
- 版本定义：pyproject.toml 与 rdc_tool/__init__.py 共同维护版本为1.0.1。
...
- 发布说明：release-notes.md 记录1.0.1版本的变更与基线，作为对外发布的依据。
...
- [scripts/verify_release_package.py:158-180](file://scripts/verify_release_package.py#L158-L180)
- [CHANGELOG.md:5-11](file://CHANGELOG.md#L5-L11)
- [docs/release-baseline.md:3-12](file://docs/release-baseline.md#L3-L12)

## 详细组件分析

### 版本源与语义化版本控制实践
- 版本来源
  - pyproject.toml 中的 project.version 设置为1.0.1，是包的权威版本来源。
  - rdc_tool/__init__.py 的 __version__ 设置为1.0.1，被打包脚本导入，确保构建期与运行期一致。
- 语义化版本
  - 采用 X.Y.Z 形式，当前为1.0.1，符合语义化版本的基本约定。
  - 1.0.1版本主要修复了LICENSE文件问题，保持向后兼容性。

```mermaid
flowchart TD
Start(["开始"]) --> ReadPyproj["读取 pyproject.toml 版本 (1.0.1)"]
ReadPyproj --> ReadInit["读取 rdc_tool/__init__.py __version__ (1.0.1)"]
...
### 变更日志与发布说明编写指南
- 变更记录
  - CHANGELOG.md 记录了1.0.1版本的主要修复：恢复完整 Apache-2.0 LICENSE 并收入新 zip，不修改 v1.0.0 tag 或旧资产。
- 发布说明
  - docs/release-notes.md 描述了1.0.1版本的变更，强调版本元数据一致性验证的重要性。
- 文档治理
...
- 兼容性保障
  ...
  - 1.0.1版本保持了向后兼容性，仅修复LICENSE文件问题。
- 向后兼容
...
- 1.0.1版本的热修复特性
  - 专门修复了LICENSE文件问题，保持了v1.0.0标签和原始资产的完整性。
...
### 版本升级迁移指南与依赖版本锁定策略
- 升级迁移
  - 根据 CHANGELOG.md 和 release-notes.md 的变更记录进行迁移；注意1.0.1版本主要是LICENSE文件修复。
...
```mermaid
graph LR
P["pyproject.toml<br/>版本: 1.0.1"] --> V["rdc_tool/__init__.py<br/>__version__: 1.0.1"]
...
- 1.0.1版本的增强验证可能增加构建时间，但确保了更高的质量保证。
...
## 结论
RDC-Tool 的版本管理以集中式版本定义为核心，结合自动化打包、清单生成与严格门禁，确保发布产物的可追溯性与稳定性。1.0.1版本的发布特别强调了版本一致性验证，通过多重检查确保所有分发元数据文件的版本信息完全对齐。通过变更记录、发布说明与文档治理，保证对外契约的一致性与可维护性。建议在外部 CI/CD 中集成发布基线步骤与版本一致性验证，以实现端到端的版本控制与发布自动化。
...
- 1.0.1版本特性
  - 恢复完整 Apache-2.0 LICENSE 并收入新 zip
  - 增强版本元数据一致性验证
  - 保持向后兼容性，不影响现有用户