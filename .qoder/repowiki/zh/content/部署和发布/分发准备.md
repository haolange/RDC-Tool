...
- [rdc_tool/core/artifact_publisher.py:15-33](file://rdc_tool/core/artifact_publisher.py#L15-L33)
...
- 生成清单：
  ...
  - SBOM.json：以 rdc-tools.sbom.v1 格式描述组件集合。
...
- 解压与定位：自动识别包根目录（包含 bin/rdc-tool.cmd）。
- 清单校验：验证 public_commands、entrypoints、files 字段，拒绝泄露 tests、.rdc、mcp 等路径。
- 许可证与 SBOM：确认 rdc-tools 许可证为 Apache-2.0，且 SBOM components 中正确声明。
- 契约测试：
...
participant CLI as "rdc 命令"
V->>Z : 打开 zip
V->>V : 检查禁止项(预GA/MCP)
V->>Z : 解压到临时目录
V->>R : 定位包根(bin/rdc-tool.cmd)
V->>R : 校验清单/许可证/SBOM
...
- 结构与文档：
  - 必需目录/文件存在性检查。
  - 用户文档不得包含 venv/pip/uv 等引导命令示例，不得出现 bin/rdc-tool.cmd 命令用法（仅保留作为启动器引用）。
  - 工具参考文档需与代码定义保持一致。
- 公开表面：
...
- 运行时与入口：
  - 校验内置 Python 布局与版本。
  - 校验 rdc-tool --help、--version、doctor、tools list 等入口行为。
- Smoke 与包：
...
- 模式选择：通过环境变量 RDC_TOOL_ARTIFACT_MODE 控制 local 或 remote。
...
- 项目许可证：清单与 SBOM 均要求 rdc-tools 许可证为 Apache-2.0。
...
- 用户文档约束：不得包含 venv/pip/uv 等引导命令，不得出现 bin/rdc-tool.cmd 命令示例（仅作为启动器引用），以确保安装与使用体验一致。
...
- 远程模式：设置 RDC_TOOL_ARTIFACT_MODE=remote，并配置 RDC_TOOL_S3_ENDPOINT、RDC_TOOL_S3_BUCKET、RDC_TOOL_S3_REGION、RDC_TOOL_S3_ACCESS_KEY、RDC_TOOL_S3_SECRET_KEY、RDC_TOOL_S3_PRESIGN_SECONDS。当制品大于等于 RDC_TOOL_REMOTE_ARTIFACT_MIN_BYTES 时，将上传至 S3 并生成预签名 URL。
...
- 常见失败点：
  ...
  - 许可证不符：LICENSE_INVENTORY.json 与 SBOM 中 rdc-tools 许可证必须为 Apache-2.0。
...
- 许可证不符：检查 LICENSE_INVENTORY.json 与 SBOM.json 中 rdc-tools 许可证是否为 Apache-2.0。
...
## 附录
- 关键环境变量：
  - RDC_TOOL_ARTIFACT_MODE：local/remote
  - RDC_TOOL_REMOTE_ARTIFACT_MIN_BYTES：远程上传最小字节数
  - RDC_TOOL_S3_ENDPOINT/BUCKET/REGION/ACCESS_KEY/SECRET_KEY/PRESIGN_SECONDS：S3 配置
- 关键输出：
  - dist/*.zip：发布包
