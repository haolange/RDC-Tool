...
- [bin/rdc-tool](file://bin/rdc-tool)
- [bin/rdc-tool.cmd](file://bin/rdc-tool.cmd)
- legacy rdc_bat_launcher.ps1（历史路径，当前不存在）
...
- bin/rdc-tool、bin/rdc-tool.cmd、legacy rdc_bat_launcher.ps1：跨平台启动器与 Windows 引导。
...
A --> E["bin/rdc-tool, bin/rdc-tool.cmd, legacy rdc_bat_launcher.ps1（已停用）"]
...
- [bin/rdc-tool:1-40](file://bin/rdc-tool#L1-L40)
...
### 启动器与入口点
- 跨平台入口：
  - bin/rdc-tool：优先使用内嵌 Python，否则回退到系统 Python。
  - bin/rdc-tool.cmd：Windows 上调用 PowerShell 启动器。
  - legacy rdc_bat_launcher.ps1（历史入口，当前不再使用）；当前入口为 bin/rdc-tool.cmd 与 cli/run_cli.py。
- 包元数据：
...
participant PS as "rdc_bat_launcher.ps1"
participant Sh as "bin/rdc-tool"
