...
## 更新摘要
**变更内容**
- 重大架构变更：Windows启动器从rdx.bat+PowerShell启动器改为bin/rdx.cmd薄启动器直接调用捆绑Python解释器
- 更新了Windows启动器架构章节，反映新的薄启动器设计
- 移除了对已删除的rdx.bat和PowerShell启动器的引用
- 更新了安装脚本以支持新的启动器结构
- 增强了发布包验证逻辑以适配新架构
...
**重要架构变更**：Windows启动器已从复杂的rdx.bat+PowerShell启动器架构简化为bin/rdx.cmd薄启动器，直接调用捆绑的Python解释器，显著降低了启动复杂性和依赖关系。
...
- **新增**：简化的Windows启动器架构，使用bin/rdx.cmd薄启动器直接调用Python解释器。
...
- **新架构**：薄启动器：bin/rdx.cmd - 直接调用捆绑Python解释器
...
- Windows安装脚本：scripts/rdx_install.ps1 - 支持新启动器架构
...
- **新架构**：Windows启动器现在使用bin/rdx.cmd薄启动器，直接调用捆绑的Python解释器，移除了对PowerShell的依赖。
...
### 薄启动器架构（bin/rdx.cmd）
**重大架构变更**：新的薄启动器架构完全替代了之前的rdx.bat+PowerShell启动器方案。

- **设计原则**：最小化启动器复杂度，直接调用捆绑的Python解释器。
- **环境变量处理**：自动设置RDX_TOOLS_ROOT指向工具根目录。
...
```mermaid
flowchart TD
Start(["用户执行 rdx"]) --> CheckEnv["设置 RDX_TOOLS_ROOT"]
...
- **工具根发现**：优先使用RDX_TOOLS_ROOT环境变量，否则回退到脚本所在目录。
...
- 允许根级文件：.gitattributes、.gitignore、AGENTS.md、CHANGELOG.md、LICENSE、THIRD_PARTY_NOTICES.md、README.md、pyproject.toml、bin/rdx.cmd、install.cmd。
...
### Windows安装脚本（scripts/rdx_install.ps1）
**架构更新**：安装脚本已更新以支持新的薄启动器架构。

- **启动器检测**：验证bin/rdx.cmd是否存在作为安装有效性检查。
- **旧文件清理**：自动移除已废弃的rdx.bat和scripts\rdx_bat_launcher.ps1。
- **PATH集成**：将bin目录添加到用户PATH环境变量。
- **健康检查**：安装后自动运行doctor命令验证安装完整性。
- **安全验证**：防止符号链接攻击和路径冲突。

**章节来源**
- [scripts/rdx_install.ps1:16-25](file://scripts/rdx_install.ps1#L16-L25)
- [scripts/rdx_install.ps1:65-113](file://scripts/rdx_install.ps1#L65-L113)
...
- **新架构问题**：
  - 如果bin/rdx.cmd报错"捆绑Python缺失"，需要重新安装或修复RDX工具。
  - 验证binaries/windows/x64/python/python.exe文件是否存在。
  - 检查RDX_TOOLS_ROOT环境变量是否正确设置。
...
## 结论
RDC-Tool的构建系统以pyproject.toml为核心声明Python包与依赖，配合专用脚本完成运行时二进制与发布包的构建。**重大架构改进**是通过引入bin/rdx.cmd薄启动器，完全替代了之前复杂的rdx.bat+PowerShell启动器方案，显著简化了Windows平台的启动流程和依赖关系。通过严格的文件过滤与路径安全检查，确保产物稳定、可追溯且易于分发。结合冒烟测试与安装脚本，形成从构建到部署的完整闭环。
...
- 安装方式：
  ...
  - **新架构**：直接使用bin/rdx.cmd启动，无需额外配置PowerShell。
...
- **新启动器参数**：
  - bin/rdx.cmd：无私有参数，直接传递所有参数给Python CLI。
  - 环境变量：RDX_TOOLS_ROOT用于指定工具根目录。
