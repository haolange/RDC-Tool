...
## 更新摘要
**所做更改**
- 新增超时架构章节，详细说明60秒工作器预算和65秒CLI传输截止时间的实现
- 更新会话上下文操作部分，说明新的超时策略和调用方职责
- 增强故障排查指南，包含超时相关的诊断信息
- 更新架构图表以反映超时机制在会话生命周期中的作用
...
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录：API调用模式与示例路径](#附录api调用模式与示例路径)
...
- **新增**：超时架构设计，包括60秒工作器预算和65秒CLI传输截止时间的统一策略。
...
- **新增**：rdc_tool/timeout_policy.py：统一的超时策略定义和管理，包括会话上下文操作的60秒预算和65秒传输截止时间。
- rdc_tool/cli.py：命令行接口，通过daemon_exec_timeout_s应用超时策略。
...
B --> H["timeout_policy.py<br/>超时策略管理"]
C --> I["RenderDoc本地/远端控制器"]
F --> J["ADB/Android设备"]
H --> K["60秒工作器预算<br/>65秒传输截止时间"]
```
...
- [timeout_policy.py:14-22](file://rdc_tool/timeout_policy.py#L14-L22)
...
- **新增**：TimeoutPolicy：统一的超时策略管理，定义会话上下文操作的60秒工作器预算和65秒CLI传输截止时间。
...
## 架构总览
下图展示从 CLI/上层调用到渲染后端的整体流程，包括上下文隔离、会话生命周期、预览与观察、Android 辅助工具和超时管理。
...
Note over T,S : 所有rd.session.*操作使用60秒预算<br/>CLI传输增加5秒缓冲区=65秒截止时间
```
...
- [timeout_policy.py:75-102](file://rdc_tool/timeout_policy.py#L75-L102)
- [cli.py:244-265](file://rdc_tool/cli.py#L244-L265)
...
**图表来源**
- [remote_bootstrap.py:294-338](file://rdc_tool/remote_bootstrap.py#L294-338)
- [remote_bootstrap.py:495-699](file://rdc_tool/remote_bootstrap.py#L495-699)
- [remote_bootstrap.py:701-754](file://rdc_tool/remote_bootstrap.py#L701-754)
...
### 统一超时策略设计
RDC-Tool 实现了统一的超时架构，专门针对会话上下文操作进行了优化：

- **60秒工作器预算**：所有 `rd.session.*` 操作（如 `rd.session.get_context`、`rd.session.observe`）在工作器层使用60秒超时预算。这个预算足够处理复杂的回放导航、目标选择和图像导出操作。
- **65秒CLI传输截止时间**：CLI传输层在工作器超时基础上添加5秒响应缓冲区，形成65秒的总截止时间。这确保了CLI能够可靠地接收工作器的响应。
- **分层超时管理**：不同操作类型有不同的超时策略，会话上下文操作使用专门的60秒预算，而重型操作（如事件查询、资源操作）使用60-180秒不等的预算。

### 超时策略实现
超时策略通过 `timeout_policy.py` 统一管理：

```python
SESSION_CONTEXT_TIMEOUT_S = 60.0  # 会话上下文操作预算
DAEMON_RESPONSE_BUFFER_S = 5.0    # CLI响应缓冲区

def operation_timeout_s(operation: str, args: dict[str, Any] | None) -> float:
    if operation.startswith("rd.session."):
        return SESSION_CONTEXT_TIMEOUT_S  # 会话操作使用60秒预算
    
def transport_timeout_s(operation: str, args: dict[str, Any] | None) -> float:
    base = operation_timeout_s(operation, args)
    return base + DAEMON_RESPONSE_BUFFER_S  # 添加5秒缓冲区
```

### 调用方职责与最佳实践
- **调用方不传递每请求超时**：所有超时由系统策略统一管理，调用方无需也不应该传递自定义超时参数。
- **主机应用程序职责**：外部CLI进程的超时设置必须大于65秒，以确保有足够时间完成会话上下文操作。
- **错误处理**：如果主机报告65秒前的超时，应验证使用的是当前打包的RDC-Tool版本，并检查外部CLI超时设置。
...
Host->>CLI : 发起rd.session.*操作
CLI->>Daemon : exec(operation, timeout=65s)
Daemon->>Worker : exec(operation, timeout=60s)
Worker->>Worker : 执行会话操作
alt 成功
Worker-->>Daemon : 结果
Daemon-->>CLI : 结果
CLI-->>Host : 成功响应
...
- [timeout_policy.py:75-102](file://rdc_tool/timeout_policy.py#L75-L102)
- [cli.py:244-265](file://rdc_tool/cli.py#L244-L265)
- [daemon/server.py:469-497](file://rdc_tool/daemon/server.py#L469-L497)

**章节来源**
- [timeout_policy.py:14-22](file://rdc_tool/timeout_policy.py#L14-L22)
- [timeout_policy.py:75-102](file://rdc_tool/timeout_policy.py#L75-L102)
- [cli.py:244-265](file://rdc_tool/cli.py#L244-L265)
- [session-model.md:42-46](file://docs/session-model.md#L42-L46)
- [troubleshooting.md:13-22](file://docs/troubleshooting.md#L13-L22)
...
- **新增**：timeout_policy 被 CLI 和 daemon 共同使用，提供统一的超时策略管理。
...
- [timeout_policy.py:97-106](file://rdc_tool/timeout_policy.py#L97-L106)
- [cli.py:46](file://rdc_tool/cli.py#L46)
...
- **新增**：统一超时管理：通过60秒工作器预算和65秒传输截止时间，确保会话操作的可靠性和响应性。
...
- **新增**：超时相关故障：
  - 会话操作超时：检查是否使用了正确的RDC-Tool版本，确认外部CLI超时设置大于65秒。
  - 传输超时：验证网络连接稳定性和远端服务的响应时间。
  - 工作器超时：检查是否有长时间运行的会话操作阻塞了工作器。
...
- [remote_bootstrap.py:156-202](file://rdc_tool/remote_bootstrap.py#L156-202)
- [remote_bootstrap.py:661-699](file://rdc_tool/remote_bootstrap.py#L661-699)
- [troubleshooting.md:13-22](file://docs/troubleshooting.md#L13-L22)

## 结论
RDC-Tool 的会话模型以 SessionManager 为核心，结合 server_runtime 的上下文隔离与快照机制，实现了本地/远端一致的 Replay 生命周期管理。**新增的统一超时架构**通过60秒工作器预算和65秒CLI传输截止时间，为会话上下文操作提供了可靠的超时保护。预览与观察功能提供了灵活的可视化与取证能力，Android 辅助工具确保了移动端调试的端到端闭环。通过严格的错误分类、资源清理、保留策略和超时管理，系统在稳定性与可维护性方面具备良好基础。
...
- Android 引导：参考 [remote_bootstrap.py:495-699](file://rdc_tool/remote_bootstrap.py#L495-699)
...
- **新增**：超时策略：参考 [timeout_policy.py:75-102](file://rdc_tool/timeout_policy.py#L75-L102)
- **新增**：CLI超时应用：参考 [cli.py:244-265](file://rdc_tool/cli.py#L244-L265)
...
- [preview_window.py:221-301](file://rdc_tool/preview_window.py#L221-301)
- [remote_bootstrap.py:495-699](file://rdc_tool/remote_bootstrap.py#L495-699)
- [context_snapshot.py:447-541](file://rdc_tool/context_snapshot.py#L447-L541)
- [runtime_worker.py:65-167](file://rdc_tool/runtime_worker.py#L65-L167)
- [timeout_policy.py:75-102](file://rdc_tool/timeout_policy.py#L75-L102)
