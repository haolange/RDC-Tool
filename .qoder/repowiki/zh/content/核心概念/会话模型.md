...
- [test_remote_presentation.py](file://tests/test_remote_presentation.py)
- [test_replay_observation.py](file://tests/test_replay_observation.py)
...
## 更新摘要
**所做更改**
- 更新了 Android 原生设备演示集成到 rd.session.observe 操作中的远程显示状态报告机制
- 新增了 GPU 加速演示功能，使用拥有远程回放连接的 PresentReplay 接口
- 增强了远程显示状态报告，支持 presented/unavailable/unsupported/not_applicable 四种状态
- 完善了无窗口内嵌观察与真实设备呈现状态的分离处理
- 更新了工具参考文档中 rd.session.observe 的返回结构说明
...
**更新** 显著增强了 Android 原生设备演示集成能力，通过 rd.session.observe 操作实现了真正的设备屏幕同步验证。新增的 GPU 加速演示功能使用拥有远程回放连接的 PresentReplay 接口，提供准确的设备呈现状态报告。新的远程显示状态机制支持 presented/unavailable/unsupported/not_applicable 四种状态，确保消费者能够准确判断设备屏幕同步的实际状态。
...
RO["replay_observation.py<br/>观察操作实现"]
SM["session_manager.py<br/>会话管理与远程演示"]
end
subgraph "测试与验证"
...
TRP["test_remote_presentation.py<br/>远程演示验证"]
TRO["test_replay_observation.py<br/>观察操作验证"]
end
DC --> DS
CLI --> DC
SR --> CS
...
- [session_manager.py:302-317](file://rdx/core/session_manager.py#L302-L317)
...
- **新增** Android 原生设备演示集成
  - 通过 PresentReplay 接口实现 GPU 加速的设备屏幕同步
  - 支持四种远程显示状态：presented/unavailable/unsupported/not_applicable
  - 使用拥有远程回放连接进行演示，确保状态准确性
- **新增** 显式守护进程上下文管理
  - 通过 `--daemon-context <id>` 参数实现严格的上下文隔离
  - 每个上下文拥有独立的守护进程实例和状态文件
  - 支持上下文的生命周期管理（启动、停止、清理）

**更新** 显著增强了 Android 原生设备演示集成能力，通过 GPU 加速的 PresentReplay 接口实现了真正的设备屏幕同步验证。新的远程显示状态机制提供了准确的设备呈现状态报告，确保消费者能够区分实际的屏幕同步与简单的图像导出。同时，显式守护进程上下文管理机制确保了多任务环境下的状态独立性。
...
- [client.py:576-832](file://rdx/daemon/client.py#L576-L832)
- [server.py:70-157](file://rdx/daemon/server.py#L70-L157)
- [replay_observation.py:116-132](file://rdx/replay_observation.py#L116-L132)
- [session_manager.py:302-317](file://rdx/core/session_manager.py#L302-L317)

## 架构总览
会话模型围绕"上下文"组织，每个上下文包含若干会话与捕获文件。CLI 通过操作上下文快照实现状态查询与更新；运行时负责会话生命周期与预览显示态计算。

**更新** 增强了 Android 原生设备演示集成，通过 GPU 加速的 PresentReplay 接口实现了真正的设备屏幕同步验证。新的远程显示状态机制独立于 PNG 导出，确保消费者能够准确判断设备屏幕同步的实际状态。
...
SR --> OBS["观察操作<br/>replay_observation.py"]
OBS --> PRESENT["远程演示<br/>session_manager.py"]
...
PRESENT --> DEVICE["Android 设备<br/>PresentReplay 接口"]
```
...
- [preview_window.py:282-308](file://rdx/preview_window.py#L282-L308)
- [agent-model.md:5-8](file://docs/agent-model.md#L5-L8)
- [replay_observation.py:120-132](file://rdx/replay_observation.py#L120-L132)
- [session_manager.py:302-317](file://rdx/core/session_manager.py#L302-L317)

## 详细组件分析

### Android 原生设备演示集成
**新增** 系统现在支持通过 GPU 加速的 PresentReplay 接口实现真正的设备屏幕同步：

- **GPU 加速演示**：使用拥有远程回放连接的 PresentReplay 接口进行设备屏幕同步
- **状态报告机制**：支持 presented/unavailable/unsupported/not_applicable 四种状态
- **序列验证**：通过单调递增的序列号确保演示结果的时效性
- **错误处理**：区分绑定能力缺失、表面传输失败、渲染失败等不同错误类型

```mermaid
sequenceDiagram
participant OBS as "观察操作"
participant SM as "会话管理器"
participant RM as "远程服务器"
participant DEV as "Android 设备"
OBS->>SM : present_remote(session_id, event_id, texture_id)
SM->>RM : PresentReplay(event_id, texture_id)
RM->>DEV : GPU 演示请求
DEV-->>RM : 成功确认 + 序列号
RM-->>SM : (status, sequence)
SM-->>OBS : remote_display 状态更新
```

**图表来源**
- [replay_observation.py:120-132](file://rdx/replay_observation.py#L120-L132)
- [session_manager.py:302-317](file://rdx/core/session_manager.py#L302-L317)
- [test_remote_presentation.py:28-35](file://tests/test_remote_presentation.py#L28-L35)

**章节来源**
- [replay_observation.py:116-132](file://rdx/replay_observation.py#L116-L132)
- [session_manager.py:302-317](file://rdx/core/session_manager.py#L302-L317)
- [test_remote_presentation.py:28-35](file://tests/test_remote_presentation.py#L28-L35)
- [test_replay_observation.py:62-68](file://tests/test_replay_observation.py#L62-L68)
...
### 观察操作的远程显示状态报告
**新增** rd.session.observe 操作现在包含完整的远程显示状态报告：

- **状态枚举**：presented（成功演示）、unavailable（不可用）、unsupported（不支持）、not_applicable（不适用）
- **序列验证**：通过单调递增的序列号确保演示结果的时效性和新鲜度
- **错误分类**：区分绑定能力缺失、表面传输失败、渲染失败等不同错误类型
- **无颜色输出**：特殊处理无颜色输出的情况，返回 unavailable 状态并记录清除序列

```mermaid
flowchart TD
Observe["observe 操作"] --> ApplyEvent["应用事件"]
ApplyEvent --> SelectTarget["选择目标纹理"]
SelectTarget --> BackendCheck{"后端类型?"}
BackendCheck --> |本地| NotApplicable["设置 not_applicable"]
BackendCheck --> |远程| PresentRemote["调用 present_remote"]
PresentRemote --> StatusCheck{"演示状态?"}
StatusCheck --> |成功| Presented["设置 presented + 序列号"]
StatusCheck --> |失败| Unavailable["设置 unavailable + 原因"]
StatusCheck --> |不支持| Unsupported["设置 unsupported + 原因"]
NotApplicable --> Return["返回结果"]
Presented --> Return
Unavailable --> Return
Unsupported --> Return
```
...
- [session_manager.py:302-317](file://rdx/core/session_manager.py#L302-L317)
- [test_replay_observation.py:62-68](file://tests/test_replay_observation.py#L62-L68)
- [tool-reference.md:194](file://docs/tool-reference.md#L194)
...
- **新增** Android 原生设备演示集成依赖会话管理器中的 PresentReplay 接口。

**更新** 增强了对守护进程上下文依赖的管理，确保在不同上下文间切换时的正确性和一致性。每个上下文都有独立的依赖链和资源管理。新的容量管理机制依赖于活跃占用检测，提高了资源控制的准确性。Android 原生设备演示集成为观察操作增加了远程演示依赖。
...
TRP["test_remote_presentation.py"] --> SM
TRO["test_replay_observation.py"] --> RO
```
...
- **新增** Android 原生设备演示集成涉及 GPU 操作和网络通信，应避免频繁调用演示操作。演示操作的序列号验证确保每次演示都是新鲜的，但也会增加一定的开销。
...
- **新增** Android 原生设备演示故障
  - 演示状态为 unsupported：检查远端设备是否安装了匹配的 RenderDoc 运行时和辅助程序
  - 演示状态为 unavailable：检查网络连接、设备表面状态和渲染管线
  - 演示状态为 not_applicable：确认当前操作是在本地环境中执行
  - 序列号验证失败：检查是否存在重复的演示请求或连接重置
...
- [session_manager.py:302-317](file://rdx/core/session_manager.py#L302-L317)
- [test_remote_presentation.py:48-51](file://tests/test_remote_presentation.py#L48-L51)
...
**更新** 通过增强的 Android 原生设备演示集成和显式守护进程上下文管理机制，进一步提升了多会话环境的隔离性和可靠性。新的 GPU 加速演示功能使用 PresentReplay 接口实现了真正的设备屏幕同步验证，四种远程显示状态报告机制确保消费者能够准确判断设备屏幕同步的实际状态。基于活跃占用的限制机制提高了资源管理的准确性，确保只有真正活跃的上下文才消耗容量。这些改进特别适合复杂的自动化任务和并发处理场景。
...
- 常用 CLI 命令
  ...
  - **新增** 观察操作：rdx observe --out-path <path> --event-id <id> --json
...
- **新增** 合理使用观察操作的远程演示功能，避免频繁的 GPU 操作
- **新增** 检查远程显示状态报告，确保设备屏幕同步的实际状态
