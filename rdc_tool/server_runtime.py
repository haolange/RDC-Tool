"""
RDC-Tool daemon/runtime server with registry-driven tool registration.

- Registers all catalog-defined tools from `rdc_tool/spec/tool_catalog.json`
...
from rdc_tool.config import RdcToolConfig
from rdc_tool.context_snapshot import (
...
from rdc_tool.preview_window import PreviewWindowHost
from rdc_tool.core.artifact_publisher import ArtifactPublisher
from rdc_tool.core.contracts import TSV_FORMAT_VERSION, canonical_error, env_bool
from rdc_tool.core.errors import CoreError, RuntimeToolError
from rdc_tool.core.engine import CoreEngine, ExecutionContext
from rdc_tool.core.renderdoc_status import (
    build_renderdoc_error_details,
    status_ok as _rd_status_ok,
    status_text as _rd_status_text,
)
from rdc_tool.progress import ProgressReporter
from rdc_tool.timeout_policy import PIXEL_HISTORY_TIMEOUT_S, remote_connect_timeout_ms
from rdc_tool.core.event_graph import EventGraphService
from rdc_tool.core.patch_engine import PatchEngine
from rdc_tool.core.perf_service import PerfService
from rdc_tool.core.pipeline_service import PipelineService
from rdc_tool.core.render_service import RenderService
from rdc_tool.core.session_manager import SessionError, SessionManager
from rdc_tool.models import PatchSpec, ShaderStage, _new_id
from rdc_tool.remote_bootstrap import (
    ConnectionBudget,
    _connection_budget,
...
from rdc_tool.runtime_paths import artifacts_dir, ensure_runtime_dirs, runtime_root
from rdc_tool.runtime_state import (
    append_runtime_log,
    clear_context_state,
    context_state_path,
...
from rdc_tool.utils.artifact_store import ArtifactStore
from rdc_tool.core.tsv_projection import project_rows, to_tsv_string

logger = logging.getLogger("rdc_tool.server")
_CURRENT_CONTEXT_ID: contextvars.ContextVar[str | None] = contextvars.ContextVar('rdc_tool_current_context_id', default=None)
_CURRENT_PROGRESS_REPORTER: contextvars.ContextVar[ProgressReporter | None] = contextvars.ContextVar(
    "rdc_tool_current_progress_reporter",
    default=None,
)
...
def _runtime_context_id() -> str:
    ...
    return normalize_context_id(os.environ.get("RDC_TOOL_CONTEXT_ID") or "default")
...
_config: Optional[RdcToolConfig] = None
...
def _apply_runtime_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = dict(cfg or {})
    if _config is None:
        _config = RdcToolConfig.from_env()
...
def _occupying_context_ids() -> set[str]:
    from rdc_tool.daemon.client import list_occupying_context_ids
...
def _preview_title(
    ...
    *,
    ...
    return (
        f"RDC-Tool Preview [{context_id}] [{backend}] {session_id} "
...
def _remote_target_control_client_name(handle: RemoteHandle) -> str:
    ...
    return f"rdc-tool-{remote_id}"[:64]
...
async def runtime_startup() -> None:
    ...
    _config = RdcToolConfig.from_env()
    artifact_root = Path(os.environ.get("RDC_TOOL_ARTIFACT_DIR", str(_config.artifact.store_path))).resolve()
    ...
    _record_log("info", "RDC-Tool runtime initialized")
...
async def runtime_shutdown(*, clear_context_state: bool = True) -> None:
    ...
    _record_log("info", "RDC-Tool runtime shutdown complete")
...
async def _dispatch_core(action: str, args: Dict[str, Any]) -> str:
    ...
    if action == "set_config":
        ...
        if _artifact_store is not None:
            _artifact_store = ArtifactStore(root=Path(str((_config or RdcToolConfig()).artifact.store_path)).resolve())
...
async def _dispatch_session(action: str, args: Dict[str, Any]) -> str:
    ...
    if action == "get_context":
        ...
        try:
            from rdc_tool.daemon.client import load_daemon_state
...
async def _dispatch_capture(action: str, args: Dict[str, Any]) -> str:
    ...
    if action == "get_thumbnail":
        from rdc_tool.core.capture_queries import read_thumbnail
        _require(args, "capture_file_id")
                ...
                return _err(
                    ...
                    details={
                        ...
                        "recovery_steps": [
                            ...
                            "rdc-tool call rd.capture.close_replay --args-file close_replay_args.json --format json",
                            "rdc-tool daemon stop --daemon-context <context>",
                        ],
                    },
                )
        state = _context_state(_runtime_context_id())
...
async def _dispatch_event(action: str, args: Dict[str, Any]) -> str:
    if action == "get_api_calls":
        from rdc_tool.core.capture_queries import query_calls
        _require(args, "session_id")
...
async def _dispatch_pipeline(action: str, args: Dict[str, Any]) -> str:
    ...
    from rdc_tool.core.pipeline_service import PIPELINE_SECTIONS
...
async def _dispatch_resource(action: str, args: Dict[str, Any]) -> str:
    ...
    if action == "get_usage":
        ...
        from rdc_tool.core.replay_facts import usage_access
...
async def _export_mesh_file(args: Dict[str, Any]) -> str:
    ...
    controller = await _get_controller(session_id)
    if space == "vs_input":
        from rdc_tool.core.mesh_data import vertex_input, input_obj, MeshReadError
        try:
    ...
    lines = [f"# RDC-Tool post-VS positions, event {event_id}"]
...
async def _dispatch_mesh(action: str, args: Dict[str, Any]) -> str:
    if action == "get_drawcall_mesh_config":
        ...
        from rdc_tool.core.mesh_data import vertex_input
        controller = await _get_controller(session_id)
        try:
    ...
    if action == "get_post_transform_data":
        ...
        from rdc_tool.core.replay_facts import mesh_attributes, post_transform_outputs
...
async def _dispatch_perf(action: str, args: Dict[str, Any]) -> str:
    ...
    if action == "get_frame_timing":
        ...
        from rdc_tool.core.replay_read import preserving_event, ReplayRestoreError
        controller = await _get_controller(session_id)
        original_event = _active_event(session_id)
        async def measure():
...
async def _vfs_call(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    from rdc_tool import server as server_entry
...
async def _vfs_root_node() -> Dict[str, Any]:
    ...
    return _vfs_node("/", kind="directory", title="RDC-Tool VFS root", entries=entries)
