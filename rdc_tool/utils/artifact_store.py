"""
rdc-tool 的内容寻址 artifact 存储（CAS）。
...
Artifacts 通过 ``rdc-tool://`` URI 引用，并由 :class:`rdc_tool.models.ArtifactRef`
描述。
"""
...
from rdc_tool.models import ArtifactRef
...
def _build_uri(sha256: str) -> str:
    """构建 artifact 的标准 ``rdc-tool://`` URI。"""
    return f"rdc-tool://artifacts/{sha256[:2]}/{sha256[2:4]}/{sha256}"
...
class ArtifactStore:
    ...
    async def store(
        ...
        -------
        ArtifactRef
            A reference containing the ``rdc-tool://`` URI, SHA256, MIME type,
