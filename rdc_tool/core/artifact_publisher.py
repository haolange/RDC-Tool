...
from rdc_tool.runtime_paths import artifacts_dir
...
class ArtifactPublisher:
    def __init__(self) -> None:
        self.mode = str(os.environ.get("RDC_TOOL_ARTIFACT_MODE", "local")).strip().lower()
        self.remote_min_bytes = int(os.environ.get("RDC_TOOL_REMOTE_ARTIFACT_MIN_BYTES", "65536"))
        self._s3_endpoint = os.environ.get("RDC_TOOL_S3_ENDPOINT")
        self._s3_bucket = os.environ.get("RDC_TOOL_S3_BUCKET")
        self._s3_region = os.environ.get("RDC_TOOL_S3_REGION")
        self._s3_access_key = os.environ.get("RDC_TOOL_S3_ACCESS_KEY")
        self._s3_secret_key = os.environ.get("RDC_TOOL_S3_SECRET_KEY")
        self._s3_presign_seconds = int(os.environ.get("RDC_TOOL_S3_PRESIGN_SECONDS", "86400"))
    ...
    def _upload_s3(self, path: Path) -> Optional[str]:
        ...
        key = f"rdc_tool/{time.strftime('%Y/%m/%d')}/{path.name}"
...
def default_artifact_root() -> Path:
    return Path(os.environ.get("RDC_TOOL_ARTIFACT_DIR", str(artifacts_dir()))).resolve()
