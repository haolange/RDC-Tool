"""rdc-tool 的 report bundle 构建器。

从已完成的 :class:`~rdc_tool.models.TaskState` 生成自包含的 debug report bundle，
包含：

* ``report.json`` —— 机器可读的结构化报告，包含 RDC-Tool 规范的全部字段。
...
logger = logging.getLogger("rdc_tool.core.report_builder")
...
<title>RDC-Tool Debug Report &mdash; {{ task_id }}</title>
<style>
/* ---- Reset & base ---- */
...
class ReportBuilder:
    ...
    async def build_bundle(
        ...
        ----------
        task_state:
            已完成 debug session 的完整 :class:`~rdc_tool.models.TaskState`。
        artifact_store:
            会话中使用的 :class:`~rdc_tool.utils.artifact_store.ArtifactStore`
    ...
    def _generate_json(self, task_state: TaskState) -> Dict[str, Any]:
        ...
        包含 RDC-Tool report schema 的全部字段：
    ...
    def _generate_markdown(
        ...
        # -- Title（标题）------------------------------------------------
        lines.append(
            f"# RDC-Tool Debug Report: {bug_type.upper()} "
...
<head>
<meta charset="utf-8">
<title>RDC-Tool Debug Report - {task_state.task_id}</title>
<style>
